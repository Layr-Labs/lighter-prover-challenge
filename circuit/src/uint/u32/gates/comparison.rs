    ...
    for ComparisonGenerator<F, D>
{
    ...
    fn run_once(
        ...
        debug_assert!(self.gate.num_chunks <= 64);
        let mut first_input_chunks = [F::ZERO; 64];
        let mut second_input_chunks = [F::ZERO; 64];
        let mut first_acc = first_input_u64;
        let mut second_acc = second_input_u64;
        for i in 0..self.gate.num_chunks {
            first_input_chunks[i] = F::from_canonical_u64(first_acc % chunk_size);
            second_input_chunks[i] = F::from_canonical_u64(second_acc % chunk_size);
        ...
        for i in 0..self.gate.num_chunks {
            let first = first_input_chunks[i];
            let second = second_input_chunks[i];
            let equal = first == second;
            let difference = second - first;
            let equality_dummy = if equal { F::ONE } else { F::ONE / difference };
            let intermediate_value = if equal {
                most_significant_diff_so_far
            } else {
                most_significant_diff_so_far = difference;
                F::ZERO
            };

            out_buffer.set_wire(local_wire(self.gate.wire_first_chunk_val(i)), first)?;
            out_buffer.set_wire(local_wire(self.gate.wire_second_chunk_val(i)), second)?;
        ...
        let mut two_n_plus_msd = (two_n + most_significant_diff).to_canonical_u64();
        for i in 0..self.gate.chunk_bits() + 1 {
            out_buffer.set_wire(
                local_wire(self.gate.wire_most_significant_diff_bit(i)),
                F::from_canonical_u64(two_n_plus_msd & 1),
            )?;
            two_n_plus_msd >>= 1;
        }
