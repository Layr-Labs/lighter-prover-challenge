    ...
    fn eval_unfiltered_base_batch_accumulate(
        ...
        for i in 0..self.num_ops {
            ...
            for j in (0..Self::num_limbs()).rev() {
                ...
                let recomposed = if j < Self::num_result_limbs() {
                    &mut combined_result
                } else {
                    &mut combined_carry
                };
                for p in 0..n {
                    recomposed[p] = recomposed[p] * base_limb + limb[p];
                }
            }
...
mod batch_tests {
    ...
    fn direct_filtered_accumulation_matches_materialized_batch() {
        ...
        let gate =
            U32AddManyGate::<F, D>::new_from_config(&CircuitConfig::standard_recursion_config(), 5);
