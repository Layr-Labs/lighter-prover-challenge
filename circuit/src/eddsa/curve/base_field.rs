...
#[inline]
fn exp_goldilocks_p_minus_one_over_two<T: Field>(x: T) -> T {
...
mod tests {
    ...
    fn legendre_symbol_matches_inverse_reference() {
        ...
        let check = |x| assert_eq!(exp_goldilocks_p_minus_one_over_two(x), reference(x));
    ...
    #[test]
    fn sqrt_exponent_matches_inverse_reference() {
        let reference = |x: QuinticExtension<F>| {
            let v = x.exp_power_of_2(31);
            x * v.exp_power_of_2(32) * v.try_inverse().unwrap_or(QuinticExtension::<F>::ZERO)
        };
        let check = |x| {
            assert_eq!(
                x * exp_goldilocks_p_minus_one_over_two(x),
                reference(x)
            )
        };

        check(QuinticExtension::<F>::ZERO);
        check(QuinticExtension::<F>::ONE);
