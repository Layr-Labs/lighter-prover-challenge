...
/// Proves the native jump transition (`JumpState::advance`,
/// `block_tx_native.rs`) exact against the REAL in-circuit transition on
/// ACTIVE transaction data, without proving full leaf circuits: a small
/// circuit folds the actual (private) `define_jump_step` over witness-supplied
/// tx fields — the same folding `define_tx_loop` performs, minus the per-tx
/// state execution whose outputs (`tx_new_*_root`) the witness supplies with
/// the natively recovered end roots. Every case is proved (so the range
/// checks, conditional asserts and `is_empty == is_padding` connect are all
/// enforced) and the circuit's `new_jump` public inputs are diffed against the
/// native result.
#[cfg(test)]
mod jump_step_oracle_tests {
    use plonky2::hash::hash_types::HashOut;

    use super::*;
    use crate::block_tx::{JUMP_STATE_SIZE, JumpState};
    use crate::block_tx_native::testkit;
    use crate::block_tx_native::TxEndRoots;
    use crate::tx::Tx;
    ...
    struct JumpOracle {
        ...
        old_jump: JumpStateTarget,
        slots: Vec<SlotTargets>,
    }

    /// Public inputs: `old_jump` (27) then the folded `new_jump` (27).
    fn build_oracle(width: usize) -> JumpOracle {
        let mut builder = Builder::new(CIRCUIT_CONFIG);
        let old_jump = JumpStateTarget::new(&mut builder);
        old_jump.register_public_inputs(&mut builder);
        let slots: Vec<SlotTargets> = (0..width)
            .map(|_| SlotTargets {
            ...
            .collect();
        let mut jump = old_jump;
        for slot in &slots {
            jump = BlockTxCircuit::define_jump_step(
                &mut builder,
                jump,
                slot.tx_index,
                slot.tx_type,
        ...
        jump.register_public_inputs(&mut builder);
    ...
    fn set_jump(
        ...
        target: &JumpStateTarget,
        value: &JumpState<F>,
    ) -> Result<()> {
        pw.set_target(target.last_active_tx_index, value.last_active_tx_index)?;
        pw.set_hash_target(target.prev_new_state_root, value.prev_new_state_root)?;
        pw.set_hash_target(target.prev_new_delta_root, value.prev_new_delta_root)?;
        pw.set_target(target.run_start_prev_index, value.run_start_prev_index)?;
        pw.set_hash_target(target.run_start_old_state_root, value.run_start_old_state_root)?;
        pw.set_hash_target(target.run_start_old_delta_root, value.run_start_old_delta_root)?;
        pw.set_hash_target(target.coverage_hash, value.coverage_hash)?;
        pw.set_hash_target(target.claims_hash, value.claims_hash)?;
        pw.set_target(target.tx_count, value.tx_count)?;
        Ok(())
    }

    /// One chunk through the oracle circuit: same tx field encodings as
    /// `set_tx_target` (`from_canonical_u8`/`from_canonical_u64`), natively
    /// recovered end roots in the slots the real circuit computes in-circuit
    /// (zeros for padding slots, where `is_active` gates them out of every
    /// select). Returns the circuit-computed `new_jump`.
    fn circuit_advance(
        oracle: &JumpOracle,
        old_jump: &JumpState<F>,
        txs: &[Tx<F>],
        end_roots: &[Option<TxEndRoots>],
    ) -> JumpState<F> {
        assert_eq!(txs.len(), oracle.slots.len(), "chunk width mismatch");
        let mut pw = PartialWitness::new();
        set_jump(&mut pw, &oracle.old_jump, old_jump).expect("old jump witness");
        for ((slot, tx), roots) in oracle.slots.iter().zip(txs).zip(end_roots) {
            pw.set_target(slot.tx_index, F::from_canonical_u64(tx.tx_index))
                .expect("tx index witness");
            pw.set_target(slot.tx_type, F::from_canonical_u8(tx.tx_type))
                .expect("tx type witness");
            pw.set_hash_target(slot.old_state_root, tx.old_state_root)
                .expect("old state root witness");
            pw.set_hash_target(slot.old_delta_root, tx.old_account_delta_tree_root)
                .expect("old delta root witness");
            let (new_state, new_delta) = roots.map_or((HashOut::ZERO, HashOut::ZERO), |roots| {
                (roots.new_state_root, roots.new_delta_root)
            });
            pw.set_hash_target(slot.new_state_root, new_state)
                .expect("new state root witness");
            pw.set_hash_target(slot.new_delta_root, new_delta)
                .expect("new delta root witness");
        }
        let proof = oracle
            .data
            .prove(pw)
            .expect("oracle proof failed — witness violates the real jump-step constraints");
        oracle
            .data
            .verify(proof.clone())
            .expect("oracle proof must verify");
        assert_eq!(
            &proof.public_inputs[..JUMP_STATE_SIZE],
            old_jump.to_vec().as_slice(),
            "old jump public inputs must echo the witness"
        );
        JumpState::from_public_inputs(&proof.public_inputs[JUMP_STATE_SIZE..2 * JUMP_STATE_SIZE])
    }

    /// The exactness result the parallel-leaf prover rests on: for every chunk
    /// of every active scenario (multi-run interleavings, first-chunk NEG_ONE
    /// sentinels with and without jumps, coverage/claims folds onto nonzero
    /// accumulators, continuity runs, wide gaps, active+padding tails, both
    /// widths), the native transition equals the constraint system's own
    /// folded jump state.
    #[test]
    fn jump_step_circuit_matches_native_transition_on_active_scenarios() {
        let heavy_oracle = build_oracle(testkit::HEAVY_WIDTH);
        let light_oracle = build_oracle(testkit::LIGHT_WIDTH);
        for (name, paths) in testkit::all_scenario_paths() {
            ...
            for (oracle, chunks, path_name) in [
                (&heavy_oracle, &scenario.heavy_chunks, "heavy"),
                (&light_oracle, &scenario.light_chunks, "light"),
            ] {
                let mut jump = initial;
                for (chunk_index, chunk) in chunks.iter().enumerate() {
                    let (txs, end_roots) = testkit::chunk_txs_and_ends(chunk);
                    let native = jump.advance(&txs, &end_roots);
                    let proved = circuit_advance(oracle, &jump, &txs, &end_roots);
                    assert_eq!(
                        native.to_vec(),
                        proved.to_vec(),
                        "{name}/{path_name} chunk #{chunk_index}: native jump transition \
                         diverged from the in-circuit fold"
                    );
                    jump = native;
                }
            }
