...
use plonky2::fri::oracle::{EvenColumns, PolynomialBatch};
use plonky2::plonk::prover::half_domain_split_wanted;
...
pub fn deserialize_embedded<T: DeserializeOwned>(bytes: &[u8]) -> Result<(T, CircuitData<F, C, D>)> {
    ...
    // Even-row companion of the constants columns, mirroring the builder. The
    // constants/sigmas LDE is circuit-fixed, so this stride-2 copy is paid
    // once per load instead of once per proof, and it is what lets the
    // half-domain quotient dispatch carry the gates that read gate constants:
    // that dispatch indexes the constants store with the wires row count,
    // which is half the LDE. Sigma columns are excluded — those kernels never
    // read them.
    let constants_even_columns = if half_domain_split_wanted(&common) {
