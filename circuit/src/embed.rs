...
pub fn deserialize_embedded<T: DeserializeOwned>(
    ...
    // Night redraw of 806087d: adopt when the CSD1 sidecar decodes, else
    // hash. A bad blob is a slow draw, not a fail.
    ...
    let constants_sigmas_commitment = if let Some(adopted) = adopted {
        match PolynomialBatch::<F, C, D>::from_values_adopt(
            ...
            Some(batch) => batch,
            None => {
                // Layout decline: cannot recover the consumed values; this
                // path is unused when the sidecar matches (806087d/8d98704).
                bail!("constants/sigmas Merkle adopt declined after decode")
            }
        }
    } else {
