    ...
    fn prune_identity_transition_runs(
        ...
        while run_start < original_len {
            ...
            for run_end in (run_start + 1)..=last_boundary {
                ...
                if all_silent && Self::same_visible_state_boundary(&txs[run_start], end_boundary) {
        ...
        let chunk_count = |heavy: usize, light: usize| {
            heavy.div_ceil(tx_per_proof).max(1) + light.div_ceil(light_tx_per_proof).max(1)
        };
...
mod tests {
    ...
    fn fixture_tx() -> Tx<F> {
        let block: Block<F> = serde_json::from_slice(include_bytes!("../../bench/bench_test.json"))
