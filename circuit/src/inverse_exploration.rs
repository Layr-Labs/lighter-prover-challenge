...
//! timed head-to-head. Not compiled into the prover.
...
fn binary_xgcd_inverse(x: u64) -> u64 {
    ...
    while u != 1 && v != 1 {
        ...
        if u >= v {
            u -= v;
            b = if b >= c { b - c } else { b.wrapping_add(P.wrapping_sub(c)) };
        } else {
            v -= u;
            c = if c >= b { c - b } else { c.wrapping_add(P.wrapping_sub(b)) };
        }
    }
    if u == 1 { b } else { c }
}
...
fn time_inverse_candidates() {
    ...
    // safegcd timing disabled until its port is fixed (non-terminating on some inputs).
}
