...
Keep and submit for M4 validation. This is not justified by the best local
draw; it is justified by a directly measured post-proof teardown boundary, a
monotonic deletion of unobservable work, both local aggregate statistics
remaining directionally positive, and trusted proof verification. The ranked
result should be recorded as a small fixed-cost experiment. A rejection does
not invalidate the safety argument; it would mean the deletion is too small
relative to official variance or transfers at a lower magnitude.
