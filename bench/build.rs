...
fn main() {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            // The final block circuit is the sixth. Every scored worker builds
            // it at runtime today: `build_block_circuit` measured 3,285 ms of a
            // 24,607 ms process on an M4 Pro, 2,401 ms of that inside
            // `preprocess`, and the profile puts it on the critical path — the
            // lane spans 808-5,469 ms and its join blocks for exactly that
            // window, so nothing else proceeds behind it.
            //
            // Nothing about it depends on the witness. `BlockCircuit::define`
            // takes `CIRCUIT_CONFIG`, `ON_CHAIN_OPERATIONS_LIMIT`, and the
            // `common`/`verifier_only` of the pre-execution and two chain
            // circuits — all compile-time constants, and all three are already
            // built right here. So build it here, where the time is untimed,
            // exactly as the other five are.
            let block_blob = {
