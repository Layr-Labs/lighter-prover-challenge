...
fn build_path_blobs(
    ...
    // Returned so the final block circuit can be defined from it here, at
    // compile time: `BlockCircuit::define` reads only `common` and
    // `verifier_only` of its inputs, both fully determined by this build.
...
fn main() {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            // Sixth blob: the final block circuit, the one startup circuit still
            // constructed inside every scored worker (measured 5.20 s, 4.14 s of
            // it in `preprocess`) on a lane that overlaps transaction proving.
