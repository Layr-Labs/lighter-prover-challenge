...
fn build_path_blobs(
    ...
    // The chain `CircuitData` is returned so the final block circuit can be
    // defined from it here, at compile time. `BlockCircuit::define` reads only
    // `common` and `verifier_only` of its three inputs, both of which are
    // fully determined by the circuits already built above.
...
fn main() {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            // Sixth blob: the final block circuit. `Circuits::build_block_circuit`
            // used to construct this inside every scored worker (measured ~5.2 s,
            // ~4.1 s of it in `preprocess`) on a lane that runs concurrently with
            // transaction proving, so it competed for cores during the busiest
            // phase of the run. It depends only on the three circuits above, all
            // of which are themselves fixed at compile time, so it is exactly as
            // precomputable as they are.
