...
fn build_pipeline_archive(out_dir: &Path) {
    ...
    let source = match std::fs::read_to_string(&shader) {
        Ok(source) => source,
        Err(error) => return write_empty(&format!("cannot read shader source: {error}")),
    };
    let Some(device) = Device::system_default() else {
        return write_empty("no Metal device on the build host");
    };
    let library = match device.new_library_with_source(&source, &CompileOptions::new()) {
        Ok(library) => library,
        Err(error) => return write_empty(&format!("shader compilation failed: {error}")),
    };
    let archive = match device.new_binary_archive_with_descriptor(&BinaryArchiveDescriptor::new()) {
        Ok(archive) => archive,
        Err(error) => return write_empty(&format!("cannot create binary archive: {error}")),
    };
    for name in KERNELS {
        let function = match library.get_function(name, None) {
            Ok(function) => function,
            Err(error) => return write_empty(&format!("kernel {name} unavailable: {error}")),
        };
        let descriptor = ComputePipelineDescriptor::new();
        descriptor.set_compute_function(Some(&function));
        if let Err(error) = archive.add_compute_pipeline_functions_with_descriptor(&descriptor) {
            return write_empty(&format!("cannot add {name} to the archive: {error}"));
        }
    }
    ...
    if let Err(error) = archive.serialize_to_url(&url) {
        return write_empty(&format!("cannot serialize the archive: {error}"));
    }

    let bytes = std::fs::metadata(&path).map_or(0, |m| m.len());
    if bytes < 64 {
        return write_empty("serialized archive is empty or tiny");
    }
    println!(
        "cargo:warning=pipeline archive: {:.2} MiB (embedded into prove via include_bytes!)",
        bytes as f64 / (1024.0 * 1024.0)
    );
    // Optional sidecar next to the crate root for offline inspection / A/B —
    // never the load path the ranked worker depends on.
    let sidecar = manifest.join(PIPELINE_ARCHIVE_NAME);
    let _ = std::fs::copy(&path, &sidecar);
}
