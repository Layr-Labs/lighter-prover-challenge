...
//! Builds the six fixed startup/final circuits at compile time and serializes them into
...
fn build_path(
    ...
    // Keep the parent-blob serialization at the same point in construction as
    // the original five-blob build script. Some prover-only setup is global,
    // so this ordering is part of the embedded/runtime identity contract.
    ...
    (tx_target, tx_data, chain_target, chain_data, tx_blob, chain_blob)
}
...
fn main() {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            // parallel with the heavy and light transaction paths. The final
            // block circuit then consumes those fixed circuit descriptions;
            // this work is deliberately moved out of scored worker startup.
            let ((_pre_target, pre_data, pre_blob), (heavy, light)) = rayon::join(
                || {
