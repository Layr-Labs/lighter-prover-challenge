...
// Caches are stored raw (u64 LE length prefix + payload), not compressed:
// the worker borrows the payload directly from the binary image with no
// decompression pass or transient allocation, and the five caches total
// ~345 MB — comfortably under the harness's 512 MiB staged-binary cap.
fn write_framed(out_dir: &Path, name: &str, bytes: &[u8]) {
    ...
    let path = out_dir.join(format!("{name}.bin.raw"));
