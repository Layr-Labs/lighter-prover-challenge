...
/// shader it was generated from, and that stamp must match the shader being
/// shipped.
///
/// This is the cheap half — a string compare, no Metal — and it is the half that
/// catches the failure we actually hit: the tip rewrote `poseidon2.metal` under
/// an archive built from the previous revision. It matters because attaching a
/// non-matching archive is NOT free; it measured ~+140 ms per worker for
/// nothing, so "detect and do not attach" is worth more than "attach and hope".
#[test]
