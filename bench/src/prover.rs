        ...
        std::thread::scope(|scope| {
            ...
            let block_circuit_handle = std::thread::Builder::new()
                ...
                .spawn_scoped(scope, move || {
                    ...
                    // Seed directly into representative slots. The previous path first built a
                    // target-keyed PartialWitness and then replayed every entry into the same
                    // partition, adding an allocation, per-target hashing and a full copy pass.
                    let mut pending = PendingPartitionWitness::start_seeded(
