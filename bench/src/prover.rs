...
pub fn prove_block(mut block: Block<F>, circuits: &Circuits) -> Proof {
    ...
    let (light_chain_proof, heavy_chain_proof, (block_target, block_data)) =
        std::thread::scope(|scope| {
            // The final block circuit is only consumed after both chain proofs
            // exist, so its build — the largest single circuit build — hides
            // behind the whole transaction-proving window. Mechanism validated
            // officially by submission `31a75fe` (+5.55%, promotion failed for
            // non-score reasons, so it is re-landed here).
            ...
            let heavy_chain_proof = heavy_handle
                .join()
                .unwrap_or_else(|panic| std::panic::resume_unwind(panic));
            let block_circuit = block_circuit_handle
                .join()
                .unwrap_or_else(|panic| std::panic::resume_unwind(panic));
