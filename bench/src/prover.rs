...
// 14 cores and 48 GiB against an 8.1-8.7 GiB per-worker peak, depth 6 means
// ~30 transaction proofs in flight machine-wide and the memory/allocator
// pressure that the ~9.5 GiB collapse note describes.
//
// Measured with two concurrent workers on one box (the smallest harness that
// reproduces GPU sharing and memory contention at all), time until both finish,
// four rounds per depth:
//   depth 6: 55.34 s   depth 5: 53.63 s   depth 4: 47.02 s (-15.1%)
//   depth 3: 46.22 s (-16.5%)             depth 2: 50.16 s (-9.4%)
// Depth 2 being worse than 3 makes this an interior optimum rather than a
// monotone artifact. Depth 4 is chosen over 3 because it captures ~92% of the
// available gain while staying nearer the depth-6 evidence, and because the
// 3-vs-4 gap is inside the run-to-run spread.
