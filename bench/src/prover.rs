...
/// Witness generation for a chain-recursion step runs on the worker thread itself, so give it the
/// same headroom the default main thread has rather than the smaller default for spawned threads.
...
pub fn prove_block(block: &Block<F>, circuits: &Circuits) -> Proof {
    ...
    // A chain-recursion step consumes its path's previous chain proof and the transaction proof of
    // the current chunk. The next transaction proof only needs the jump state, which is already
    // available from the current transaction proof's public inputs. Chain recursion therefore runs
    // on a second thread, overlapped with the next transaction proof, while both remain in their
    // original per-path order. The bounded channel keeps at most one completed transaction proof
    // queued, so peak memory grows by one proof rather than by the whole block.
    let (send, recv) = std::sync::mpsc::sync_channel::<(ChunkRoute, Proof)>(1);
