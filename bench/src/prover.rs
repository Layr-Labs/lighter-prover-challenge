    ...
    let (light_chain_proof, heavy_chain_proof, block_target, block_data) =
        std::thread::scope(|scope| {
            let heavy_handle = std::thread::Builder::new()
                .name("heavy-path-then-block-build".into())
                .stack_size(PROVER_THREAD_STACK_BYTES)
                .spawn_scoped(scope, || {
                    ...
                    (heavy_chain_proof, block_target, block_data)
                })
                .expect("heavy transaction and block build thread must start");
            ...
            let (heavy_chain_proof, block_target, block_data) = heavy_handle
                .join()
                .unwrap_or_else(|panic| std::panic::resume_unwind(panic));
            (
                light_chain_proof,
