...
// Opening the producer window from the second step keeps an extra chunk
// proof ahead of the chain one step earlier; the early chain steps are the
// cheapest, so the retention cost arrives before the memory-pressure peak.
...
fn prove_path(
    ...
    std::thread::scope(|scope| {
        ...
        loop {
            ...
            // one-extra-producer buffering as the light path; its chunks are
            // few, so the added retention is bounded by a single proof.
            let max_in_flight =
