        ...
        std::thread::scope(|scope| {
            ...
            let block_circuit_handle = std::thread::Builder::new()
                ...
                .spawn_scoped(scope, move || {
                    ...
                    // change: slow-pool 24.38, 24.21, 25.43, 25.86, 25.48; first
                    // fast-pool draw 30.3408 against a fast-pool mean of 30.21
                    // (sd 0.22, n=21), i.e. about +0.6 sd. Suggestive, one sample.
                    // Slow-pool draws agree in direction: 25.43/25.86/25.48/25.61
                    // against a slow-pool mean of 25.36. Also: under a 2-worker
                    // concurrent harness this build is 52.47s vs the frontier's
                    // 53.87s, where a 1-worker run shows it 2.4% SLOWER — the
                    // sign flips, because 1 worker shares neither GPU nor memory.) The block circuit is leaked and therefore resident
