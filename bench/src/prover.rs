...
// Series draw marker: v8 surface, sample 2 (identical trees deduplicate to
...
fn generate_tx_witness<'a>(
    ...
    // TEMPORARY MEASUREMENT SCAFFOLD (revert before submitting).
    let profile_chunk: Option<usize> = std::env::var("LIGHTER_GEN_PROFILE_CHUNK")
        .ok()
        .and_then(|v| v.parse().ok());
    let profiling = profile_chunk == Some(chunk_index);
    if profiling {
        plonky2::iop::generator::gen_profile::start(tx_data.prover_only.generators.len());
    }
    let t_start = std::time::Instant::now();
    let mut pending = PendingPartitionWitness::start_seeded(
        &tx_data.prover_only,
        &tx_data.common,
        |seeder| BlockTxCircuit::generate_witness_into_no_jump(&block_tx, tx_target, seeder),
    )
    .unwrap_or_else(|error| {
        panic!("{path:?} chunk #{chunk_index} phase-1 witness failed: {error:?}")
    });
    let t_phase1 = t_start.elapsed();
    let pending_after_phase1 = format!("{pending:?}");
    pending
        .feed_seeded(|feeder| {
            BlockTxCircuit::generate_witness_into_jump_only(&block_tx, tx_target, feeder)
        })
        .unwrap_or_else(|error| {
            panic!("{path:?} chunk #{chunk_index} phase-2 witness failed: {error:?}")
        });
    let t_phase2 = t_start.elapsed() - t_phase1;
    let partition_witness = pending.finish().unwrap_or_else(|error| {
        panic!("{path:?} block transaction chunk #{chunk_index} witness generation failed: {error:?}")
    });
    eprintln!(
        "WITSPLIT path={path:?} chunk={chunk_index} phase1_ms={:.2} phase2_ms={:.3} after_phase1={pending_after_phase1}",
        t_phase1.as_secs_f64() * 1e3,
        t_phase2.as_secs_f64() * 1e3,
    );
    if profiling {
        let (stats, rounds, worklist_ns) = plonky2::iop::generator::gen_profile::stop();
        let mut by_id: std::collections::HashMap<String, (u64, u32, u32)> =
            std::collections::HashMap::new();
        let mut total_ns = 0u64;
        for (idx, (ns, runs)) in stats.iter().enumerate() {
            if *runs == 0 {
                continue;
            }
            total_ns += ns;
            let id = tx_data.prover_only.generators[idx].0.id();
            let key = id.split(&[',', '<', '{', '('][..]).next().unwrap_or(&id).to_owned();
            let entry = by_id.entry(key).or_default();
            entry.0 += ns;
            entry.1 += runs;
            entry.2 += 1;
        }
        ...
        eprintln!(
            "GENPROF chunk={chunk_index} generators_total={} ran={} gen_ns={:.2}ms worklist_ns={:.2}ms rounds={} round_sizes={:?}",
            tx_data.prover_only.generators.len(),
            stats.iter().filter(|(_, r)| *r > 0).count(),
            total_ns as f64 / 1e6,
            worklist_ns as f64 / 1e6,
            rounds.len(),
            &rounds[..rounds.len().min(40)],
        );
        for (id, (ns, runs, distinct)) in rows.iter().take(25) {
            eprintln!(
                "GENPROF   {:>9.2}ms  runs={:<8} distinct={:<8} {}",
                *ns as f64 / 1e6,
                runs,
                distinct,
                id
