...
pub(crate) fn prove_block_after_pre(
    ...
    // columns over 2^21 LDE rows. Keep only best-fit stores for those two
    // allocations; retired transaction buffers would otherwise leave up to
    // 2.5 GiB cached during the peak-RSS final phase.
