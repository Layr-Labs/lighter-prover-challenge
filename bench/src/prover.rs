        ...
        std::thread::scope(|scope| {
            ...
            let block_circuit_handle = std::thread::Builder::new()
                ...
                .spawn_scoped(scope, move || {
                    ...
                    // Marker: drop-preproof-1786714000. On the perm-inv-merge
                    // 30.560 keeper. After the pending partition owns the
                    // early assignments, the source recursive proof has no
                    // remaining consumer (`pre_output` already holds the
                    // public-input view). Drop it before the heavy join so
                    // its commitment/opening graph is not live across the
                    // light spine. Ownership only; same witness, same proof.
                    drop(pre_proof);
                    #[cfg(feature = "diagnostic_profile")]
                    let _heavy_wait =
