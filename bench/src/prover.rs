...
use plonky2::iop::generator::{PendingPartitionWitness, generate_partial_witness};
...
pub fn prove_block(mut block: Block<F>, circuits: &Circuits) -> Proof {
    ...
    std::thread::scope(|scope| {
        ...
        // Phase 1: the block witness and the pre-execution proof are available before the chain
        // proofs, so their generators run while the transaction paths are still proving.
        let (block_target, block_data) = block_circuit_handle
            .join()
            .unwrap_or_else(|panic| std::panic::resume_unwind(panic));
        let mut pending = BlockCircuit::witness_inputs_early(&block_target, &block, &pre_proof)
            .and_then(|early_inputs| {
            ...
            .expect("final block early witness failed");

        // Phase 2: feed the chain proofs as their path threads join, then prove.
        ...
        BlockCircuit::witness_inputs_light_chain(&block_target, light_chain_input)
            .and_then(|inputs| pending.feed(inputs))
            .expect("final block light chain witness failed");
        BlockCircuit::witness_inputs_heavy_chain(&block_target, heavy_chain_input)
            .and_then(|inputs| pending.feed(inputs))
            .expect("final block heavy chain witness failed");
