...
/// Proves the block pre-execution circuit. The serial path sets the exclusive
/// GPU phase because nothing else runs; the startup-overlap path must NOT set
/// it, because the remaining circuit loads are still using the GPU normally.
...
pub fn prove_block(mut block: Block<F>, mut circuits: Circuits) -> Proof {
    ...
    plonky2::hash::poseidon2::set_exclusive_gpu_phase(true);
    let pre_proof =
        prove_pre_execution_parallel(&circuits.pre_data, &circuits.pre_target, &BlockPreExec::from_block(&block));
    plonky2::hash::poseidon2::set_exclusive_gpu_phase(false);
