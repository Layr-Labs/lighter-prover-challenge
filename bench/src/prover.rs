...
pub fn prove_block(block: &Block<F>, circuits: &Circuits, tx_per_proof: usize) -> Proofs {
    ...
    for (index, txs) in block.txs.chunks(tx_per_proof).enumerate() {
        let input = BlockTx {
            created_at: block.created_at,
            old_system_config: system_config,
            register_stack_before: registers,
            all_assets_before: assets,
            all_market_details_before: markets,
        ...
        let tx = BlockTxCircuit::prove(&circuits.tx_data, &input, &circuits.tx_target)
        ...
        .expect("block transaction chain proof failed");
    }
