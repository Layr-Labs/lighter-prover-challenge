...
fn chain_step_proof(
    ...
    let result = (|| {
        ...
        let mut pending = PendingPartitionWitness::start_seeded(
            ...
            &chain_data.common,
            |seeder| {
                BlockTxChainCircuit::seed_witness_inputs_early(
                    seeder,
                    chain_target,
                    chain_data,
                    chain_step,
        ...
        let cyclic_proof = previous_proof.as_ref().unwrap_or(base_proof);
        pending.feed_seeded(|feeder| {
            BlockTxChainCircuit::seed_witness_inputs_cyclic(feeder, chain_target, cyclic_proof)
        })?;
