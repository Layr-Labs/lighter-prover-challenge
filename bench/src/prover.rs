...
/// the same allocator. Quiet-host single-worker: depth 6 beats 4 by
/// ~4.6%. Official 5-worker ranked harness inverts that: RealAdii
/// `3fa2966` / `abd4b3d` retuned 6→4 and promoted at 30.944. Five
/// workers × depth 6 is ~30 in-flight proofs machine-wide against
/// the allocator/fault churn a 48 GiB host still pays.
    ...
    let chain_proof = std::thread::scope(|scope| {
        ...
        loop {
            ...
            if next_witness.is_none() {
                ...
                // would shrink the pipelined window toward depth 2. Depth 2
                // lost to 3/4 on RealAdii's two-worker sweep.
                while in_flight
                    .front()
