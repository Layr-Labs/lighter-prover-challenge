...
fn prove_tx_witness(
    ...
    // The Light path runs the sequential transaction proofs main blocks on; each
    // runs on its own spawned thread, which starts at the default QoS.
    // Adopted from tekuuu, who re-aimed the tip's existing helper from the chain
    // spine (~0% of wall) at the path that actually binds.
