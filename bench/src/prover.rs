...
fn chain_step_proof_prepared(
    ...
    BlockTxChainCircuit::witness_inputs_early(
        &mut early_inputs,
        chain_target,
        chain_data,
        chain_step,
    ...
    let mut pending =
        PendingPartitionWitness::start(early_inputs, &chain_data.prover_only, &chain_data.common)
            .unwrap_or_else(|error| {
                panic!("{path:?} block transaction chain step #{chain_step} failed: {error:?}")
            });
    ...
    let mut cyclic_inputs = PartialWitness::new();
    ...
    pending.feed(cyclic_inputs).unwrap_or_else(|error| {
        panic!("{path:?} block transaction chain step #{chain_step} failed: {error:?}")
    });
    BlockTxChainCircuit::prove_prepared(pending, chain_data).unwrap_or_else(|error| {
        panic!("{path:?} block transaction chain step #{chain_step} failed: {error:?}")
    })
}
