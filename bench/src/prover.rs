...
// for experiments.
// The ranked harness invokes workers sequentially (`run_private_sequence`), so
// a depth tuned for multiple simultaneous worker processes is inapplicable.
// In the sequential local harness, depth 6 completed in 50.57 s, ahead of the
// immediately preceding depth-3/4 measurements (~55.7--56.0 s). The runtime
// override below keeps this small scheduling choice easy to remeasure.
