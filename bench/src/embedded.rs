...
impl Circuits {
    ...
    /// Loads the final block circuit from its embedded blob.
    ///
    /// This is the deserialization counterpart of the `block.embed` blob that
    /// `build.rs` now produces. `BlockCircuit::define` reads only `common` and
    /// `verifier_only` of the pre-execution and two chain circuits, all three
    /// of which are already fixed at compile time, so the block circuit is
    /// exactly as precomputable as they are — and it was the one startup
    /// circuit still being constructed inside every scored worker.
...
mod tests {
    ...
    /// Equality oracle for the sixth blob: the embedded final block circuit
    /// must be value-identical to the one `Circuits::build_block_circuit_from_scratch`
    /// constructs at runtime. Same obligation and same helper as
    /// `embedded_matches_rebuilt` covers for the other five.
    #[test]
