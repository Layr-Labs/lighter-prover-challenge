...
mod draw_control {
    ...
    #[test]
    fn zstd_repeated_data_compresses() {
        let raw = vec![7u8; 1 << 20];
        let mut out = Vec::new();
        circuit::embed::write_compressed_section(&mut out, &raw);
        let mut pos = 0usize;
        let decoded = circuit::embed::read_compressed_section(&out, &mut pos)
            .expect("repeated-data zstd section must decode");
        assert_eq!(decoded, raw);
        assert_eq!(pos, out.len());
    }

    #[test]
    fn zstd_mixed_data_round_trips() {
        let mut raw = Vec::new();
        for i in 0..(1 << 18) {
            raw.push(((i * 31 + 7) % 251) as u8);
        }
        let mut out = Vec::new();
        circuit::embed::write_compressed_section(&mut out, &raw);
        let mut pos = 0usize;
        let decoded = circuit::embed::read_compressed_section(&out, &mut pos)
            .expect("mixed-data zstd section must decode");
        assert_eq!(decoded, raw);
        assert_eq!(pos, out.len());
    }

    #[test]
    fn zstd_empty_section_round_trips() {
        let raw: Vec<u8> = Vec::new();
        let mut out = Vec::new();
        circuit::embed::write_compressed_section(&mut out, &raw);
        let mut pos = 0usize;
        let decoded = circuit::embed::read_compressed_section(&out, &mut pos)
            .expect("empty zstd section must decode");
        assert_eq!(decoded, raw);
        assert_eq!(pos, out.len());
    }

    #[test]
    fn zstd_high_entropy_data_round_trips() {
        use plonky2::field::goldilocks_field::GoldilocksField;
        use plonky2::field::types::Sample;
        let mut raw = Vec::with_capacity(1 << 16);
        for _ in 0..(1 << 16) {
            raw.extend_from_slice(&GoldilocksField::rand().0.to_le_bytes());
        }
        let mut out = Vec::new();
        circuit::embed::write_compressed_section(&mut out, &raw);
        let mut pos = 0usize;
        let decoded = circuit::embed::read_compressed_section(&out, &mut pos)
            .expect("high-entropy zstd section must decode");
        assert_eq!(decoded, raw);
        assert_eq!(pos, out.len());
    }

    #[test]
    fn zstd_partial_stream_fails_loudly() {
        let raw = b"partial-stream-failure-check";
        let mut out = Vec::new();
        circuit::embed::write_compressed_section(&mut out, raw);
        let truncated = &out[..out.len() - 3];
        let mut pos = 0usize;
        assert!(
            circuit::embed::read_compressed_section(truncated, &mut pos).is_err(),
            "truncated zstd section must fail"
        );
    }

    #[test]
    fn zstd_multi_section_stream_round_trips() {
        let mut out = Vec::new();
        for i in 0..8u8 {
            let raw = vec![i; 1 << 12];
            circuit::embed::write_compressed_section(&mut out, &raw);
        ...
        let mut pos = 0usize;
        for i in 0..8u8 {
            let expected = vec![i; 1 << 12];
            let decoded = circuit::embed::read_compressed_section(&out, &mut pos)
                .expect("multi-section zstd stream must decode");
