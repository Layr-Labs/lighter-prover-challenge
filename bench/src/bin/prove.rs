...
fn main() {
    ...
    let pre_handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            (pre_proof, pre_target, pre_data)
        })
        .expect("pre-execution startup thread must start");
    // Load only the four circuits not already owned by the pre-execution
    // thread. Once proving finishes, move that exact pre circuit into the full
    // set instead of deserializing and retaining a duplicate.
    let remaining_circuits = Circuits::load_remaining();
    let (pre_proof, pre_target, pre_data) = pre_handle
        .join()
        .unwrap_or_else(|panic| std::panic::resume_unwind(panic));
    let circuits = remaining_circuits.with_pre((pre_target, pre_data));
