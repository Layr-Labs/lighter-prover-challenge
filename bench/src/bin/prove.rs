...
// to the OS, so that the next allocation of a similar shape reuses already-
// resident, TLB-warm pages without a page fault.
//
// The ranked harness runs the five fixtures sequentially (one worker at a
// time), so the process's resident set is not a shared-resource concern:
// no concurrent worker competes for the same physical pages. The prover
// allocates and frees the same shapes of multi-hundred-megabyte witness,
// coefficient and digest buffers 50+ times across the chain spine. With
// decay periods set to -1 (never decay) the allocator retains freed extents
// indefinitely, so every reuse after the first is a hot-page hit instead of
// a kernel page-fault round trip. The trade-off is higher peak RSS (the
// high-water mark of the heap rather than the live set), but on the M4 Pro
// host (24 GB) the headroom is ample for a single sequential worker.
//
...
// correctly sized, correctly aligned storage, and no arithmetic, ordering
// or buffer content depends on the option.
//
