...
fn main() {
    ...
    // staging area for the prebuilt pipeline archive: it is the one path the
    // ranked Seatbelt profile lets this process write, and an archive hit lets
    // the prewarm skip the AIR->ISA lowering outright. Reading argv here (and
    // asserting its shape only after the prewarm is in flight) keeps the
    // lowering off the critical path even by the few microseconds arg parsing
    // costs.
    ...
    // PGO instrumentation writes its .profraw from an atexit handler, which
    // `_exit(2)` skips. Profile-collection runs set LIGHTER_PGO_EXIT=1 to take
    // libc `exit(3)` instead so the counters reach disk. The scored path never
    // sets it (the ranked environment is cleared), so it costs one getenv at
    // process end — after the proof bytes are already with the kernel.
    if std::env::var_os("LIGHTER_PGO_EXIT").is_some() {
        std::process::exit(0);
    }
    unsafe { _exit(0) }
}
