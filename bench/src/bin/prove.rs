...
fn main() {
    ...
    // Local-only startup probe: records the process entry moment. Reads no
    // prover state and does not run under the ranked worker's scoring window;
    // ranked harness nulls worker output, so this is a local/diagnostic signal.
    plonky2::hash::poseidon2::startup_mark("start");
    ...
    // Local-only startup probe report. Ranked harness nulls worker output, so
    // this is a diagnostic channel for local runs only; it reads no prover
    // state and does not run under the ranked worker's scoring window.
    plonky2::hash::poseidon2::startup_report();
    unsafe extern "C" {
        fn _exit(status: i32) -> !;
    }
    unsafe { _exit(0) }
...
// p90-fire-exp41bar-redraw-joelchristianai3-jpg-1786833405374
