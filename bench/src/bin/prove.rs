...
fn main() {
    ...
    // This one-shot diagnostic uses the exact bridge-approved proof-output path,
    // not a sibling or process-global temporary path. The bridge may leave the
    // output absent or install a writable placeholder; the archive stage creates
    // or truncates that exact entry. On Apple silicon the call never returns: it
    // stages and opens the exact target-26 archive there, probes the six required
    // pipelines, and exits in the result's wall-clock slot before any fixture
    // byte is read or proof work begins. The archive can remain at `output`
    // because this diagnostic never reaches proof serialization. Non-Metal
    // builds keep the no-op API so ordinary Linux validation can continue below.
    {
        #[cfg(feature = "diagnostic_profile")]
