...
fn main() {
    ...
    // Overlap the cold Metal shader compile (the sandbox denies the shader
    // cache, so every spawn pays it) with startup instead of the first tree
    // build; joined below so a slow compile degrades to the previous behavior.
