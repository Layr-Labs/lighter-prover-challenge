...
fn main() {
    ...
    // Startup overlap: load the pre-execution circuit and prove it on a
    // separate thread while the remaining four circuit loads (LDE FFTs and
    // GPU merkle builds) run on the main thread. The overlap path deliberately
    // does not enter the exclusive GPU phase, so the concurrent loads keep
    // their normal GPU routing and the single-queue machinery arbitrates.
