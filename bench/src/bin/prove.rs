...
fn main() {
    ...
    // Measurement only, and only when `LIGHTER_COMMIT_PROBE=1` is set, which the
    // scored path cannot do: the trusted verifier clears the worker's
    // environment. Printed after the proof has reached its descriptor so it
    // cannot perturb anything the harness times or reads.
