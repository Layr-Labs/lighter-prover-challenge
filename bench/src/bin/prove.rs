...
fn main() {
    ...
    // This one-shot diagnostic must use the bridge-approved proof directory,
    // not process-global temporary-directory state. On Apple silicon the call
    // never returns: it stages and opens the exact target-26 archive, probes the
    // six required pipelines, and exits in the result's wall-clock slot before
    // any fixture byte is read or any proof work begins. Non-Metal builds keep
    // the no-op API so ordinary Linux validation can continue below.
    {
        #[cfg(feature = "diagnostic_profile")]
