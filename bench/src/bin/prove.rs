...
fn main() {
    ...
    // The pre-execution proof (small circuit, mostly CPU) runs on its own
    // thread while the remaining circuit loads (LDE FFTs, commitments,
    // transpose, quotient cache) run on the main thread. The overlap path
    // deliberately does not enter the exclusive GPU phase, so the concurrent
    // loads keep their normal GPU routing and the single-queue Metal
    // machinery arbitrates. This hides the pre-execution proof behind the
    // loads, the same class of win as the frontier's readback overlap.
