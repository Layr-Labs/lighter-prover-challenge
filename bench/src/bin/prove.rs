...
fn main() {
    ...
    // Pipeline-archive staging withdrawn: the directory is deliberately not
    // registered, so no archive is staged or attached and pipeline creation
    // proceeds exactly as before the mechanism was added.
    let _ = std::path::Path::new(&output).parent();
    {
        #[cfg(feature = "diagnostic_profile")]
