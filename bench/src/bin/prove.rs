...
fn main() {
    ...
    // context publishes only after all six sequential strict archive probes
    // succeed; the five optional pipelines are ordinary asynchronous library
    // lowers and never receive or call the retained archive.
    ...
    // occurs after all six required probes complete, so no later read depends on
    ...
    // complete sequentially before context publication. Optional builders use
