...
fn main() {
    ...
    // Circuit deserialization and fixture parsing are independent startup work.
    // Load the embedded circuits on a scoped thread while the main thread reads
    // and parses the fixture, then join before proving begins.
    let (block, circuits) = std::thread::scope(|scope| {
        let circuits_handle = std::thread::Builder::new()
            .stack_size(PROVER_THREAD_STACK_BYTES)
            .spawn_scoped(scope, Circuits::load)
