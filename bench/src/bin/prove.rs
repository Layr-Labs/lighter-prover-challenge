...
///
/// The pre-execution witness does not read transactions. The unchanged full parse below remains
/// authoritative for every transaction and all input validation; this scanner only lets the
/// transaction-free envelope start the pre proof while that full parse continues.
