...
impl<T: Send + 'static> Deferred<T> {
    ...
    /// Whether construction has finished, without blocking on it. A finished
    /// but not-yet-joined thread counts as ready: its work (and any GPU tree
    /// builds it issued) is already done.
...
impl Circuits {
    ...
    /// Whether the transaction/chain path circuits have finished constructing,
    /// without blocking. Gates the pre-execution exclusive-GPU-phase hint,
    /// which assumes nothing else contends for the serialized GPU stream.
    pub(crate) fn paths_ready(&self) -> bool {
        self.paths.is_ready()
    }
