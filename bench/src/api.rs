...
impl Circuits {
    ...
    /// Builds the final block circuit. Nothing consumes its output until the
    /// final block proof, so the caller may overlap this with transaction and
    /// chain proving instead of paying it as a serial stage before proving.
