...
fixture="${root}/bench/bench_test.json"
trusted_dir="${root}/benchmark-tools/trusted"
verifier="${trusted_dir}/lighter-benchmark-verifier"
harness_manifest="${root}/benchmark-tools/harness/Cargo.toml"
harness_source="${root}/benchmark-tools/harness/src/main.rs"
publisher="${root}/benchmark-tools/build-trusted-verifier.sh"
mixed_revision="381fd529eb61dfff9ad245d94fce214a0a64d927"
public_fixture_sha256="6f1fbd2d5e64ed84f656b0c2dc299a8628801ac66488dfe021fdc4b2af53eb4b"
...
[[ -f "${harness_manifest}" && -f "${harness_source}" && -f "${publisher}" ]] \
  || fail "migrated verifier source contract is incomplete"
rg -Fq -- "rev = \"${mixed_revision}\"" "${harness_manifest}" \
  || fail "trusted harness manifest is not pinned to the mixed revision"
rg -Fq -- "const VERIFIER_SOURCE_REV: &str = \"${mixed_revision}\";" "${harness_source}" \
  || fail "trusted harness source revision metadata is stale"
rg -Fq -- "\"${public_fixture_sha256}\"" "${harness_source}" \
  || fail "trusted harness public fixture digest is stale"
[[ "$(shasum -a 256 "${fixture}" | awk '{print $1}')" == "${public_fixture_sha256}" ]] \
  || fail "migrated public fixture digest is wrong"
[[ "$(/usr/bin/lipo -archs "${verifier}")" == arm64 ]] \
  || fail "published trusted verifier is not a single-architecture arm64 binary"

# SHA256SUMS is the authenticity control; the ad-hoc codesign check that
# follows is tamper-evidence only. See benchmark-tools/trusted/README.md.
(
  cd "${trusted_dir}"
...
# The publication source pin is intentionally advanced only when the reviewed
# mixed verifier is rebuilt and signed. Until then, verify the legacy artifact's
# integrity above and skip behavior that it cannot execute against the new
# fixture. Once REVIEWED_COMMIT points at mixed source, this test automatically
# exercises timeout, malformed, oversized, and sandbox handling end to end.
reviewed_commit="$(awk -F= '$1 == "readonly REVIEWED_COMMIT" { print $2 }' "${publisher}")"
[[ "${reviewed_commit}" =~ ^[0-9a-f]{40}$ ]] \
  || fail "publisher REVIEWED_COMMIT is invalid"
reviewed_manifest="$(git -C "${root}" show \
  "${reviewed_commit}:benchmark-tools/harness/Cargo.toml")" \
  || fail "cannot inspect the reviewed verifier manifest"
if ! printf '%s\n' "${reviewed_manifest}" | rg -Fq -- "rev = \"${mixed_revision}\""; then
  echo "SKIP: published verifier predates the reviewed mixed-block source"
  exit 0
fi
...
verify_protected_state() {
  ...
  if ! codesign --verify --strict "${verifier}" >/dev/null 2>&1; then
    echo "FAIL: published verifier ad-hoc code-signature integrity check failed during cleanup" >&2
    failed=1
  fi
  return "${failed}"
}
