...
readonly WRAPPER=".yukon/hooks/yukon-trace.sh"
readonly PLUGIN=".opencode/plugin/yukon-trace.ts"
readonly OMP_EXTENSION=".omp/extensions/yukon-trace.ts"
readonly PI_EXTENSION=".pi/extensions/yukon-trace.ts"
...
fixture() {
  ...
  tar -C "${root}" -cf - .yukon .opencode .omp .pi .claude .cursor .codex opencode.json \
    | tar -C "${dir}" -xf -
  printf '%s' "${dir}"
}
...
dir="$(fixture)"
substitute "${dir}/.cursor/hooks.json" 's|"matcher": "yukon"|"matcher": ""|'
