...
readonly PLUGIN_SHA256=b79332b9cc0426a3201ff75063200294e693b2abad9845167c3f52d744fea2eb
...
readonly EXTENSION_SHA256=7cd674f953f29bddcb4583b840e25022fe0fb7fc5e44e7b0af3ed9a6dfe248ba
...
WRAPPER
  ...
  {
    printf '# Hilbert trace hooks are managed by `%s trace init`.\n\n' "${CLI}"
...
#    and add a second, unpinned hook file. The legacy .yukon/ tree is rejected outright — the CLI
#    rename is complete in this repository, so a legacy path reappearing is either a bad revert or
#    an attempt to run a wrapper this check does not cover. .omp/ and .pi/ are managed here rather
#    than rejected: `hilbert trace init` writes them for the OMP and Pi adapters, they are checked in
#    to sibling challenge repositories, and because the CLI's routine refresh only touches adapters
#    that already have a config present, a repository that omits them gets no capture at all from
#    those two agents. They are pinned by digest below, so they are validated rather than tolerated.
for path in \
  "${WRAPPER}" \
  "${PLUGIN}" \
  "${OMP_EXTENSION}" \
...
[[ ! -e .yukon ]] || fail ".yukon/ exists but is not covered by this check"
...
#    Note that these files legitimately contain the string YUKON_ in the session-id environment
#    variable names the CLI still reads, so this must stay a digest comparison: a blanket rule
#    against the legacy name in file contents would reject the CLI's own current output.
check_cli_source() {
