...
# The SHA-256 pin is the only authenticity control on the trusted verifier: it
# binds this binary to the reviewed commit that published it. The codesign check
# below is ad-hoc (no signing identity, no Team ID), so it proves the binary has
# not been altered since it was signed and proves nothing about who signed it.
# See benchmark-tools/trusted/README.md.
[[ -x "${trusted}" ]] || die "trusted verifier is missing"
(
  cd "${root}/benchmark-tools/trusted"
  shasum -a 256 -c SHA256SUMS
) || die "trusted verifier does not match the pinned SHA-256 in SHA256SUMS"
codesign --verify --strict "${trusted}" \
  || die "trusted verifier failed its ad-hoc code-signature integrity check (tamper-evidence only; it attests no signer)"
