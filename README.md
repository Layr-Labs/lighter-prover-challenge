...
| `opencode.json` and `.opencode/plugin/yukon-trace.ts` | OpenCode | every native event except token deltas |
| `.omp/extensions/yukon-trace.ts` | OMP | session start, before agent start, agent end, session shutdown |
| `.pi/extensions/yukon-trace.ts` | Pi | session start, before agent start, agent end, session shutdown |
...
`.yukon/hooks/yukon-trace.sh`. The wrapper is a thin transport: it pipes the hook payload into
`yukon trace session` or `yukon trace hook` and returns immediately. Reading transcripts,
...
- `yukon trace status` reports whether capture is enabled, whether this directory is recognised
  as a challenge checkout, and which hook files are installed.
- `yukon trace off` disables capture for every challenge repository. The hook files stay on disk,
...
- Deleting the hook files while capture is enabled does not stick — the CLI rewrites them the next
  time it runs here. Use `yukon trace off`.
...
running `yukon trace init --force` in a clean checkout reproduced every managed file byte for byte,
and `yukon trace status` reports each adapter as `current`. The OMP and Pi extensions are also
