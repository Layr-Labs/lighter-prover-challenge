...
impl<F: Field> PolynomialValues<F> {
    ...
    /// Coset IFFT when `scales[i] = n^{-1} * inverse_shift^i` is already fused.
    pub fn coset_ifft_with_prescaled_powers(self, scales: &[F]) -> PolynomialCoeffs<F> {
        ifft_with_prescaled_postscale(self, None, None, scales)
    }
...
mod tests {
    ...
    fn test_coset_ifft_with_powers_matches_separate_postscale() {
        ...
        for k in [1usize, 3, 8] {
            ...
            let prescaled = inverse_powers.iter().map(|&p| n_inv * p).collect::<Vec<_>>();
            let fused = evals.coset_ifft_with_prescaled_powers(&prescaled);
            assert_eq!(
                fused.coeffs.iter().map(|value| value.0).collect::<Vec<_>>(),
                expected
                    .coeffs
                    .iter()
                    .map(|value| value.0)
