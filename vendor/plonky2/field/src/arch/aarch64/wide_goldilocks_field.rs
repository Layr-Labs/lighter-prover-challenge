...
unsafe impl PackedField for WideGoldilocksField {
    ...
    #[inline]
    fn multiply_accumulate_packed(&self, x: Self, y: Self) -> Self {
...
/// Multiply four independent lane pairs, add four accumulators, and reduce modulo
/// `2^64 - 2^32 + 1`.
#[inline(always)]
fn mul_add_reduce_quad(accumulator: [u64; 4], lhs: [u64; 4], rhs: [u64; 4]) -> [u64; 4] {
    ...
    unsafe {
        asm!(
            ...
            accumulator2 = in(reg) accumulator2,
            accumulator3 = in(reg) accumulator3,
            epsilon = in(reg) GoldilocksField::ORDER.wrapping_neg(),
            options(pure, nomem, nostack),
        );
    }
...
mod tests {
    ...
    #[test]
    fn fused_multiply_accumulate_matches_scalar_boundaries() {
        let values = boundary_values();
        for i in 0..values.len() {
            for j in 0..values.len() {
                for k in 0..values.len() {
                    ...
                    assert_eq!(
                        packed_accumulator
                            .multiply_accumulate_packed(packed_a, packed_b)
                            .as_slice(),
