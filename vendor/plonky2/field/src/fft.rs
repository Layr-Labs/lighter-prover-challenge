...
#[cfg(target_arch = "aarch64")]
use crate::goldilocks_field::{mul_16th_root_powers, GoldilocksField};
...
fn fft_classic_simd_single_layer<P: PackedField>(
    ...
    #[cfg(target_arch = "aarch64")]
    if core::any::TypeId::of::<P>() == core::any::TypeId::of::<WideGoldilocksField>() {
        ...
        // SAFETY: TypeId check above proves P = WideGoldilocksField,
        // so P::Scalar = GoldilocksField. The reference is the same address,
        // only the type spelling differs — same pattern as the slice cast.
        let rt = unsafe {
            &*(root_table as *const FftRootTable<P::Scalar>
                as *const FftRootTable<GoldilocksField>)
        };
        unsafe {
            crate::neon_fft::fft_classic_simd_single_layer_neon(wide_values, lg_half_m, rt);
        }
        return;
    }
...
fn fft_classic_simd_fused_two_layers<P: PackedField>(
    ...
    if core::any::TypeId::of::<P>() == core::any::TypeId::of::<WideGoldilocksField>() {
        ...
        // SAFETY: TypeId check above proves P = WideGoldilocksField,
        // so P::Scalar = GoldilocksField.
        let rt = unsafe {
            &*(root_table as *const FftRootTable<P::Scalar>
                as *const FftRootTable<GoldilocksField>)
        };
        unsafe {
            crate::neon_fft::fft_classic_simd_fused_two_layers_neon(wide_values, lg_half_m, rt);
        }
        return;
    }
