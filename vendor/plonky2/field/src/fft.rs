...
/// Bounded fused prefix for an ordinary base-field transform.
///
/// Canonicalize each butterfly's right operand once, then share that range
/// bound across the sum and difference. This removes both second-correction
/// paths while keeping the two layers and their shuffles in vector registers.
#[cfg(target_arch = "aarch64")]
#[inline(never)]
fn fft_classic_simd_first_two_layers_neon(
    ...
    let eps = unsafe { vdupq_n_u64((1u64 << 32) - 1) };
    let order = unsafe { vdupq_n_u64(0xffff_ffff_0000_0001) };

    for block in values.chunks_exact_mut(8) {
        unsafe {
            let a_lo = vld1q_u64(block.as_ptr().cast::<u64>());
            let a_hi = vld1q_u64(block.as_ptr().add(2).cast::<u64>());
            let b_lo = vld1q_u64(block.as_ptr().add(4).cast::<u64>());
            let b_hi = vld1q_u64(block.as_ptr().add(6).cast::<u64>());

            // Layer zero: deinterleave adjacent butterflies. Canonical raw
            // right lanes use the short correction pair; rare aliases retain
            // the historical raw-word operations.
            let u0_lo = vuzp1q_u64(a_lo, b_lo);
            let v0_lo = vuzp2q_u64(a_lo, b_lo);
            let u0_hi = vuzp1q_u64(a_hi, b_hi);
            let v0_hi = vuzp2q_u64(a_hi, b_hi);
            let (add0_lo, sub0_lo) =
                gl_addsub_neon_canonical_or_raw_rhs(u0_lo, v0_lo, eps, order);
            let (add0_hi, sub0_hi) =
                gl_addsub_neon_canonical_or_raw_rhs(u0_hi, v0_hi, eps, order);

            // The zip is the two packed interleaves between layers. Layer one
            // has unity in lane zero and root4 in lane one of each pair.
            let u1_lo = vzip1q_u64(add0_lo, sub0_lo);
            let u1_hi = vzip2q_u64(add0_lo, sub0_lo);
            let v1_lo = vzip1q_u64(add0_hi, sub0_hi);
            let v1_hi = vzip2q_u64(add0_hi, sub0_hi);
            let products = roots
                * NeonGoldilocksField([
                    crate::goldilocks_field::GoldilocksField(vgetq_lane_u64(v1_lo, 1)),
                    crate::goldilocks_field::GoldilocksField(vgetq_lane_u64(v1_hi, 1)),
                ]);
            let t1_lo = vsetq_lane_u64(products.0[0].0, v1_lo, 1);
            let t1_hi = vsetq_lane_u64(products.0[1].0, v1_hi, 1);
            let (add1_lo, sub1_lo) =
                gl_addsub_neon_canonical_or_raw_rhs(u1_lo, t1_lo, eps, order);
            let (add1_hi, sub1_hi) =
                gl_addsub_neon_canonical_or_raw_rhs(u1_hi, t1_hi, eps, order);

            vst1q_u64(block.as_mut_ptr().cast::<u64>(), add1_lo);
            vst1q_u64(block.as_mut_ptr().add(2).cast::<u64>(), sub1_lo);
            vst1q_u64(block.as_mut_ptr().add(4).cast::<u64>(), add1_hi);
            vst1q_u64(block.as_mut_ptr().add(6).cast::<u64>(), sub1_hi);
        }
    }
}
...
/// Use the shortened one-correction butterfly only when both raw right lanes
/// are already canonical. A noncanonical lane is rare but raw-representation
/// sensitive, so it takes the historical two-correction operations unchanged.
#[cfg(target_arch = "aarch64")]
#[inline(always)]
unsafe fn gl_addsub_neon_canonical_or_raw_rhs(
    x: core::arch::aarch64::uint64x2_t,
    y: core::arch::aarch64::uint64x2_t,
    eps: core::arch::aarch64::uint64x2_t,
    order: core::arch::aarch64::uint64x2_t,
    ...
    use core::arch::aarch64::*;
    let noncanonical = vcgeq_u64(y, order);
    if (vgetq_lane_u64(noncanonical, 0) | vgetq_lane_u64(noncanonical, 1)) == 0 {
        (
            gl_add_neon_canonical_rhs(x, y, eps),
            gl_sub_neon_canonical_rhs(x, y, eps),
        )
    } else {
        (gl_add_neon(x, y, eps), gl_sub_neon(x, y, eps))
    }
}
...
fn fft_classic_simd_single_layer_neon_ext(
    ...
    unsafe {
        let eps = vdupq_n_u64(EPSILON);
        let order = vdupq_n_u64(
            <GoldilocksField as crate::types::Field64>::ORDER,
        );
        let mut k = 0;
        while k + m <= values.len() {
            let mut j = 0;
            while j + 1 <= half {
                ...
                let (sum, diff) =
                    gl_addsub_neon_canonical_or_raw_rhs(uv, tv, eps, order);
...
#[inline(always)]
fn fft_zero_padded_rate_8_first_layer_block_control(
...
fn fft_zero_padded_rate_8_first_layer_block(
    ...
    for pair in (0..nonzero_len / 2).rev() {
        ...
        let v = packed_values[(source + 1) / 4].as_slice()[(source + 1) % 4];
        let products = mul_16th_root_powers(v);
        // One guard per source pair keeps the common path short without four
        // vector-to-scalar lane checks. Rare aliases retain the historical raw
        // add/sub representation for every output derived from this pair.
        let products_are_canonical = products
            .iter()
            .all(|product| product.0 < 0xffff_ffff_0000_0001);
        let pair_destination = destination + pair * 4;
        unsafe {
            let uv = vdupq_n_u64(u.0);
            let out = packed_values
                .as_mut_ptr()
                .add(pair_destination)
                .cast::<u64>();
            if products_are_canonical {
                for j in 0..4 {
                    let product = vld1q_u64(products.as_ptr().add(2 * j).cast::<u64>());
                    vst1q_u64(out.add(2 * j), gl_add_neon_canonical_rhs(uv, product, eps));
                    vst1q_u64(
                        out.add(8 + 2 * j),
                        gl_sub_neon_canonical_rhs(uv, product, eps),
                    );
                }
            } else {
                for j in 0..4 {
                    let product = vld1q_u64(products.as_ptr().add(2 * j).cast::<u64>());
                    vst1q_u64(out.add(2 * j), gl_add_neon(uv, product, eps));
                    vst1q_u64(out.add(8 + 2 * j), gl_sub_neon(uv, product, eps));
                }
            }
        }
    }
...
mod tests {
    ...
    fn fused_full_array_layer_pairs_match_reference() {
        for (lg_n, r) in [(19usize, 0usize), (20, 3), (19, 3), (18, 0)] {
            ...
            let nonzero_len = n >> r;
            let roots = fft_root_table(n);
            let mut padded = deterministic_values(nonzero_len)
                .into_iter()
            ...
            for i in 0..64 {
                padded[i * nonzero_len / 64] = GoldilocksField(aliases[i & 3]);
            }
    ...
    fn fused_first_two_layers_match_general_raw_words() {
        ...
        for lg_n in [3usize, 8, 14, 16, 18] {
            ...
            let mut input: Vec<_> = (0..1usize << lg_n)
                .map(|i| {
                    GoldilocksField(
                        0xffff_ffff_0000_0000u64.wrapping_add(
                            0x9e37_79b9_7f4a_7c15u64.wrapping_mul(i as u64 + 1),
            ...
            input[..8].copy_from_slice(&[
                ...
                GoldilocksField(0xffff_ffff_0000_0001),
                GoldilocksField(0),
                GoldilocksField(0xffff_ffff_0000_0002),
            ...
            assert_eq!(expected, actual, "raw mismatch at lg_n={lg_n}");
            assert_eq!(
                expected
                    .iter()
                ...
                actual
                    ...
                    .map(|x| x.to_canonical_u64())
                    .collect::<Vec<_>>(),
                "canonical mismatch at lg_n={lg_n}",
            );
        }
    }
    ...
    #[test]
    #[cfg(target_arch = "aarch64")]
    fn rate8_expansion_matches_control_raw_words() {
        use super::{
            fft_zero_padded_rate_8_first_layer_block,
            fft_zero_padded_rate_8_first_layer_block_control,
        };
        use crate::arch::aarch64::wide_goldilocks_field::WideGoldilocksField;
        use crate::types::PrimeField64;

        for lg_n in [3usize, 8, 14, 16] {
            let nonzero_len = 1usize << lg_n;
            let mut input = vec![GoldilocksField::ZERO; 8 * nonzero_len];
            for (i, value) in input[..nonzero_len].iter_mut().enumerate() {
                let x = 0x9e37_79b9_7f4a_7c15u64.wrapping_mul(i as u64 + 1);
                *value = GoldilocksField(if i & 7 == 0 { u64::MAX - (x >> 60) } else { x });
            }
            input[..8].copy_from_slice(&[
                GoldilocksField(0),
                GoldilocksField(0xffff_ffff_0000_0001),
            ...
            fft_zero_padded_rate_8_first_layer_block_control(
                WideGoldilocksField::pack_slice_mut(&mut control),
                0,
                nonzero_len,
                0,
            );
            fft_zero_padded_rate_8_first_layer_block(
                WideGoldilocksField::pack_slice_mut(&mut candidate),
                0,
                nonzero_len,
                0,
            );
            assert_eq!(control, candidate, "raw mismatch at lg_n={lg_n}");
            assert_eq!(
                control.iter().map(|x| x.to_canonical_u64()).collect::<Vec<_>>(),
                candidate.iter().map(|x| x.to_canonical_u64()).collect::<Vec<_>>(),
                "canonical mismatch at lg_n={lg_n}",
            );
        }
    }
    ...
    fn zero_padded_extension_fft_matches_reference() {
        ...
        for lg_n in [9usize, 11, 12, 13, 15, 17] {
            ...
            let mut actual = (0..nonzero_len)
                ...
                .collect::<Vec<_>>();
            actual[..4].copy_from_slice(&[
                QuadraticExtension([F::ZERO, F::ZERO]),
                QuadraticExtension([GoldilocksField(0xffff_ffff_0000_0001), F::ZERO]),
                QuadraticExtension([
                    GoldilocksField(0xffff_ffff_0000_0001),
                    GoldilocksField(0xffff_ffff_0000_0002),
                ]),
