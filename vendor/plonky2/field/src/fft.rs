...
fn fft_classic_simd_single_layer_neon(
    ...
    unsafe {
        ...
        while k + m <= values.len() {
            ...
            // issue back-to-back; each butterfly uses the same per-lane
            // NeonGoldilocksField mul + gl_add_neon/gl_sub_neon as the 2-wide
            // loop below, in the same order. Raw limbs are therefore identical
            // to running the 2-wide kernel twice. Production half is always a
            // power of two >= 2, so the 4-wide body covers full blocks and the
            // 2-wide loop only drains an odd pair when half is 2.
            while j + 4 <= half {
