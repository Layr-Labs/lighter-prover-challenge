...
pub fn lde_coset_fft<F: Field>(
    ...
    debug_assert!(coset_powers.len() >= n);

    // Only the real coefficients need the coset scaling; the padding is zero. The caller supplies
    // `coset_powers` (`shift^0..shift^(n-1)`) so the sequential power chain is generated once per
    // batch rather than once per polynomial. Bit-reverse this small, cache-resident array rather
    // than the extended one, so the emit loop below reads and writes sequentially.
    let mut scaled: Vec<F> = coset_powers.iter().zip(coeffs).map(|(&r, &c)| r * c).collect();
