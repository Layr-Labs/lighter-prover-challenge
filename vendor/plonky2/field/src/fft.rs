...
fn prepare_zero_padded_fft<F: Field>(
    ...
    if r >= lg_packed_width && r < lg_n {
        // Keep values plus the largest local twiddle row near 48 KiB. Goldilocks values use
        // 2^12-element blocks; wider extension values use 2^11.
...
mod tests {
    ...
    #[test]
    #[ignore = "manual release-mode FFT kernel microbenchmark"]
    fn fft_kernel_microbench() {
        use std::hint::black_box;
        use std::time::Instant;

        type F = GoldilocksField;

        for lg_n in [16, 18, 20] {
            let n = 1 << lg_n;
            let roots = fft_root_table(n);

            for r in [0, 3] {
                let nonzero_len = n >> r;
                let input = (0..nonzero_len)
                    .map(deterministic_value)
                    .chain(core::iter::repeat_n(F::ZERO, n - nonzero_len))
                    .collect::<Vec<_>>();

                let mut warmup = input.clone();
                fft_classic(black_box(&mut warmup), r, black_box(&roots));
                let mut reference_warmup = input.clone();
                fft_classic_reference(black_box(&mut reference_warmup), r, black_box(&roots));

                let mut optimized_samples = Vec::with_capacity(15);
                let mut reference_samples = Vec::with_capacity(15);
                let mut ratios = Vec::with_capacity(15);
                for sample in 0..15 {
                    let measure_optimized = || {
                        let mut values = input.clone();
                        let start = Instant::now();
                        fft_classic(black_box(&mut values), r, black_box(&roots));
                        black_box(values);
                        start.elapsed()
                    };
                    let measure_reference = || {
                        let mut values = input.clone();
                        let start = Instant::now();
                        fft_classic_reference(black_box(&mut values), r, black_box(&roots));
                        black_box(values);
                        start.elapsed()
                    };

                    let (reference, optimized) = if sample % 2 == 0 {
                        (measure_reference(), measure_optimized())
                    } else {
                        let optimized = measure_optimized();
                        (measure_reference(), optimized)
                    };
                    reference_samples.push(reference);
                    optimized_samples.push(optimized);
                    ratios.push(optimized.as_secs_f64() / reference.as_secs_f64());
                }
                optimized_samples.sort_unstable();
                reference_samples.sort_unstable();
                ratios.sort_by(f64::total_cmp);
                let optimized = optimized_samples[optimized_samples.len() / 2];
                let reference = reference_samples[reference_samples.len() / 2];
                let ratio = ratios[ratios.len() / 2];
                println!(
                    "fft-kernel lg_n={lg_n} r={r}: reference {reference:?}, \
                     optimized {optimized:?}, change {:+.2}%",
                    (ratio - 1.0) * 100.0,
                );
            }
        }
