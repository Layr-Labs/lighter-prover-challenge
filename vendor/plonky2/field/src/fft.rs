...
fn fft_classic_simd_first_two_layers_neon(
    ...
    for block in values.chunks_exact_mut(8) {
        unsafe {
            ...
            // Layer zero: deinterleave adjacent butterflies, canonicalize the
            // raw right lanes once, and share that bound across sum/difference.
            let u0_lo = vuzp1q_u64(a_lo, b_lo);
            let v0_lo = gl_canonicalize_neon(vuzp2q_u64(a_lo, b_lo), order);
            let u0_hi = vuzp1q_u64(a_hi, b_hi);
            let v0_hi = gl_canonicalize_neon(vuzp2q_u64(a_hi, b_hi), order);
            let add0_lo = gl_add_neon_canonical_rhs(u0_lo, v0_lo, eps);
            let sub0_lo = gl_sub_neon_canonical_rhs(u0_lo, v0_lo, eps);
            let add0_hi = gl_add_neon_canonical_rhs(u0_hi, v0_hi, eps);
            let sub0_hi = gl_sub_neon_canonical_rhs(u0_hi, v0_hi, eps);
            ...
            let t1_lo = gl_canonicalize_neon(t1_lo, order);
            let t1_hi = gl_canonicalize_neon(t1_hi, order);
            let add1_lo = gl_add_neon_canonical_rhs(u1_lo, t1_lo, eps);
            let sub1_lo = gl_sub_neon_canonical_rhs(u1_lo, t1_lo, eps);
            let add1_hi = gl_add_neon_canonical_rhs(u1_hi, t1_hi, eps);
            let sub1_hi = gl_sub_neon_canonical_rhs(u1_hi, t1_hi, eps);
    ...
    unsafe {
        ...
        while k + m <= values.len() {
            ...
            while j + 1 <= half {
                ...
                let tv = gl_canonicalize_neon(
                    vcombine_u64(vcreate_u64(c0), vcreate_u64(c1)),
                    order,
                );

                let u = *values.get_unchecked(k + j);
                let uv = vcombine_u64(vcreate_u64(u.0[0].0), vcreate_u64(u.0[1].0));
                let sum = gl_add_neon_canonical_rhs(uv, tv, eps);
                let diff = gl_sub_neon_canonical_rhs(uv, tv, eps);
    ...
    for pair in (0..nonzero_len / 2).rev() {
        ...
        unsafe {
            ...
            let out = packed_values
                ...
                .cast::<u64>();
            for j in 0..4 {
                let product = gl_canonicalize_neon(
                    vld1q_u64(products.as_ptr().add(2 * j).cast::<u64>()),
                    order,
                );
...
mod tests {
    ...
    #[test]
    #[cfg(target_arch = "aarch64")]
    fn fused_first_two_layers_match_general_canonical_words() {
    ...
    #[test]
    #[cfg(target_arch = "aarch64")]
    fn rate8_expansion_canonical_rhs_model_matches_canonical_words() {
        use super::{
            fft_zero_padded_rate_8_first_layer_block,
