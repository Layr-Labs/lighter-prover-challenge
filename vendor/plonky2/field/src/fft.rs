...
/// 4-wide variant of the single-layer ext kernel: each iteration carries four
/// independent extension-element butterflies (four products in flight instead
/// of one), the same issue-schedule widening the base-field
/// `fft_classic_simd_single_layer_neon_w4` applies. Same values, same raw
/// words, different instruction schedule. Falls back to the 2-wide ext kernel
/// when a row is too short (`lg_half_m < 2`).
#[cfg(target_arch = "aarch64")]
#[inline(never)]
fn fft_classic_simd_single_layer_neon_ext_w4(
    values: &mut [crate::extension::quadratic::QuadraticExtension<
        crate::goldilocks_field::GoldilocksField,
    >],
    lg_half_m: usize,
    ...
    const W: u64 = 7;
    if lg_half_m < 2 {
        return fft_classic_simd_single_layer_neon_ext(values, lg_half_m, omega_row);
    }
    let half = 1usize << lg_half_m;
    let m = half << 1;
    debug_assert!(omega_row.len() >= half);
    ...
    unsafe {
        ...
        while k + m <= values.len() {
            let mut j = 0;
            while j + 4 <= half {
                let mut tv = [vdupq_n_u64(0); 4];
                for i in 0..4 {
                    let v = *values.get_unchecked(k + half + j + i);
                    let w = *omega_row.get_unchecked(j + i);
                    let (c0, c1) = if base_subfield {
                        let p = NeonGoldilocksField([w.0[0], w.0[0]])
                            * NeonGoldilocksField([v.0[0], v.0[1]]);
                        (p.0[0].0, p.0[1].0)
                    ...
                    tv[i] = vcombine_u64(vcreate_u64(c0), vcreate_u64(c1));
                }
                for i in 0..4 {
                    let u = *values.get_unchecked(k + j + i);
                    let uv = vcombine_u64(vcreate_u64(u.0[0].0), vcreate_u64(u.0[1].0));
                    let sum = gl_add_neon(uv, tv[i], eps);
                    let diff = gl_sub_neon(uv, tv[i], eps);
                    *values.get_unchecked_mut(k + j + i) = QuadraticExtension([
                        GoldilocksField(vgetq_lane_u64(sum, 0)),
                        GoldilocksField(vgetq_lane_u64(sum, 1)),
                    ]);
                    *values.get_unchecked_mut(k + half + j + i) = QuadraticExtension([
                        GoldilocksField(vgetq_lane_u64(diff, 0)),
                        GoldilocksField(vgetq_lane_u64(diff, 1)),
                    ]);
                }
...
/// 4-wide variant of the fused pair-column ext kernel: each iteration carries
/// four independent 2-lane groups (four extension elements, one 128-bit vector
/// per quarter position), so sixteen scalar `mul_reduce_pair` blocks (four
/// paired products per stage per group) are in flight between the dependent
/// stage-1 -> stage-2 rounds instead of four — the same issue-schedule trick
/// `fft_classic_simd_two_layers_neon_w4` applies to the base-field kernel.
/// Same values, same raw words, different instruction schedule. Falls back to
/// the 2-wide ext kernel when a row is too short (`lg_half_m < 2`).
#[cfg(target_arch = "aarch64")]
#[inline(never)]
fn fft_classic_simd_two_layers_neon_ext_w4(
    values: &mut [crate::extension::quadratic::QuadraticExtension<
        crate::goldilocks_field::GoldilocksField,
    >],
    lg_half_m: usize,
    ...
    const W: u64 = 7;
    if lg_half_m < 2 {
        return fft_classic_simd_two_layers_neon_ext(values, lg_half_m, w1_row, w2_row);
    }
    let q = 1usize << lg_half_m;
    let w1_row = &w1_row[..q];
    let (w2_lo, w2_hi) = w2_row[..2 * q].split_at(q);
    ...
    for block in values.chunks_exact_mut(4 * q) {
        ...
        for (((((a4, b4), c4), d4), (w14, w2a4)), w2b4) in quarter_a
            ...
            .zip(w2_hi.chunks_exact(4))
        {
            // Stage-1 products for both butterflies, four elements wide.
            let t1 = [
                product(&w14[0], (b4[0].0[0].0, b4[0].0[1].0)),
                product(&w14[1], (b4[1].0[0].0, b4[1].0[1].0)),
                product(&w14[2], (b4[2].0[0].0, b4[2].0[1].0)),
                product(&w14[3], (b4[3].0[0].0, b4[3].0[1].0)),
            ];
            let t2 = [
                product(&w14[0], (d4[0].0[0].0, d4[0].0[1].0)),
                product(&w14[1], (d4[1].0[0].0, d4[1].0[1].0)),
                product(&w14[2], (d4[2].0[0].0, d4[2].0[1].0)),
                product(&w14[3], (d4[3].0[0].0, d4[3].0[1].0)),
            ];
            // C/D stage-1 butterflies in scalar registers: these values are the
            // stage-2 multiplier inputs, so keeping them out of the vector file
            // avoids NEON->GPR crossings on the critical path.
            let cd0 = [
                [c4[0].0[0] + GoldilocksField(t2[0].0), c4[0].0[1] + GoldilocksField(t2[0].1)],
                [c4[1].0[0] + GoldilocksField(t2[1].0), c4[1].0[1] + GoldilocksField(t2[1].1)],
                [c4[2].0[0] + GoldilocksField(t2[2].0), c4[2].0[1] + GoldilocksField(t2[2].1)],
                [c4[3].0[0] + GoldilocksField(t2[3].0), c4[3].0[1] + GoldilocksField(t2[3].1)],
            ];
            let cd1 = [
                [c4[0].0[0] - GoldilocksField(t2[0].0), c4[0].0[1] - GoldilocksField(t2[0].1)],
                [c4[1].0[0] - GoldilocksField(t2[1].0), c4[1].0[1] - GoldilocksField(t2[1].1)],
            ...
            // Stage-2 products: four more independent pair-muls.
            let t3 = [
                product(&w2a4[0], (cd0[0][0].0, cd0[0][1].0)),
                product(&w2a4[1], (cd0[1][0].0, cd0[1][1].0)),
                product(&w2a4[2], (cd0[2][0].0, cd0[2][1].0)),
                product(&w2a4[3], (cd0[3][0].0, cd0[3][1].0)),
            ];
            let t4 = [
                product(&w2b4[0], (cd1[0][0].0, cd1[0][1].0)),
                product(&w2b4[1], (cd1[1][0].0, cd1[1][1].0)),
                product(&w2b4[2], (cd1[2][0].0, cd1[2][1].0)),
                product(&w2b4[3], (cd1[3][0].0, cd1[3][1].0)),
            ];
            // SAFETY: identical invariants to the 2-wide ext kernel; chunks hold
            // exactly 4 elements and every access stays inside its chunk. The
            // vector intrinsics are the same two-lane adds/subs the single-layer
            // ext kernel uses, applied per element (two words) per position.
            unsafe {
                let t1v = [
                    vcombine_u64(vcreate_u64(t1[0].0), vcreate_u64(t1[0].1)),
                    vcombine_u64(vcreate_u64(t1[1].0), vcreate_u64(t1[1].1)),
                    vcombine_u64(vcreate_u64(t1[2].0), vcreate_u64(t1[2].1)),
                    vcombine_u64(vcreate_u64(t1[3].0), vcreate_u64(t1[3].1)),
                ];
                let t3v = [
                    vcombine_u64(vcreate_u64(t3[0].0), vcreate_u64(t3[0].1)),
                    vcombine_u64(vcreate_u64(t3[1].0), vcreate_u64(t3[1].1)),
                    vcombine_u64(vcreate_u64(t3[2].0), vcreate_u64(t3[2].1)),
                    vcombine_u64(vcreate_u64(t3[3].0), vcreate_u64(t3[3].1)),
                ];
                let t4v = [
                    vcombine_u64(vcreate_u64(t4[0].0), vcreate_u64(t4[0].1)),
                    vcombine_u64(vcreate_u64(t4[1].0), vcreate_u64(t4[1].1)),
                    vcombine_u64(vcreate_u64(t4[2].0), vcreate_u64(t4[2].1)),
                    vcombine_u64(vcreate_u64(t4[3].0), vcreate_u64(t4[3].1)),
                ];
                for i in 0..4 {
                    let av = vld1q_u64(a4[i].0.as_ptr().cast::<u64>());
                    let ab0 = gl_add_neon(av, t1v[i], eps);
                    let ab1 = gl_sub_neon(av, t1v[i], eps);
                    vst1q_u64(a4[i].0.as_mut_ptr().cast::<u64>(), gl_add_neon(ab0, t3v[i], eps));
                    vst1q_u64(c4[i].0.as_mut_ptr().cast::<u64>(), gl_sub_neon(ab0, t3v[i], eps));
                    vst1q_u64(b4[i].0.as_mut_ptr().cast::<u64>(), gl_add_neon(ab1, t4v[i], eps));
                    vst1q_u64(d4[i].0.as_mut_ptr().cast::<u64>(), gl_sub_neon(ab1, t4v[i], eps));
                }
            }
        }
    }
    ...
    M: FftTwiddleMul<P>,
{
            ...
            >()
    {
        ...
        fft_classic_simd_single_layer_neon_ext_w4(ext_values, lg_half_m, omega_row);
        return;
    }
            ...
            >()
    {
        let mut lg_half_m = start;
        {
            ...
            while lg_half_m + 2 <= end {
                ...
                fft_classic_simd_two_layers_neon_ext_w4(ext_values, lg_half_m, w1_row, w2_row);
                lg_half_m += 2;
            }
        }
        if lg_half_m < end {
...
mod tests {
    ...
    fn neon_two_layers_ext_matches_generic_raw_words() {
        use super::{fft_classic_simd_two_layers_neon_ext, fft_classic_simd_two_layers_neon_ext_w4};
        type F = QuadraticExtension<GoldilocksField>;
        for lg_n in [4usize, 5, 8, 11, 13, 16] {
            let n = 1 << lg_n;
            let roots = fft_root_table::<F>(n);
            for start in 2..lg_n - 1 {
                let mut expected = ext_deterministic_values(n);
                let mut actual2 = expected.clone();
                let mut actual4 = expected.clone();
                generic_ext_single_layer(&mut expected, start, &roots);
                generic_ext_single_layer(&mut expected, start + 1, &roots);
                ...
                fft_classic_simd_two_layers_neon_ext_w4(
                    &mut actual4,
                    start,
                    &roots[start],
                    &roots[start + 1],
                );
                for (i, ((a2, a4), e)) in actual2.iter().zip(actual4.iter()).zip(expected.iter()).enumerate() {
                    assert_eq!(
                        [a2.0[0].0, a2.0[1].0],
                        [e.0[0].0, e.0[1].0],
                        "NEON ext 2-wide fusion mismatch at 2^{lg_n} start {start} index {i}"
                    );
                    assert_eq!(
                        [a4.0[0].0, a4.0[1].0],
                        [e.0[0].0, e.0[1].0],
                        "NEON ext w4 fusion mismatch at 2^{lg_n} start {start} index {i}"
                    );
                }
            }
        }
