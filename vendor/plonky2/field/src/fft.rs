...
pub(crate) fn ifft_with_options_and_postscale<F: Field>(
    ...
    match postscale {
        None => {
            // Same reverse + `1/n` as `ifft_borrowed`. The packed helper is
            // raw-word-identical to this scalar pair loop (see
            // `packed_ifft_ninv_postscale_matches_scalar_pairs`). Production
            // non-routed wire columns go through `.ifft()` → here.
            packed_ifft_ninv_postscale(&mut buffer, n_inv);
        }
        Some(scales) => {
...
/// Same mirrored WIDTH reverse-mul as [`packed_ifft_prescaled_postscale`],
/// but every scale is the IFFT's `1/n`. Used by `ifft_borrowed` so the
/// `from_values` / routed-wire path does not allocate a constant scale
/// table. Bit-identical to the scalar pair loop.
pub(crate) fn packed_ifft_ninv_postscale<F: Field>(buffer: &mut [F], n_inv: F) {
    let n = buffer.len();
    if n == 0 {
        return;
    }
    ...
    if width >= 2 {
        while i + width <= n / 2 {
            ...
            let buf_lo = *<F as Packable>::Packing::from_slice(&buffer[i..i + width]);
            let buf_hi = *<F as Packable>::Packing::from_slice(&buffer[hi_start..hi_start + width]);
            let dest_lo = reverse_packed(buf_hi) * n_inv;
            let dest_hi = reverse_packed(buf_lo) * n_inv;
            buffer[i..i + width].copy_from_slice(dest_lo.as_slice());
            buffer[hi_start..hi_start + width].copy_from_slice(dest_hi.as_slice());
            i += width;
        }
...
mod tests {
    ...
    /// Constant-`n_inv` reverse-mul must match the scalar pair loop that
    /// `ifft_borrowed` used, including ragged tails. Also re-checks the
    /// existing `ifft_borrowed` vs `ifft` contract on Goldilocks.
    #[test]
    fn packed_ifft_ninv_postscale_matches_scalar_pairs() {
        use crate::fft::{ifft_borrowed, packed_ifft_ninv_postscale};
        use crate::types::{PrimeField64, Sample};

        type F = GoldilocksField;

        fn scalar_ninv(buffer: &mut [F], n_inv: F) {
            let n = buffer.len();
            if n == 0 {
                return;
            }
        ...
        for k in [1usize, 2, 3, 4, 5, 6, 8, 10] {
            ...
            let original = F::rand_vec(n);
            let mut expected = original.clone();
            scalar_ninv(&mut expected, n_inv);
            let mut actual = original.clone();
            packed_ifft_ninv_postscale(&mut actual, n_inv);
            for (idx, (a, e)) in actual.iter().zip(expected.iter()).enumerate() {
                assert_eq!(
                    a.to_canonical_u64(),
                    e.to_canonical_u64(),
            ...
            let borrowed = ifft_borrowed(&original);
            let classic = ifft(PolynomialValues::new(original));
            assert_eq!(borrowed.coeffs, classic.coeffs, "ifft_borrowed n={n}");
        }
    }
