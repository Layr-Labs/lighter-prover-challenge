    ...
    + DeserializeOwned
{
    ...
    /// Invert two equal-length batches and multiply the inverses into selected
    /// product slots. Backends may fuse the reverse Montgomery traversal with
    /// the indexed product update, avoiding materializing the inverse batches.
    ///
    /// # Safety
    ///
    /// Every entry of `outputs` must be a valid index into both product slices.
    #[doc(hidden)]
