...
impl Field for GoldilocksField {
    ...
    fn batch_multiplicative_inverse_pair_into(
        ...
        let mut i = WIDTH;
        while i < full_end {
            cumul0 = Self::multiply_quad(
                cumul0,
                x0[i..i + WIDTH].try_into().unwrap(),
            );
            cumul1 = Self::multiply_quad(
                cumul1,
                x1[i..i + WIDTH].try_into().unwrap(),
            );
            buf0.extend(cumul0);
            buf1.extend(cumul1);
            i += WIDTH;
        ...
        while end > WIDTH {
            let base = end - WIDTH;
            let inverse0 = Self::multiply_quad(
                buf0[base - WIDTH..base].try_into().unwrap(),
                a_inv0,
            );
            let inverse1 = Self::multiply_quad(
                buf1[base - WIDTH..base].try_into().unwrap(),
                a_inv1,
            );
            buf0[base..end].copy_from_slice(&inverse0);
            buf1[base..end].copy_from_slice(&inverse1);
            a_inv0 = Self::multiply_quad(
                a_inv0,
                x0[base..end].try_into().unwrap(),
            );
            a_inv1 = Self::multiply_quad(
                a_inv1,
                x1[base..end].try_into().unwrap(),
            );
            end = base;
        }
