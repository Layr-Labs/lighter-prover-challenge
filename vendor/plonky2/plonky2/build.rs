...
/// pair. Sixteen neighboring rows cooperatively stage all 136 production wire
/// columns (17 KiB), matching the profile-tested full-width shape. Every later
/// reference therefore uses the tile; the helper retains the original device
/// load for forward compatibility if a future circuit addresses a higher column.
...
fn build_aux_archive(source: &Path, output: &Path) -> Result<(), String> {
    ...
    let mut archiver = Command::new(&helper);
    archiver.env("TMPDIR", &tool_tmp);
    if command_line_tools.is_dir() && command_line_tools_sdk.is_dir() {
        archiver
            .env("DEVELOPER_DIR", command_line_tools)
            .env("SDKROOT", &command_line_tools_sdk);
