...
const TILE_COLS: usize = 136;
const TILE_LOAD_THREADS: usize = 32;
...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        if (argc != 3) {
            fprintf(stderr, "usage: range-tile-archiver source output\n");
            return 2;
        }
        ...
        id<MTLFunction> function =
            [library newFunctionWithName:@"range_check_gate_quotient_tiled"];
        if (function == nil) {
            fprintf(stderr, "Range/U32 tiled kernel missing after source compilation\n");
            return 1;
        }
        ...
        if (![archive addComputePipelineFunctionsWithDescriptor:pipeline_desc error:&error]) {
            return fail("add Range/U32 tiled pipeline", error);
        }
        ...
        // The ranked worker loads the archive through `newLibraryWithData:`.
        // Exercise that exact boundary and construct the pipeline before Cargo
        // accepts the artifact, rather than trusting a nonempty file alone.
        ...
        id<MTLFunction> reloaded_function =
            [reloaded newFunctionWithName:@"range_check_gate_quotient_tiled"];
        if (reloaded_function == nil || reloaded_function.functionType != MTLFunctionTypeKernel) {
            fprintf(stderr, "reloaded Range/U32 tiled entry is not a kernel\n");
            return 1;
        }
        id<MTLComputePipelineState> pipeline =
            [device newComputePipelineStateWithFunction:reloaded_function error:&error];
        if (pipeline == nil) return fail("lower reloaded Range/U32 tiled pipeline", error);
        return 0;
    }
}
"#;
...
fn main() {
    ...
    if cfg!(all(target_arch = "aarch64", target_os = "macos")) {
        if let Err(archive_error) = build_aux_archive(&generated_source, &output) {
            println!("cargo:warning=Range/U32 tiled GPU archive unavailable: {archive_error}");
            fs::write(&output, []).expect("cannot restore tiled Metal library placeholder");
            if let Err(air_error) = build_aux_metallib(&generated_source, &source_dir, &output) {
                println!("cargo:warning=Range/U32 tiled AIR library unavailable: {air_error}");
...
/// pair. One full 32-lane SIMDgroup cooperatively stages all 136 production
/// wire columns for sixteen neighboring rows (17 KiB), matching the
/// profile-tested full-width shape while using otherwise-idle lanes during the
/// load. The first sixteen lanes then evaluate the rows. Every later reference
/// therefore uses the tile; the helper retains the original device load for
/// forward compatibility if a future circuit addresses a higher column.
fn specialize_range_kernel(source: &str) -> Result<String, String> {
    ...
    replace_once(
        ...
        &format!(
            "    uint row_base = group_position.x * {tile_rows}u;\n\
             \tthreadgroup ulong tiled_wires[{tile_len}];\n\
             \tfor (uint item = tid; item < {tile_len}u; item += {load_threads}u) {{\n\
             \t    uint column = item / {tile_rows}u;\n\
             \t    uint row_lane = item - column * {tile_rows}u;\n\
             \t    uint staged_source_row = (row_base + row_lane) * step;\n\
             \t    tiled_wires[item] =\n\
             \t        device_wires[(ulong)column * lde_rows + staged_source_row];\n\
             \t}}\n\
             \tthreadgroup_barrier(mem_flags::mem_threadgroup);\n\
             \tif (tid >= {tile_rows}u) {{\n\
             \t    return;\n\
             \t}}\n\
             \tuint gid = row_base + tid;\n\
             \tif (gid >= quotient_rows) {{\n\
             \t    return;\n\
             \t}}\n\
             \tuint source_row = gid * step;\n",
            ...
            tile_rows = TILE_ROWS,
            load_threads = TILE_LOAD_THREADS,
        ),
    )?;
    ...
    let mut specialized = String::with_capacity(kernel_start + kernel.len() + helper.len());
    specialized.push_str(&source[..kernel_start]);
    // The auxiliary library resolves only the uniquely named tiled entry.
    // Keep every declaration and inline helper that precedes the source
    // kernel, then omit the untiled body and all unrelated later NTT/Merkle
    // kernels. This shortens both native archive construction and the worker's
    // asynchronous source fallback without changing the specialized body.
...
/// Builds and executes the same native M4 archive-helper path that reached an
/// official normal-class run for the parent2 experiment. The generated tile
/// source contains the established shader plus one specialized kernel; the
/// archive records only that new pipeline and therefore keeps its cold
/// AIR-to-ISA lowering outside every scored worker.
fn build_aux_archive(source: &Path, output: &Path) -> Result<(), String> {
    let out_dir = output.parent().ok_or("auxiliary output has no parent")?;
    let helper_source = out_dir.join("poseidon2_range_tile_archiver.m");
    let helper = out_dir.join("poseidon2_range_tile_archiver");
    let tool_tmp = out_dir.join("range-tile-tool-tmp");
    let module_cache = tool_tmp.join("clang-modules");
    fs::create_dir_all(&module_cache)
        .map_err(|error| format!("cannot create Range/U32 tool cache: {error}"))?;
    fs::write(&helper_source, ARCHIVER_SOURCE)
        .map_err(|error| format!("cannot write Range/U32 archive helper: {error}"))?;

    let command_line_tools = Path::new("/Library/Developer/CommandLineTools");
    let command_line_tools_sdk = command_line_tools.join("SDKs/MacOSX.sdk");
    let command_line_tools_bin = command_line_tools.join("usr/bin");
    let command_line_tools_clang = command_line_tools_bin.join("clang");
    // `/usr/bin/clang` is an xcrun/libxcselect shim on the ranked host and
    // tries to update the sandbox-denied per-user xcrun_db before it honors
    // ordinary compiler arguments. Prefer the installed compiler executable
    // itself so this subprocess never enters that lookup path.
    let clang_executable = if command_line_tools_clang.is_file() {
        command_line_tools_clang.as_path()
    } else {
        Path::new("/usr/bin/clang")
    };
    let mut clang = Command::new(clang_executable);
    clang
        // The ranked build sandbox permits Cargo's OUT_DIR but not the host's
        // default Darwin user temp/cache directory. Keep both xcrun's database
        // and Clang's framework-module cache beside the generated artifact.
        .env("TMPDIR", &tool_tmp)
        .arg("-fobjc-arc")
        .arg(format!(
            "-fmodules-cache-path={}",
    ...
    if command_line_tools.is_dir() && command_line_tools_sdk.is_dir() {
        // `/usr/bin/clang` is an Apple developer-tool shim. An explicit
        // isysroot alone does not stop libxcselect from resolving the active
        // developer directory through its sandbox-denied xcrun_db cache.
        // Apple's documented process-local override bypasses that lookup and
        // requires neither xcode-select mutation nor elevated privileges.
        clang
            .env("DEVELOPER_DIR", command_line_tools)
            .env("SDKROOT", &command_line_tools_sdk)
            .arg(format!("-B{}", command_line_tools_bin.to_string_lossy()))
            .arg("-isysroot")
            .arg(&command_line_tools_sdk);
    }
    let compiled = clang
        .args(["-framework", "Metal", "-framework", "Foundation"])
        .arg(&helper_source)
        .arg("-o")
        .arg(&helper)
        .output()
        .map_err(|error| format!("cannot launch clang for Range/U32 archive helper: {error}"))?;
    if !compiled.status.success() {
        return Err(command_failure("clang Range/U32 archiver", &compiled));
    }
    ...
    let archived = Command::new(&helper)
        ...
        .arg(output)
        .output()
        .map_err(|error| format!("cannot launch Range/U32 archiver: {error}"))?;
    if !archived.status.success() {
        return Err(command_failure("Range/U32 archiver", &archived));
    }
    let length = fs::metadata(output)
        .map_err(|error| format!("cannot inspect Range/U32 GPU archive: {error}"))?
        .len();
    if length == 0 {
        return Err("Range/U32 GPU archive is empty".to_owned());
    }
    println!("cargo:warning=embedded {length}-byte Range/U32 tiled GPU archive");
    Ok(())
}
