...
fn build_aux_archive(source: &Path, output: &Path) -> Result<(), String> {
    ...
    let command_line_tools_sdk = Path::new("/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk");
    if command_line_tools_sdk.is_dir() {
        // Avoid asking xcrun to discover the SDK through its otherwise
        // sandbox-denied per-user cache. This is also the active developer
        // directory reported by the ranked host when the Metal CLI is absent.
        clang.arg("-isysroot").arg(command_line_tools_sdk);
    }
    let compiled = clang
