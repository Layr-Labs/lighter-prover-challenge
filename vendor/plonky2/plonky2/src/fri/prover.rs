...
// p3e-proven-domain-guard-1786604900.
...
>(
    ...
    // The differential component oracle proves this retained-column mapping
    // only for the ranked production d16/d18, arity-16, cap-4 shapes. Keep
    // every other (including private/unseen) shape on the established flat
    // path instead of allowing the host-dependent allocator to broaden the
    // semantic surface merely because a GPU happens to admit it.
    const D16_LDE_LEN: usize = 1 << (16 + 3);
    const D18_LDE_LEN: usize = 1 << (18 + 3);
    if arity != 16
        || cap_height != 4
        || !matches!(values.len(), D16_LDE_LEN | D18_LDE_LEN)
    {
        return None;
    }
