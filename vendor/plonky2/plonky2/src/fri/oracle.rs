...
pub(crate) fn coset_fft_zero_tail<F: Extendable<D>, const D: usize>(
    ...
    // The FRI coset is generated by a base-field shift, so every `shift^i`
    // has zero non-base limbs. Multiplying two extension elements would form
    // and reduce those known-zero cross-products. Keep the powers in the base
    // field and scale each extension limb directly instead.
    scaled.extend(
        shift
            .powers()
...
mod tests {
    ...
    fn coset_fft_zero_tail_matches_classic() {
        fn check<F: Field + Sample + Extendable<D>, const D: usize>()
        where
            F::Extension: Sample,
        {
            for lg_n in [1usize, 2, 4, 6, 9] {
                ...
                for rate_bits in 1..=3usize.min(lg_n) {
                    ...
                    for live in [support, support / 2, support.saturating_sub(1)] {
                        let mut coeffs = F::Extension::rand_vec(live);
                        coeffs.resize(n, F::Extension::ZERO);
                ...
                let poly = PolynomialCoeffs::new(F::Extension::rand_vec(n));
                let shift = F::rand();
                assert_eq!(
        ...
        check::<GoldilocksField, 1>();
        check::<GoldilocksField, 2>();

        // Raw-representation seam: embedded extension multiplication and
        // per-limb scalar multiplication must agree even for noncanonical
        // Goldilocks words, not merely after canonicalization.
        ...
        let mut coeffs = (0..live)
            .map(|i| {
                QuadraticExtension([
                    GoldilocksField(edge_words[i % edge_words.len()]),
                    GoldilocksField(edge_words[(i * 5 + 1) % edge_words.len()]),
                ])
            })
            .collect::<Vec<_>>();
        coeffs.resize(n, QuadraticExtension::ZERO);
        let poly = PolynomialCoeffs::new(coeffs);
        let shift = GoldilocksField(u64::MAX);
        let expected = poly.coset_fft_with_options(shift.into(), Some(3), None);
        let actual = coset_fft_zero_tail::<GoldilocksField, 2>(&poly, shift, live, Some(3), None);
        assert_eq!(actual.values, expected.values);
    }
