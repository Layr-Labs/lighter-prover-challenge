    ...
    PolynomialBatch<F, C, D>
{
    ...
    pub fn prove_openings(
        ...
        let lde_final_values = timed!(
            ...
            coset_fft_zero_tail_base_shift::<F, D>(
                &lde_final_poly,
                F::coset_shift(),
                live_coeffs,
...
/// Zero-tail FFT for extension coefficients when the coset shift lies in the
/// base field. Scaling each extension limb directly avoids the unused cross
/// products of multiplying by an embedded extension element `(r, 0)`.
pub(crate) fn coset_fft_zero_tail_base_shift<BF: Extendable<D>, const D: usize>(
    ...
    if zero_tail_is_unread {
        // SAFETY: identical zero-tail contract to `coset_fft_zero_tail`.
        unsafe { scaled.set_len(len) };
    } else {
...
mod tests {
    ...
    /// The specialization runs before FFT butterflies, so it must preserve
    /// raw Goldilocks representatives as well as the abstract field value.
    #[test]
        ...
        for &a0 in &words {
            for &a1 in &words {
                ...
                for &scalar in &words {
                    ...
                    let expected_raw: [u64; 2] =
                        <E as FieldExtension<2>>::to_basefield_array(&expected)
                            .map(|x| x.to_noncanonical_u64());
                    let actual_raw: [u64; 2] =
                        <E as FieldExtension<2>>::to_basefield_array(&actual)
                            .map(|x| x.to_noncanonical_u64());
