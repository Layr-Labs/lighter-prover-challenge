    ...
    PolynomialBatch<F, C, D>
{
    ...
    /// Whether a contiguous PolyMajor gather can visit one stored column at a
    /// time without falling back through row leaves.
    pub(crate) fn has_column_lde_storage(&self) -> bool {
        matches!(&self.merkle_tree.leaves, MerkleLeaves::Columns { .. })
    }

    /// Borrows consecutive values from one retained LDE column. This is used
    /// by consumers that can combine a circuit-fixed column with another hot
    /// gather without first materializing a second compact copy.
    pub(crate) fn lde_column_contiguous(
        &self,
        column: usize,
        index_start: usize,
        n: usize,
        ...
        let index_end = index_start.checked_add(n)?;
        columns.col(column).get(index_start..index_end)
    }
    ...
    /// The column-backed contiguous gather with a callback immediately after
    /// each destination column has been written. This preserves the exact
    /// copy path while allowing a caller to consume a hot compact column
    /// before the next column displaces it. Returns `false` for row-backed
    /// leaves without touching `out`.
    pub(crate) fn fill_lde_batch_contiguous_with<FN>(
        &self,
        index_start: usize,
        n: usize,
        col_range: core::ops::Range<usize>,
        out: &mut Vec<F>,
        mut consume_column: FN,
    ) -> bool
    where
        FN: FnMut(usize, &[F]),
    {
