    ...
    PolynomialBatch<F, C, D>
{
    ...
    pub fn from_coeffs_with_ready_ldes(
        ...
        if let Some(mut columns) = C::Hasher::try_allocate_merkle_tree_columns(
            ...
            // Marker: ready-stream-1786702000. `from_coeffs` already overlaps
            // each 8-column LDE fill with the previous group's GPU sponge
            // absorb. This ready-LDE entry used to materialize every column
            // and only then hash, so early-IFFT columns sat idle through a
            // serial hash. Same per-column body as the fallback below
            // (memcpy a ready LDE, else the production fill+FFT); only the
            // schedule changes. A wrong absorb is a Merkle-cap / FRI
                ...
                C::Hasher::try_build_merkle_tree_column_store_streamed(
                    ...
                    &|group, destinations: &mut [&mut [F]]| {
                        destinations.par_iter_mut().enumerate().for_each(
                            |(k, destination)| {
                                let j = group * 8 + k;
                                if let Some(lde) = ready.get(j).and_then(|c| c.as_ref()) {
                                    debug_assert_eq!(lde.len(), destination.len());
