...
/// Scale each extension limb by the base-field power rather than performing a
/// general extension multiplication by an embedded scalar.
pub(crate) fn coset_fft_zero_tail_base_shift<BF: Extendable<D>, const D: usize>(
    ...
    if zero_tail_is_unread {
        // SAFETY: the zero-tail FFT reads only the initialized live prefix
        // before overwriting every tail slot.
        unsafe { scaled.set_len(len) };
    } else {
