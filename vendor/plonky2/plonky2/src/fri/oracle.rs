...
pub(crate) fn coset_fft_zero_tail_base_shift<BF: Extendable<D>, const D: usize>(
    ...
    let mut scaled = Vec::with_capacity(len);
    if shift == BF::coset_shift() {
        // Every ordinary LDE already publishes this process-global table.
        // Reuse it for the final opening FFT instead of rebuilding the same
        // serial power chain. FRI rounds whose shift has been exponentiated
        // retain the generic base-scalar path below.
        let powers = crate::plonk::prover::precomputed::coset_shift_powers::<BF>(live);
        scaled.extend(
            coeffs.coeffs[..live]
                .iter()
                .zip(powers.iter())
                .map(|(c, &r)| c.scalar_mul(r)),
        );
    } else {
        scaled.extend(
            shift
