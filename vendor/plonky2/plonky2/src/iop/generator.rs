    ...
    PendingPartitionWitness<'a, F, C, D>
{
    ...
    /// Starts witness generation from an already-seeded `PartitionWitness`
    /// (values written directly into representative slots, bypassing the
    /// `PartialWitness` hash map entirely). The ready-generator worklist is
    /// derived purely from the partition's set-ness, so a pre-seeded witness
    /// behaves identically to one seeded from a map with the same contents.
    pub fn start_from_partition(
        witness: PartitionWitness<'a, F>,
        prover_data: &'a ProverOnlyCircuitData<F, C, D>,
        common_data: &CommonCircuitData<F, D>,
    ) -> Result<Self> {
        Self::start_partition_with_threshold(
            witness,
            prover_data,
            common_data,
            PARALLEL_WORKLIST_THRESHOLD,
    ...
    fn start_with_threshold(
        ...
        Self::start_partition_with_threshold(witness, prover_data, common_data, parallel_threshold)
    }

    fn start_partition_with_threshold(
        witness: PartitionWitness<'a, F>,
        prover_data: &'a ProverOnlyCircuitData<F, C, D>,
        common_data: &CommonCircuitData<F, D>,
        parallel_threshold: usize,
