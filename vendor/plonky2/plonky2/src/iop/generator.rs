...
/// Set to `0` to force the legacy materialized `Vec<usize>` queue for the first
/// all-generators-ready round. The default uses an [`InitialAll`] range.
#[cfg(feature = "std")]
...
fn generator_initial_range_enabled() -> bool {
    ...
    *ENABLED.get_or_init(|| {
        !std::env::var_os(GENERATOR_INITIAL_RANGE_ENV).is_some_and(|value| value == "0")
    })
}
