...
>(
    ...
    for (watch, watchers) in generator_indices_by_watches.iter() {
...
mod tests {
    ...
    #[test]
    fn generator_watch_index_retains_sparse_lookup_and_order() {
        let index = crate::plonk::circuit_data::GeneratorWatchIndex::from_map(BTreeMap::from([
            (1, vec![3, 7]),
            (2, vec![]),
            (4, vec![2]),
        ]));
        ...
        assert_eq!(index.get(1), Some(&[3, 7][..]));
    ...
    #[test]
    fn circuit_data_serialization_round_trip_preserves_watcher_bytes() {
        let mut builder = CircuitBuilder::<F, D>::new(CircuitConfig::standard_recursion_config());
        ...
        let bytes = data
            ...
            .unwrap();
        let decoded = crate::plonk::circuit_data::CircuitData::<F, C, D>::from_bytes(
            &bytes,
            &gate_serializer,
        ...
        assert_eq!(
            data.prover_only.generator_indices_by_watches,
