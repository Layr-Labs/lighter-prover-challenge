...
/// Parallel rounds only pay off when the pool actually has more than one thread; otherwise the
/// per-round buffer collection is pure overhead over the sequential loop.
...
mod tests {
    ...
    fn parallel_worklist_matches_sequential_for_recursive_circuit() -> Result<()> {
        ...
        // Parallel at the default threshold, at a stress threshold that parallelizes every
        // round, and split across start/feed with the stress threshold.
    ...
    fn parallel_worklist_synthetic_fanout_timing() -> Result<()> {
        ...
        let measure = |label: &str, threshold: usize| -> Result<()> {
            let mut best = None;
            for _ in 0..5 {
        ...
        measure("sequential (threshold=MAX)", usize::MAX)?;
        for &threshold in &[16usize, 64, 256, 1024] {
            measure(&format!("parallel threshold={threshold}"), threshold)?;
        }
