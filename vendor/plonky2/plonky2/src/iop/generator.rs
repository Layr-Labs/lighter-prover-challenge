...
/// later runs for the same circuit replay that order directly, dispatching each generator exactly
/// once instead of rediscovering the execution order through the watch map. See
...
>(
    ...
    #[cfg(feature = "std")]
    {
        ...
        match cell.get() {
            ...
            {
                run_recorded_schedule(&mut witness, prover_data, schedule)?;
            }
...
/// dispatch. If a dispatch nevertheless diverges — a dependency is unset or a generator does not
/// finish, e.g. because a different set of input targets was provided than when the schedule was
/// recorded — the values produced by the failed dispatch are discarded and the remaining
/// generators are completed by the dynamic scheduler, which is correct from any partially
/// populated witness state.
#[cfg(feature = "std")]
...
>(
    ...
    for (position, &generator_idx) in order.iter().enumerate() {
        ...
        if !finished {
            ...
            // dispatch produced (the dynamic scheduler will regenerate it legitimately) and
            // finish dynamically; generators replayed so far each ran with satisfied
