...
>(
    ...
    //
    // The queues hold generator indices, which the embedded CSR watcher
    // format already bounds to `u32`. The initial native-width vector is
    // checked once at entry, and every later append originates from the `u32`
    // watcher stream, so narrowing both queues halves the index traffic on
    // the hottest witness-generation path. Indices are widened only at the
    // point where they index arrays.
    let mut pending_generator_indices: Vec<u32> = pending_generator_indices
        .into_iter()
        .map(|index| u32::try_from(index).expect("generator index exceeds u32"))
        .collect();
    let mut next_pending_generator_indices: Vec<u32> = Vec::new();
                    ...
                    for (t, v, rep_index, watchers) in annotated_values.by_ref().take(value_count) {
                        ...
                        if let Some(watchers) = watchers {
                            for &watching_generator_idx in watchers {
                                let watching = watching_generator_idx as usize;
                                if !generator_is_expired[watching] {
                                    debug_assert_ne!(unresolved_watches[watching], 0);
                                    unresolved_watches[watching] -= 1;
