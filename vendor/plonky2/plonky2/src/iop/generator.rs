...
>(
    ...
    generate_partial_witness_with(prover_data, common_data, move |witness| {
...
/// Creates a partition witness, fills its inputs directly, then runs the existing generator
/// scheduler. This avoids staging inputs in a separate [`PartialWitness`].
pub fn generate_partial_witness_with<
    'a,
    F: RichField + Extendable<D>,
    C: GenericConfig<D, F = F>,
    Fill,
...
where
    Fill: FnOnce(&mut PartitionWitness<'a, F>) -> Result<()>,
{
...
mod tests {
    ...
    #[test]
    fn direct_fill_matches_legacy_and_enforces_partition_aliases() {
        let mut builder = CircuitBuilder::<F, D>::new(CircuitConfig::standard_recursion_config());
        let input = builder.add_virtual_target();
        let input_alias = builder.add_virtual_target();
        let output = builder.add_virtual_target();
        builder.connect(input, input_alias);
        builder.generate_copy(input, output);
        builder.register_public_input(output);

        let circuit = builder.build::<C>();
        let value = F::from_canonical_u64(17);

        let legacy_witness = || {
            let mut inputs = PartialWitness::new();
            inputs.set_target(input, value).unwrap();
            inputs.set_target(input_alias, value).unwrap();
            generate_partial_witness(inputs, &circuit.prover_only, &circuit.common).unwrap()
        };
        let direct_witness = || {
            generate_partial_witness_with(
                &circuit.prover_only,
                &circuit.common,
                |witness| {
                    witness.set_target(input, value)?;
                    witness.set_target(input, value)?;
                    witness.set_target(input_alias, value)
                },
            )
            .unwrap()
        };

        let legacy = legacy_witness();
        let legacy_again = legacy_witness();
        let direct = direct_witness();
        let direct_again = direct_witness();

        // Zero-knowledge blinding generators deliberately randomize unused representatives. Compare
        // every representative which is stable within both routes, and check the semantic targets
        // explicitly below.
        let mut deterministic_representatives = 0;
        for (((legacy_value, legacy_again_value), direct_value), direct_again_value) in legacy
            .values
            .iter()
            .zip(&legacy_again.values)
            .zip(&direct.values)
            .zip(&direct_again.values)
        {
            let legacy_is_stable = legacy_value == legacy_again_value;
            let direct_is_stable = direct_value == direct_again_value;
            assert_eq!(legacy_is_stable, direct_is_stable);
            if legacy_is_stable {
                assert_eq!(legacy_value, direct_value);
                deterministic_representatives += 1;
            }
        }
        assert!(deterministic_representatives > 0);

        assert_eq!(direct.get_target(input), value);
        assert_eq!(direct.get_target(input_alias), value);
        assert_eq!(direct.get_target(output), value);

        let conflicting_duplicate = generate_partial_witness_with(
            &circuit.prover_only,
            &circuit.common,
            |witness| {
                witness.set_target(input, value)?;
                witness.set_target(input, value + F::ONE)
            },
        )
        .unwrap_err();
        assert!(
        ...
        let conflicting_alias = generate_partial_witness_with(
            &circuit.prover_only,
            &circuit.common,
            |witness| {
                witness.set_target(input, value)?;
                witness.set_target(input_alias, value + F::ONE)
            },
        )
        .unwrap_err();
        assert!(
