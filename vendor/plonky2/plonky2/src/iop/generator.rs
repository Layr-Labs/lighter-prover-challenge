...
>(
    ...
    // These totals are derived once while circuit data is built (and reconstructed when prover
    // data is deserialized). Starting from them avoids rescanning every watcher edge for every
    // proof; only representatives first populated by the partial witness need to be subtracted.
...
mod tests {
    ...
    #[test]
    fn derived_generator_watch_counts_round_trip_without_changing_serialized_bytes() {
        let mut builder = CircuitBuilder::<F, D>::new(CircuitConfig::standard_recursion_config());
        ...
        let circuit = builder.build::<C>();
        let expected_counts = circuit.prover_only.generator_watch_counts.clone();
        ...
        assert_eq!(restored.prover_only.generator_watch_counts, expected_counts);
        assert_eq!(
            restored
                .to_bytes(&gate_serializer, &generator_serializer)
                .unwrap(),
        ...
        let mut inputs = PartialWitness::new();
        inputs
            .set_target(input, F::from_canonical_u64(19))
            .unwrap();
        let witness =
            generate_partial_witness(inputs, &restored.prover_only, &restored.common).unwrap();
        assert_eq!(witness.get_target(output), F::from_canonical_u64(19));
    }
}
