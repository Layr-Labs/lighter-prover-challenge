...
impl<'a, F: Field> PartitionWitness<'a, F> {
    pub fn new(num_wires: usize, degree: usize, representative_map: &'a [usize]) -> Self {
        Self {
            ...
            set_bitmap: vec![0; (representative_map.len() + 63) / 64],
            representative_map,
            num_wires,
            degree,
        }
    ...
    pub fn set_target_returning_rep(&mut self, target: Target, value: F) -> Result<Option<usize>> {
        ...
        let bitmap_word = &mut self.set_bitmap[rep_index / 64];
        let bitmap_mask = 1u64 << (rep_index % 64);
        if *bitmap_word & bitmap_mask != 0 {
            if value != self.values[rep_index] {
                return Err(anyhow!(
            ...
            *bitmap_word |= bitmap_mask;
            Ok(Some(rep_index))
        }
    }

    pub(crate) fn is_set_by_rep_index(&self, rep_index: usize) -> bool {
        self.set_bitmap[rep_index / 64] & (1u64 << (rep_index % 64)) != 0
    }
    ...
    pub fn full_witness(self) -> MatrixWitness<F> {
        ...
        for i in 0..self.degree {
            for j in 0..self.num_wires {
                wire_values[j][i] = self.values[self.representative_map[i * self.num_wires + j]];
            }
        }
...
impl<F: Field> Witness<F> for PartitionWitness<'_, F> {
    fn try_get_target(&self, target: Target) -> Option<F> {
        let rep_index = self.representative_map[self.target_index(target)];
        self.is_set_by_rep_index(rep_index).then_some(self.values[rep_index])
    }
}
