...
impl<'a, F: Field> PartitionWitness<'a, F> {
    ...
    pub fn full_witness(self) -> MatrixWitness<F> {
        ...
        // This section sits on the serial per-proof spine, so splitting it
        // across workers removes wall time the fused pass alone could not.
                ...
                .for_each(|(chunk, columns)| {
                    ...
                    for i in 0..rows {
                        for column in columns.iter_mut() {
                            column[i]
                                .write(self.values[self.representative_map[wire_index]]);
                            wire_index += 1;
                        }
                    }
                });
