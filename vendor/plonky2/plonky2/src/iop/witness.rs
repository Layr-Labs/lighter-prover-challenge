...
impl<'a, F: Field> PartitionWitness<'a, F> {
    ...
    #[cfg(test)]
    pub(crate) fn representative_values(&self) -> impl Iterator<Item = Option<F>> + '_ {
        (0..self.values.len()).map(|rep_index| self.representative_value(rep_index))
    }
...
mod tests {
    ...
    #[test]
    fn partition_witness_sparse_storage_produces_a_valid_proof_on_fused_frontier() {
        const D: usize = 2;
