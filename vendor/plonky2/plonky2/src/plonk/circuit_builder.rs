...
impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilder<F, D> {
    ...
    fn constant_polys(&self) -> Vec<PolynomialValues<F>> {
        let max_constants = self
            ...
            .max()
            .unwrap();
        // Emit each constant column directly instead of materializing a padded
        // row-major copy of every instance's constants (one Vec clone + resize
        // per instance) and then transposing it. Column `c`, row `j` receives
        // exactly the value the pad+transpose produced: `constants[c]` when
        // present, else the `F::ZERO` padding.
