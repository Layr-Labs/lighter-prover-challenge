...
impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilder<F, D> {
    ...
    pub fn try_build_with_options<C: GenericConfig<D, F = F>>(
        ...
        for (i, generator) in self.generators.iter().enumerate() {
            let mut watch_rep_indices = generator
                .0
                .watch_list()
                .into_iter()
                .map(|watch| forest.parents[forest.target_index(watch)])
                .collect::<Vec<_>>();
            watch_rep_indices.sort_unstable();
            watch_rep_indices.dedup();
            generator_watch_counts[i] = u32::try_from(watch_rep_indices.len())
                .expect("generator watches more than u32::MAX target representatives");
