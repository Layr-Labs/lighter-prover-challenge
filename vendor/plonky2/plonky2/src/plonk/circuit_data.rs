...
> {
    ...
    /// Compact even-row companion of the leading *constants* columns of
    /// [`Self::constants_sigmas_commitment`] (row `k` is LDE row `2k`); see
    /// [`EvenColumns`]. The half-domain Range/U32 quotient dispatch indexes
    /// the constants store with the wires row count, so gates that read gate
    /// constants need a store already restricted to the even rows. Those
    /// columns are circuit-fixed, so this is built once per circuit rather
    /// than once per proof. Runtime-only: not serialized, and empty wherever
    /// the half-domain split cannot run.
    pub constants_even_columns: EvenColumns<F>,
...
mod generator_watch_index_tests {
    ...
    fn narrow_watch_index_matches_wide_reference_raw_words() {
        ...
        fn edge_lists(seed: u64) -> BTreeMap<usize, Vec<usize>> {
            ...
            map.insert(4096, vec![0, 1, 255, 256, 65_535, 65_536, 16_777_215, 16_777_216]);
        ...
        fn reference_get(map: &BTreeMap<usize, Vec<usize>>, representative: usize) -> Option<Vec<u64>> {
