...
impl Forest {
    ...
    /// representative.
    ///
    /// Representatives are fixed by merge history and invariant under compression order, so
    /// root resolution is a read-only map over the immutable pre-pass array and parallelizes
    /// cleanly. The transient `roots` vector is freed as soon as it replaces `parents`.
