...
pub trait Hasher<F: RichField>: Sized + Copy + Debug + Eq + PartialEq {
    ...
    /// independent permutation chains while preserving individual results.
    fn hash_or_noop_pair(input_a: &[F], input_b: &[F]) -> (Self::Hash, Self::Hash) {
        (Self::hash_or_noop(input_a), Self::hash_or_noop(input_b))
    }

    /// Four-input equivalent of [`Hasher::hash_or_noop_pair`].
    ...
    /// Compute two independent node compressions together.
    fn two_to_one_pair(
        x0: Self::Hash,
        y0: Self::Hash,
        x1: Self::Hash,
