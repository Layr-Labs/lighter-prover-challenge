...
mod quotient_layout_tests {
    ...
    /// Pins this archive to the post-butterfly terminal writer without
    /// changing release code or the transform itself.
    #[test]
    fn quotient_ifft_terminal_archive_identity_regression() -> Result<()> {
        let butterflies = vec![
        ...
        let n_inv = F::inverse_2exp(crate::util::log2_strict(butterflies.len()));
        for k in 0..butterflies.len() {
            let source = if k == 0 { 0 } else { butterflies.len() - k };
            let mut expected = butterflies[source];
            expected *= n_inv;
            expected *= scales[k];
            assert_eq!(
                actual[k / 2].coeffs[k % 2].to_noncanonical_u64(),
                expected.to_noncanonical_u64()
            );
        }
        assert!(
            !FORCE_LEGACY_QUOTIENT_IFFT_TERMINAL.with(core::cell::Cell::get)
        );
        Ok(())
    }
