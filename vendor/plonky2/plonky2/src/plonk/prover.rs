                    ...
                    for (t, &x) in xs.iter().enumerate() {
                        ...
                        for chunk in 0..num_chunks {
                            ...
                            for j in start..end {
                                ...
                                // Marker: pp-mulacc-1786712000. On the
                                // perm-inv-merge 30.560 keeper. Numerator
                                // and denominator of a challenge share
                                // `wire + gamma` and differ by `beta_k*x`
                                // vs `beta*sigma`. One addend + the
                                // existing `multiply_accumulate` (self +
                                // x*y) is field-identical by commutativity
                                // of add. Wrong product is a wrong Z —
                                // fail-closed.
                                let base_0 = wire_value + gamma_0;
                                let base_1 = wire_value + gamma_1;
                                numerator_0 *=
                                    base_0.multiply_accumulate(beta_k_is_0[j], x);
                                numerator_1 *=
                                    base_1.multiply_accumulate(beta_k_is_1[j], x);
                                denominator_0 *=
                                    base_0.multiply_accumulate(beta_0, sigma);
                                denominator_1 *=
                                    base_1.multiply_accumulate(beta_1, sigma);
                            }
