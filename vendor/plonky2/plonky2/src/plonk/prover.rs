...
>(
    ...
    // Every producer contributes a numerator over the same Z_H denominator.
    // Sum the numerators first and multiply once while writing the IFFT
    // columns. This also removes the per-batch CPU denominator loop above.
    quotient_values
        ...
        .for_each(|(i, cpu_values)| {
            ...
            for (challenge, (&cpu, column)) in cpu_values.iter().zip(column_ptrs).enumerate()
            {
                let mut numerator = cpu;
                #[cfg(all(feature = "std", target_arch = "aarch64", target_os = "macos"))]
                {
                    if let Some(values) = gpu_poseidon_values {
                        numerator += values[start + challenge];
                    }
                ...
                // SAFETY: point `i` is owned by this parallel iteration, and
                // every (challenge, point) destination is written exactly once.
                unsafe { *column.0.add(i) = numerator * denominator_inv };
            }
        });
