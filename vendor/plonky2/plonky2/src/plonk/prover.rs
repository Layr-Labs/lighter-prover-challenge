        ...
        if let Some(u32_gate) = u32_gate {
            let (kind, num_ops, expected_wires, expected_constraints) = match u32_gate {
                ...
                U32QuotientGate::RandomAccess {
                    ...
                    // array holding half the list, so the supported list
                    ...
                    let routed = vec_size
                        .checked_add(2)?
                        .checked_mul(num_copies)?
                        .checked_add(num_extra_constants)?;
                    (
                        U32QuotientKind::RandomAccess {
                            bits,
                            num_extra_constants,
                            // The gate sees its constants after the selector
                            // prefix is removed, so `local_constants[i]` is
                            // this absolute constants column plus `i`.
                            constants_base: common_data.selectors_info.num_selectors()
                                + common_data.num_lookup_selectors,
                        },
                        num_copies,
                        routed.checked_add(num_copies.checked_mul(bits)?)?,
                        num_copies
