...
>(
    ...
    // Keep the final GPU contribution separate so it can be folded directly
    // into the point-major -> challenge-column scatter below. Earlier GPU
    // contributions are still merged as soon as they finish, preserving the
    // existing overlap between that CPU work and later commands on Metal's
    // serialized queue.
    #[cfg(all(feature = "std", target_arch = "aarch64", target_os = "macos"))]
    let mut final_gpu_values: Option<&[F]> = None;
    #[cfg(not(all(feature = "std", target_arch = "aarch64", target_os = "macos")))]
    let final_gpu_values: Option<&[F]> = None;

    #[cfg(all(feature = "std", target_arch = "aarch64", target_os = "macos"))]
    if let Some((_, job)) = &gpu_poseidon {
        let defer_merge = gpu_range.is_none() && gpu_permutation.is_none();
        ...
        if defer_merge {
            final_gpu_values = Some(gpu_values);
        } else {
            quotient_values
    ...
    if let Some((_, job)) = &gpu_range {
        let defer_merge = gpu_permutation.is_none();
        let gpu_values = match job.finish() {
            Ok(values) => {
    ...
    if let Some(gpu_values) = final_gpu_values {
        quotient_values
            ...
            .for_each(|(chunk_i, (cpu_chunk, gpu_chunk))| {
                    ...
                    .enumerate()
                {
                    ...
                    for ((column, &cpu), &gpu) in column_ptrs
                        .iter()
                        .zip(cpu_values)
                        .zip(gpu_values)
                    {
                        let mut value = cpu;
                        value += gpu * denominator_inv;
