...
>(
    ...
    for (gate_index, gate) in common_data.gates.iter().enumerate() {
            ...
            gate.0.as_any().downcast_ref::<ExponentiationGate<F, D>>()
        {
            ...
            // which sits on the serial tail; measurement favours the
            // offload both solo and under two-worker contention, so it is
            // the default. `LIGHTER_EXP_GPU=off` restores the CPU-only
