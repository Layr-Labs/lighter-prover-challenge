...
use crate::plonk::vanishing_poly::{
    deferred_alpha_reduction_enabled, direct_exponentiation_alpha_plan,
    eval_vanishing_poly_base_batch, exponentiation_alpha_fusion_enabled,
    gate_constraint_alpha_powers, get_lut_poly, interleave_pair_plan, PermutationBatch,
    VanishingScratch,
};
...
>(
    ...
    let direct_gate_alpha_plan = direct_exponentiation_alpha_plan(
        common_data,
        &cpu_gate_indices,
        interleave_pair.as_ref(),
        enable_deferred_alpha_reduction,
        exponentiation_alpha_fusion_enabled(),
        alphas.len(),
        true,
    );
    #[cfg(all(feature = "std", target_arch = "aarch64", target_os = "macos"))]
