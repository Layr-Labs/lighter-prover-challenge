...
>(
    ...
    // One common denominator scale for all merged numerator producers. In the
    // CPU-only case this pass is skipped: the batch loop already multiplied by
    // `Z_H(i)^-1`, preserving the established schedule byte for byte.
    if any_gpu_quotient {
        quotient_values
            .par_chunks_exact_mut(num_challenges)
            .enumerate()
