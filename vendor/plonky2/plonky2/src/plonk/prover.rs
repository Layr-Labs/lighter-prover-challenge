...
>(
    ...
    for (gate_index, gate) in common_data.gates.iter().enumerate() {
        ...
        if let Some(u32_gate) = u32_gate {
            // The four- and six-bit random-access shapes evaluate 16- and
            // 64-entry selection folds inside the process-shared Range/U32
            // Metal command. Keep them on the existing CPU
