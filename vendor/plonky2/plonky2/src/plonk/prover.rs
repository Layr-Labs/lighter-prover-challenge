    ...
    C::InnerHasher: Hasher<F>,
{
    ...
    let (wires_coeffs, ready_ldes) = timed!(
        ...
        {
            // Marker: skip-ifft-1786710000. On the perm-inv-merge 30.560
            // keeper. v2 LDE body is official at 30.379; preimage equality
            // implies a unique IFFT and that LDE. Skip the tail `2^18` IFFT
            // of matching columns. Solo draw `119a43a` was 25.711
            // slow-class verify only. Fail-closed: mismatch recomputes;
            // wrong reuse is a Merkle-cap / FRI reject.
