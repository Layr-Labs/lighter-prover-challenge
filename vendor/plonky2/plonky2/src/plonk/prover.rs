...
>(
    ...
    // The two Z chains share no state: each walks its own product buffer,
    // keeps its own running `z_x`, and writes its own columns. After the
    // fused product pass they were still sequential on the per-proof spine
    // (between the Rayon product join and the Zs/partial-products
    // commitment). `join` runs the identical helpers on the two buffers;
    // every limb is bit-identical to the serial pair because each helper is
    // a pure function of its own argument. This does not change INV_BATCH,
    // the product pass, or any Metal routing floor.
