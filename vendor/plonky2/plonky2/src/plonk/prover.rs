...
>(
    ...
    let quotient_values: Vec<F> = points_batches
        ...
        .flat_map(|(batch_i, xs_batch)| {
            ...
            // Stack-resident index strip: avoids a per-batch heap `Vec<usize>` of
            // arithmetic indices while preserving the exact `indices_batch` slice
            // shape consumed by `eval_vanishing_poly_base_batch`.
            let index_start = BATCH_SIZE * batch_i;
            let mut indices_arr = [0usize; BATCH_SIZE];
            for (j, slot) in indices_arr[..xs_batch.len()].iter_mut().enumerate() {
                *slot = index_start + j;
            }
            let indices_batch = &indices_arr[..xs_batch.len()];

            let batch_len = xs_batch.len();

            // Stack-resident batch buffers: every per-batch scratch buffer above
            // is at most BATCH_SIZE elements, so all nine heap `Vec`s that used
            // to be allocated per batch now live on the rayon worker stack.
            // Only the flattened constant/wire matrices (sized by the circuit
            // width) and the returned quotient values remain heap-backed.
            let mut shifted_xs_arr = [F::ZERO; BATCH_SIZE];
            let mut local_zs_arr: [&[F]; BATCH_SIZE] = [&[]; BATCH_SIZE];
            let mut next_zs_arr: [&[F]; BATCH_SIZE] = [&[]; BATCH_SIZE];
            let mut local_lookup_arr: [&[F]; BATCH_SIZE] = [&[]; BATCH_SIZE];
            let mut next_lookup_arr: [&[F]; BATCH_SIZE] = [&[]; BATCH_SIZE];
            let mut partial_products_arr: [&[F]; BATCH_SIZE] = [&[]; BATCH_SIZE];
            let mut s_sigmas_arr: [&[F]; BATCH_SIZE] = [&[]; BATCH_SIZE];
            let mut local_constants_refs_arr: [&[F]; BATCH_SIZE] = [&[]; BATCH_SIZE];
            let mut local_wires_refs_arr: [&[F]; BATCH_SIZE] = [&[]; BATCH_SIZE];

            for (j, (&i, &x)) in indices_batch.iter().zip(xs_batch).enumerate() {
                ...
                if has_lookup {
                    ...
                    local_lookup_arr[j] = local_lookup_zs;
                    next_lookup_arr[j] = next_lookup_zs;
                }

                debug_assert_eq!(local_wires.len(), common_data.config.num_wires);
                debug_assert_eq!(local_zs.len(), num_challenges);

                local_constants_refs_arr[j] = local_constants;
                local_wires_refs_arr[j] = local_wires;

                shifted_xs_arr[j] = shifted_x;
                local_zs_arr[j] = local_zs;
                next_zs_arr[j] = next_zs;
                partial_products_arr[j] = partial_products;
                s_sigmas_arr[j] = s_sigmas;
            }

            // NB (JN): I'm not sure how (in)efficient the below is. It needs measuring.
            let mut local_constants_batch =
                vec![F::ZERO; batch_len * local_constants_refs_arr[0].len()];
            for i in 0..local_constants_refs_arr[0].len() {
                for (j, constants) in local_constants_refs_arr[..batch_len]
                    .iter()
                    .enumerate()
                {
            ...
            let mut local_wires_batch =
                vec![F::ZERO; batch_len * local_wires_refs_arr[0].len()];
            for i in 0..local_wires_refs_arr[0].len() {
                for (j, wires) in local_wires_refs_arr[..batch_len].iter().enumerate() {
                    local_wires_batch[i * batch_len + j] = wires[i];
                }
            }

            let lookup_len = if has_lookup { batch_len } else { 0 };
            ...
            let mut quotient_values_batch = eval_vanishing_poly_base_batch::<F, D>(
                common_data,
                indices_batch,
                &shifted_xs_arr[..batch_len],
                vars_batch,
                &local_zs_arr[..batch_len],
                &next_zs_arr[..batch_len],
                &local_lookup_arr[..lookup_len],
                &next_lookup_arr[..lookup_len],
                &partial_products_arr[..batch_len],
                &s_sigmas_arr[..batch_len],
                betas,
                gammas,
                deltas,
                alphas,
