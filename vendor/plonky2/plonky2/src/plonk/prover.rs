...
///
/// Schedule: blocked prefix-product. Per-point chunk prefixes and the full
/// product are independent of Z and form in parallel; an exclusive scan of the
/// full products recovers `Z(x)` at each point; each partial-product column is
/// then filled in parallel by scaling that point's prefixes with Z. Field mul
/// is associative, so the limbs match the classical left-to-right chain.
...
>(
    ...
    // 256 amortizes Montgomery inversion better than 128 over the 2^16 domain
    // × ~10 chunks without enlarging peak scratch beyond a few KiB per Rayon
    // worker (3 × 256 × num_chunks field elements).
