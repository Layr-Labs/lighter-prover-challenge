    ...
    C::InnerHasher: Hasher<F>,
{
    ...
    #[cfg(feature = "diagnostic_profile")]
    {
        ...
        count("degree_rows", common_data.degree());
        count("lde_rows", common_data.lde_size());
        count("wire_values", common_data.degree() * config.num_wires);
        count(
            "routed_wire_values",
            common_data.degree() * config.num_routed_wires,
        );
        count(
            "quotient_domain_points",
            common_data.degree() * common_data.quotient_degree_factor,
        );
    ...
    // `compute_quotient_polys` writes the IFFT result directly into these
    // degree-n allocations. The former split copied the entire q-factor-wide
    // quotient coefficient image once per challenge and proof.
...
>(
    ...
    challenge_columns
        ...
        .flat_map(|column| {
            ...
            // whole coefficient vector again afterwards. Write the normalized
            // coefficients directly into the degree-n quotient chunks instead
            // of copying the complete image in a later split pass.
