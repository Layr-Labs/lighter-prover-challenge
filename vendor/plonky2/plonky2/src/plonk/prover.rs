    ...
    for (gate_index, gate) in common_data.gates.iter().enumerate() {
        ...
        if let Some(u32_gate) = u32_gate {
            let (kind, num_ops, expected_wires, expected_constraints) = match u32_gate {
                ...
                // Two routed words then 32 big-endian bit wires per operation.
                ...
                // Four routed words then 64 big-endian bit wires per op.
        ...
        } else if let Some(random_access) =
            gate.0.as_any().downcast_ref::<RandomAccessGate<F, D>>()
        {
            // The shader folds the accessed list in a fixed-size thread-local
            // buffer, so lists longer than 64 stay on the CPU. The gate's
            // extra constants are gate-local constants, i.e. the columns
            // immediately after the selector prefix.
            let bits = random_access.bits;
            let num_extra_constants = random_access.num_extra_constants;
            if bits > 6 || num_extra_constants > 8 {
                None
            } else {
                let vec_size = 1usize << bits;
                let routed = random_access
                    .num_copies
                ...
                Some((
                    U32QuotientKind::RandomAccess {
                        bits,
                        num_extra_constants,
                        constant_column: common_data.selectors_info.num_selectors(),
                    },
                    random_access.num_copies,
                    routed.checked_add(random_access.num_copies.checked_mul(bits)?)?,
                    random_access
                        .num_copies
