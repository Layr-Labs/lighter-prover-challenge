...
>(
    ...
    for (gate_index, gate) in common_data.gates.iter().enumerate() {
        ...
        if let Some(u32_gate) = u32_gate {
            // Every random-access shape — six-bit (one 64-entry fold), four-bit
            // (four 16-entry folds) and three-bit (eight 8-entry folds) — is the
            // same witness-dependent ~64-select fold per thread. On the
            // five-worker ranked workload that data-dependent branch extends
            // the process-shared Range/U32 Metal command disproportionately.
            // Keep every random-access shape on the existing CPU
