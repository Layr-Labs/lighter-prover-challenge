...
>(
    ...
    let gpu_challenge_columns =
        if gpu_poseidon.is_some() || gpu_range.is_some() || gpu_permutation.is_some() {
            let denominator_inverses = (0..1 << quotient_degree_bits)
                .map(|i| z_h_on_coset.eval_inverse(i))
                .collect::<Vec<_>>();
            match crate::hash::poseidon2::metal::accumulate_quotient_columns(
                &quotient_values,
                &denominator_inverses,
                gpu_poseidon.as_ref().map(|(_, job)| job),
                gpu_range.as_ref().map(|(_, job)| job),
                gpu_permutation.as_ref(),
            ) {
                Ok(columns) => Some(columns),
                Err(error) => {
                    log::warn!(
                        "Metal quotient accumulator failed; recomputing quotient on CPU: {error}"
                    );
                    return compute_quotient_polys(
                        common_data,
                        prover_data,
