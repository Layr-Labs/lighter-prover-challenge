...
fn extend_and_combine_low_range_quotient<F: RichField>(
    ...
    let odd: Vec<Vec<F>> = (0..gates.len() * 2)
        ...
        .map(|t| {
            ...
            // SAFETY: every element is initialized by the architecture-specific
            // deinterleave or the portable fallback before `values` is read.
            ...
            if core::mem::size_of::<F>() == 8 && core::mem::align_of::<F>() >= 8 {
                unsafe {
                    ...
                    for _ in 0..chunks {
                        let first = vld2q_u64(src_ptr);
                        let second = vld2q_u64(src_ptr.add(4));
                        if c == 0 {
                            vst1q_u64(dst_ptr, first.0);
                            vst1q_u64(dst_ptr.add(2), second.0);
                        } else {
                            vst1q_u64(dst_ptr, first.1);
                            vst1q_u64(dst_ptr.add(2), second.1);
                        }
                        src_ptr = src_ptr.add(8);
                        dst_ptr = dst_ptr.add(4);
                    }
                    for i in 0..half_rows % 4 {
...
>(
    ...
    if has_gpu_values {
        ...
        #[cfg(all(feature = "std", target_arch = "aarch64", target_os = "macos"))]
        let poseidon_values = gpu_poseidon_values;
        #[cfg(all(feature = "std", target_arch = "aarch64", target_os = "macos"))]
        let range_values = gpu_range_values;
        #[cfg(all(feature = "std", target_arch = "aarch64", target_os = "macos"))]
        let range_low_values = gpu_range_low_values.as_deref();
        #[cfg(all(feature = "std", target_arch = "aarch64", target_os = "macos"))]
        let permutation_values = gpu_permutation_values;
        ...
        if num_challenges == 2 && column_ptrs.len() == 2 {
            ...
            quotient_values
                ...
                .for_each(|(chunk_index, chunk)| {
                    let base_point = chunk_index * MERGE_CHUNK_POINTS;
                    for (offset, cpu_pair) in chunk.chunks_exact(2).enumerate() {
                        let i = base_point + offset;
                        let denominator_inv = z_h_on_coset.eval_inverse(i);
                        let start = i * 2;
                        let mut value_0 = cpu_pair[0];
                        let mut value_1 = cpu_pair[1];
                        #[cfg(all(feature = "std", target_arch = "aarch64", target_os = "macos"))]
                        {
                            if let Some(values) = poseidon_values {
                                value_0 += values[start];
                                value_1 += values[start + 1];
                            }
                            if let Some(values) = range_values {
                                value_0 += values[start];
                                value_1 += values[start + 1];
                            }
                            if let Some(values) = range_low_values {
                                value_0 += values[start];
                                value_1 += values[start + 1];
                            }
                            if let Some(values) = permutation_values {
                                value_0 += values[start];
                                value_1 += values[start + 1];
                            }
                        }
                        ...
                        // SAFETY: point `i` belongs to this disjoint chunk and
                        // both challenge destinations are written exactly once.
                        unsafe {
                            *column_0.0.add(i) = value_0;
                            *column_1.0.add(i) = value_1;
                        }
                    }
                });
        } else {
                ...
                .for_each(|(chunk_index, chunk)| {
                    ...
                    for (offset, cpu_values) in chunk.chunks_exact(num_challenges).enumerate() {
                            ...
                            cpu_values.iter().zip(column_ptrs).enumerate()
                        {
                            ...
                            unsafe { *column.0.add(i) = value * denominator_inv };
                        }
                    }
                });
        }
