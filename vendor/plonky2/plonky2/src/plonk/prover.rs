...
>(
    ...
    let shifted_points =
        precomputed::shifted_two_adic_subgroup::<F>(
...
pub(crate) mod precomputed {
    ...
    mod imp {
        ...
        /// Cached `g * F::coset_shift()` over the whole two-adic subgroup of
        ...
        /// only on its size, so the full multiply-and-write traversal runs
