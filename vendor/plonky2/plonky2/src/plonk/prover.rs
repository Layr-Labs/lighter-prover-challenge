...
>(
    ...
    #[cfg(all(feature = "std", target_arch = "aarch64", target_os = "macos"))]
    {
        ...
        // path also evaluated and multiplied by the same `Z_H(x)^{-1}` once
        // per job; distributivity lets us sum the raw gate/permutation
        // contributions first and normalize once:
        //
        //   cpu + p/d + r/d + perm/d == cpu + (p + r + perm)/d.
        //
        // This preserves queue ordering, fallback behavior, and every field
        // value while deleting two full read/modify/write passes in the common
        // three-job case (16 MiB each at d16, 64 MiB each at d18 for the
        // two-challenge input/output traffic), plus two field multiplications
        // and repeated inverse-table lookups per quotient point.
        let gpu_values = [
            gpu_poseidon_values,
            gpu_range_values,
        ...
        quotient_values
            ...
            .for_each(|(i, cpu_values)| {
                ...
                for (challenge, cpu) in cpu_values.iter_mut().enumerate() {
                    let gpu_sum = gpu_values
                        .iter()
                        .flatten()
                        .fold(F::ZERO, |sum, values| sum + values[offset + challenge]);
