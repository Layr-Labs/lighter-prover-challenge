...
/// Goldilocks evaluates both challenge lanes through
/// `GoldilocksField::two_challenge_permutation_factors` (same
/// `wire + beta * point + gamma` association; two-wide interleaved mul on
/// AArch64). The 2G Z-polynomial FFTs stay independently scheduled.
#[inline(always)]
fn accumulate_two_challenge_permutation_factors<F: Field + Field64>(
    numerator_0: &mut F,
    numerator_1: &mut F,
    denominator_0: &mut F,
    denominator_1: &mut F,
    ...
    if core::mem::size_of::<F>() == core::mem::size_of::<GoldilocksField>()
        && F::ORDER == GoldilocksField::ORDER
    {
        // SAFETY: Goldilocks is the only PrimeField64 in this crate with this
        // ORDER and `repr(transparent)` u64 layout. The paired permutation
        // test compares raw limbs against the scalar per-challenge path.
        unsafe {
            let w = *(&wire_value as *const F).cast::<GoldilocksField>();
            let s = *(&sigma as *const F).cast::<GoldilocksField>();
            let xx = *(&x as *const F).cast::<GoldilocksField>();
            let (nf, df) = GoldilocksField::two_challenge_permutation_factors(
                w,
                s,
                xx,
                [
                    *(&beta_k_0 as *const F).cast::<GoldilocksField>(),
                    *(&beta_k_1 as *const F).cast::<GoldilocksField>(),
                ],
                [
                    *(&beta_0 as *const F).cast::<GoldilocksField>(),
                    *(&beta_1 as *const F).cast::<GoldilocksField>(),
                ],
                [
                    *(&gamma_0 as *const F).cast::<GoldilocksField>(),
                    *(&gamma_1 as *const F).cast::<GoldilocksField>(),
                ],
            );
            let mut nacc = [
                *(&*numerator_0 as *const F).cast::<GoldilocksField>(),
                *(&*numerator_1 as *const F).cast::<GoldilocksField>(),
            ];
            let mut dacc = [
                *(&*denominator_0 as *const F).cast::<GoldilocksField>(),
                *(&*denominator_1 as *const F).cast::<GoldilocksField>(),
            ];
            GoldilocksField::mul_assign_pair(&mut nacc, nf);
            GoldilocksField::mul_assign_pair(&mut dacc, df);
            *numerator_0 = *(&nacc[0] as *const GoldilocksField).cast::<F>();
            *numerator_1 = *(&nacc[1] as *const GoldilocksField).cast::<F>();
            *denominator_0 = *(&dacc[0] as *const GoldilocksField).cast::<F>();
            *denominator_1 = *(&dacc[1] as *const GoldilocksField).cast::<F>();
        }
    } else {
        *numerator_0 *= wire_value + beta_k_0 * x + gamma_0;
        *numerator_1 *= wire_value + beta_k_1 * x + gamma_1;
                    ...
                    for (t, &x) in xs.iter().enumerate() {
                        ...
                        for chunk in 0..num_chunks {
                            ...
                            for j in start..end {
                                ...
                                accumulate_two_challenge_permutation_factors(
                                    &mut numerator_0,
                                    &mut numerator_1,
                                    &mut denominator_0,
                                    &mut denominator_1,
