                ...
                |scratch, (chunk_idx, ((products_0, products_1), xs))| {
                    ...
                    for (t, &x) in xs.iter().enumerate() {
                        ...
                        for chunk in 0..num_chunks {
                            ...
                            for j in start..end {
                                // Sole routed member of its copy component: sigma = id,
                                // so N = D for both challenges and the factors cancel.
...
>(
    ...
    // per-challenge columns. Range contributions (when present) are applied
    // here rather than as a separate full-buffer RMW, preserving Poseidon
    // merge / Range-kernel overlap and the former raw-field op order.
