...
mod quotient_layout_tests {
    ...
    /// Pins this archive to the default two-column scatter seam while keeping
    /// the assertion entirely outside production builds.
    #[test]
    fn direct_challenge_columns_archive_identity_regression() {
        let point_major = [
        ...
        let mut challenge_0 = [MaybeUninit::uninit(); 3];
        let mut challenge_1 = [MaybeUninit::uninit(); 3];
        ...
        for i in 0..3 {
            // SAFETY: the scatter helper writes every cell in both columns.
            let out_0 = unsafe { challenge_0[i].assume_init() };
            // SAFETY: the scatter helper writes every cell in both columns.
            let out_1 = unsafe { challenge_1[i].assume_init() };
            assert_eq!(out_0.0, point_major[2 * i].0);
            assert_eq!(out_1.0, point_major[2 * i + 1].0);
        }
        assert!(
            !FORCE_INTERLEAVED_QUOTIENT_LAYOUT.with(core::cell::Cell::get)
        );
    }
