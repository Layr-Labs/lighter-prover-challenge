...
>(
    ...
    for (gate_index, gate) in common_data.gates.iter().enumerate() {
        ...
        if ra_gate.is_some() && (range.is_some() || u32_gate.is_some()) {
            if gpu_poseidon_quotient_diagnostics_enabled() {
                eprintln!(
                    "[gpu-range-quotient] gate {gate_index} advertised conflicting RandomAccess layout"
                );
            }
            return None;
        }
        ...
        if let Some(ra_gate) = ra_gate {
            if ra_gate.bits > 6 || ra_gate.num_copies == 0 {
                if gpu_poseidon_quotient_diagnostics_enabled() {
                    eprintln!(
                        "[gpu-range-quotient] invalid RandomAccess metadata: {ra_gate:?}"
                    );
                }
                return None;
            }
            let routed_per_copy = 2usize.checked_add(1usize << ra_gate.bits)?;
            let num_routed_wires = routed_per_copy
                .checked_mul(ra_gate.num_copies)?
                .checked_add(ra_gate.num_extra_constants)?;
            let expected_wires = num_routed_wires
                .checked_add(ra_gate.num_copies.checked_mul(ra_gate.bits)?)?;
            let expected_constraints = ra_gate
                .num_copies
                .checked_mul(ra_gate.bits.checked_add(2)?)?
                .checked_add(ra_gate.num_extra_constants)?;
            if gate.0.num_wires() != expected_wires
                || gate.0.num_constraints() != expected_constraints
            {
                if gpu_poseidon_quotient_diagnostics_enabled() {
                    eprintln!(
                        "[gpu-range-quotient] RandomAccess layout mismatch gate={gate_index} \
    ...
    if specs.is_empty() && u32_specs.is_empty() && ra_specs.is_empty() {
        if gpu_poseidon_quotient_diagnostics_enabled() {
            eprintln!("[gpu-range-quotient] no advertised RangeCheck/U32/RandomAccess gates");
        }
        return None;
    }
