...
>(
    ...
    for (gate_index, gate) in common_data.gates.iter().enumerate() {
            ...
            gate.0.as_any().downcast_ref::<ExponentiationGate<F, D>>()
        {
            ...
            // constant 0, i.e. the column immediately after the full selector
            ...
            gate.0.as_any().downcast_ref::<ReducingExtensionGate<D>>()
        {
            ...
            // The base arithmetic/addition family reads its gate-local
            // constants starting immediately after the full selector prefix,
            // exactly like Equality's "one" value above.
            if raw_constant_base.checked_add(2)? > common_data.num_constants {
                None
            } else {
                Some((
        ...
        } else if let Some(arithmetic) =
            gate.0.as_any().downcast_ref::<ArithmeticExtensionGate<D>>()
        {
            if D != 2 || raw_constant_base.checked_add(2)? > common_data.num_constants {
                None
            } else {
                Some((
                    U32QuotientKind::ArithmeticExtension {
                        constant_column: raw_constant_base,
                    },
                    arithmetic.num_ops,
        ...
        {
            if D != 2 || raw_constant_base.checked_add(1)? > common_data.num_constants {
                None
            } else {
                Some((
        ...
        } else if let Some(addition) = gate.0.as_any().downcast_ref::<AdditionGate>() {
            if raw_constant_base.checked_add(2)? > common_data.num_constants {
                None
            } else {
                Some((
                    ...
                    addition.num_ops,
                    addition.num_ops.checked_mul(3)?,
                    addition.num_ops,
                ))
            }
