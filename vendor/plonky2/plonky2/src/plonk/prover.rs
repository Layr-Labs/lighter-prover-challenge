                    ...
                    for (t, &x) in xs.iter().enumerate() {
                        ...
                        for chunk in 0..num_chunks {
                            ...
                            for j in start..end {
                                ...
                                // `numerator_0`/`denominator_0` (and the `_1` pair) share the
                                // same `wire_value + gamma` addend; the two field-equal ways of
                                // writing the rest (`beta_k_is[j] * x` vs `beta * sigma`) are
                                // each one `multiply_accumulate` (self + x*y) instead of a
                                // separate mul and two adds. Raw-representative differences are
                                // harmless here: these values only ever feed further field `*=`
                                // and the eventual polynomial-value serialization canonicalizes.
