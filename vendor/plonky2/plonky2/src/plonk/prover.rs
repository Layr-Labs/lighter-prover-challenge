...
mod l_0_table_cache {
    ...
    fn build<F: Field>(degree_bits: usize, quotient_degree_bits: usize) -> Vec<F> {
        ...
        let shift = F::coset_shift();
        // Every entry is the inverse of an independent product, so the table
        // needs one inversion per chunk rather than one per entry: Montgomery's
        // trick trades `len - 1` inversions for three multiplies each. The
        // products formed here are the same products the per-entry path formed
        // and batch inversion is exact, so every entry stays bit-identical to
        // the value it replaces.
        let mut table: Vec<F> = F::two_adic_subgroup(degree_bits + quotient_degree_bits)
            .into_par_iter()
            .map(|x| n * (shift * x - F::ONE))
            .collect();
        // Chunked so the scratch stays bounded and the chunks still run in
        // parallel; each worker reuses one buffer across its chunks.
        const INVERSE_CHUNK: usize = 4096;
        table
            .par_chunks_mut(INVERSE_CHUNK)
            .for_each_init(Vec::new, |buf, chunk| {
                F::batch_multiplicative_inverse_into(chunk, buf);
