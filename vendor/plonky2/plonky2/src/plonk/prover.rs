...
fn accumulate_z_columns_block_parallel<F: Field>(
    ...
    blocks
        ...
        .for_each(|(block_index, ((columns, _), &base))| {
            if block_index == 0 {
                // This block already has the true base Z=1. Besides avoiding needless work,
...
/// Block-prefix scan that writes each block directly into its final column ranges. The first
/// parallel pass owns disjoint `MaybeUninit` slices of every output column, so it needs neither
/// per-block field buffers nor a concatenation copy. A short serial scan turns block totals into
/// bases, and a second parallel pass scales the already-in-place ranges.
fn accumulate_z_columns_block_parallel_direct<F: Field>(
    all_quotient_chunk_products: &[F],
    num_chunks: usize,
    block_points: usize,
) -> Vec<Vec<F>> {
    ...
    let n_points = all_quotient_chunk_products.len() / num_chunks;
    let num_blocks = n_points.div_ceil(block_points);
    let products_per_block = block_points * num_chunks;
    let mut columns: Vec<Vec<F>> = (0..num_chunks)
        .map(|_| Vec::with_capacity(n_points))
        .collect();
    let mut block_bases = vec![F::ONE; num_blocks];

    {
        // Carve every final column allocation into block-sized, non-overlapping slices. This is
        // the same initialized-spare-capacity pattern used by `MatrixWitness::full_witness`.
        let mut segments: Vec<Vec<&mut [core::mem::MaybeUninit<F>]>> = (0..num_blocks)
            .map(|_| Vec::with_capacity(num_chunks))
            .collect();
        for column in columns.iter_mut() {
            let mut rest = crate::hash::merkle_tree::capacity_up_to_mut(column, n_points);
            for block_columns in segments.iter_mut() {
                let take = block_points.min(rest.len());
                let (head, tail) = rest.split_at_mut(take);
                block_columns.push(head);
                rest = tail;
            }
        }
        ...
        segments
            .par_iter_mut()
            .zip(block_bases.par_iter_mut())
            .zip(all_quotient_chunk_products.par_chunks(products_per_block))
            .for_each(|((block_columns, block_total), block_products)| {
                debug_assert_eq!(block_columns.len(), num_chunks);
                debug_assert_eq!(
                    block_products.len() / num_chunks,
                    block_columns[0].len()
                );
                let mut z_x = F::ONE;
                for (row, quotient_chunk_products) in
                    block_products.chunks_exact(num_chunks).enumerate()
                {
                    let mut acc = z_x;
                    for (k, (&quotient_chunk_product, column)) in quotient_chunk_products
                        .iter()
                        .zip(block_columns.iter_mut())
                        .enumerate()
                    {
                        acc *= quotient_chunk_product;
                        column[row].write(if k + 1 == num_chunks { z_x } else { acc });
                    }
                    z_x = acc;
                }
                *block_total = z_x;
    ...
    for column in columns.iter_mut() {
        // SAFETY: the indexed parallel traversal above pairs each input block with the matching
        // disjoint segment in every column and writes each of its rows exactly once.
        unsafe { column.set_len(n_points) };
    }

    let mut z_x = F::ONE;
    for block_total in block_bases.iter_mut() {
        let next_z = z_x * *block_total;
        *block_total = z_x;
        z_x = next_z;
    }

    // A single list of borrowed ranges avoids recreating the old per-block field allocations.
    // Block zero already has base one, so omit it and preserve its exact raw words.
    let mut scale_segments = Vec::with_capacity(num_chunks * num_blocks.saturating_sub(1));
    for column in columns.iter_mut() {
        let mut rest = column.as_mut_slice();
        for (block_index, &base) in block_bases.iter().enumerate() {
            let take = block_points.min(rest.len());
            let (head, tail) = rest.split_at_mut(take);
            if block_index != 0 {
                scale_segments.push((head, base));
            }
            rest = tail;
        }
    }
    scale_segments.par_iter_mut().for_each(|(values, base)| {
...
fn accumulate_z_columns<F: Field>(
    ...
    #[cfg(feature = "parallel")]
    {
        ...
        {
            // Keep enough blocks to fill the ranked 12-core pool at d14, while using larger
            // contiguous writes for the less frequent d16/d18 scans.
            let block_points = if n_points <= (1 << 14) {
                1024
            } else if n_points <= (1 << 16) {
                2048
            } else {
                4096
            };
            return accumulate_z_columns_block_parallel_direct(
...
mod flat_chunk_products_tests {
    ...
    use super::{
        ...
        accumulate_z_columns_block_parallel_direct, accumulate_z_columns_serial,
        divide_chunk_products,
    };
    ...
    fn block_parallel_z_columns_match_serial_canonically() {
        ...
        for &n_points in &[1usize, 127, 2048, 2049, 4097, 1 << 14] {
            ...
            let direct =
                accumulate_z_columns_block_parallel_direct(&products, NUM_CHUNKS, BLOCK_POINTS);

            for (direct_column, actual_column) in direct.iter().zip(&actual) {
                assert_eq!(raw(direct_column), raw(actual_column));
            }
