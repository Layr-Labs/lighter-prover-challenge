...
>(
    ...
    #[cfg(all(feature = "std", target_arch = "aarch64", target_os = "macos"))]
    if gpu_poseidon_values.is_some()
        || gpu_range_values.is_some()
        || gpu_permutation_values.is_some()
    {
        // Preserve the point-major destination through GPU accumulation. The
        // direct-to-IFFT scatter fusion scored 29.9992646875433 on a normal
        // validator, below the 30.3906741982145 lineage: its two destination
        // streams and per-element branches outweighed the removed pass.
        // Ranked redraw: 50cece6f verified 5/5 proofs in the recurring slow
        // validator population; this comment changes no executable behavior.
        if let (2, Some(poseidon), Some(range), Some(permutation)) = (
            num_challenges,
            gpu_poseidon_values,
            gpu_range_values,
            gpu_permutation_values,
        ) {
            // Every production circuit has two challenges and all three Metal
            // producers. Match that stable shape once, outside the hot row
            // loop, so each point avoids three Option branches, three slice
            // bounds, and the generic inner iterators. Keep the additions in
            // producer order for limb-identical quotient coefficients.
            // Ranked draw 2ccb374d verified 5/5 in the slow-host population;
            // this audit comment preserves identical executable behavior.
            quotient_values
                .par_chunks_exact_mut(2)
                .zip(poseidon.par_chunks_exact(2))
                .zip(range.par_chunks_exact(2))
                .zip(permutation.par_chunks_exact(2))
                .enumerate()
                .for_each(|(i, (((cpu, poseidon), range), permutation))| {
                    let denominator_inv = z_h_on_coset.eval_inverse(i);
                    cpu[0] += poseidon[0] * denominator_inv;
                    cpu[0] += range[0] * denominator_inv;
                    cpu[0] += permutation[0] * denominator_inv;
                    cpu[1] += poseidon[1] * denominator_inv;
                    cpu[1] += range[1] * denominator_inv;
                    cpu[1] += permutation[1] * denominator_inv;
                });
        } else {
            quotient_values
                ...
                .for_each(|(i, cpu_values)| {
                    ...
                    if let Some(values) = gpu_poseidon_values {
                        for (cpu, &gpu) in cpu_values
                            .iter_mut()
                            .zip(&values[start..start + num_challenges])
                        {
    ...
    // One closure covers 4 KiB of point-major input in the production
    // two-challenge case. This keeps both 2 KiB destination streams hot while
    // reducing the indexed Rayon item count 8x versus the general evaluator's
    // 32-point batch. It changes neither worker count nor access order.
    // Ranked draw 458b5cfd verified 5/5 in the slow-host population; this
    // audit comment preserves identical executable behavior for one redraw.
    const QUOTIENT_SCATTER_POINTS: usize = BATCH_SIZE * 8;
    if num_challenges == 2 {
        // Keep the transpose as its own streaming pass, but specialize the
        // production two-column shape outside the hot loop. This avoids the
        // dynamic inner zip and challenge-sized chunk arithmetic without
        // mixing the transpose stores back into the multi-producer quotient
        // accumulation pass (which regressed on a normal ranked host).
        // Ranked draw 5efb0533 verified 5/5 in the slow-host population;
        // this audit comment preserves identical executable behavior.
        ...
        quotient_values
            ...
            .for_each(|(chunk_i, chunk)| {
                ...
                for (k, point_values) in chunk.chunks_exact(2).enumerate() {
                    ...
                    // and each destination column owns a separate allocation.
                    unsafe {
