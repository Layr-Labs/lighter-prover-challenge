...
>(
    ...
    #[cfg(all(feature = "std", target_arch = "aarch64", target_os = "macos"))]
    {
        ...
        // Consume and recycle the range output before waiting for permutation.
        // Keeping both shared Metal outputs live saved one CPU read/modify/write
        // pass, but extended a quotient-sized allocation across the next queue
        // wait. On the 48 GiB ranked host that extra unified-memory lifetime is
        // more expensive than the contiguous pass it removes.
