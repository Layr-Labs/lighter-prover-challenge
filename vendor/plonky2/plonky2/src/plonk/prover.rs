    ...
    C::InnerHasher: Hasher<F>,
{
    ...
    let (wires_coeffs, ready_ldes) = timed!(
        ...
        {
            // Marker: skip-ifft-1786703000. v2 always IFFT'd the finished
            // witness and reused LDE only on coefficient equality, because
            // v1 (`8020574`) compared preimages and reused a *wrong LDE
            // body*. v2's `lde_one_column` is the production `lde_values`
            // body and official-verified at 30.379. Preimage equality now
            // implies a unique IFFT and that LDE, so the tail `2^18` IFFT
            // of a matching column is dead work. Mismatch (light feed,
            // lookup padding) falls through to the production IFFT.
            // Fail-closed: a wrong reuse is a Merkle-cap / FRI mismatch.
            let mut ready_ldes: Vec<Option<Vec<F>>> = vec![None; num_wires];
            let mut coeffs: Vec<Option<PolynomialCoeffs<F>>> = vec![None; num_wires];
            let mut skip = vec![false; num_wires];
            if let Some(early) = early_wire_lde.as_mut() {
                for j in 0..num_wires {
                    let pre_ok = matches!(
                        (
                            early.preimages.get(j).and_then(|p| p.as_ref()),
                            witness.wire_values.get(j)
                        ),
                        (Some(pre), Some(now)) if pre.as_slice() == now.as_slice()
                    );
                    if pre_ok {
                        if let (Some(saved), Some(lde)) =
                            (early.coeffs[j].take(), early.ldes[j].take())
                        {
                            skip[j] = true;
                            if j >= num_routed_wires {
                                let _ = core::mem::take(&mut witness.wire_values[j]);
                            }
                            ready_ldes[j] = Some(lde);
                            coeffs[j] = Some(saved);
                        }
                    }
                }
            }
            let computed: Vec<(usize, PolynomialCoeffs<F>)> = witness
                .wire_values
                .par_iter_mut()
                .enumerate()
                .filter(|(j, _)| !skip[*j])
                .map(|(j, column)| {
                    let poly = if j < num_routed_wires {
                ...
                .collect();
            for (j, poly) in computed {
                coeffs[j] = Some(poly);
            }
            let coeffs: Vec<PolynomialCoeffs<F>> = coeffs
                .into_iter()
                .map(|c| c.expect("every wire column is either skipped or IFFT'd"))
                .collect();
            (coeffs, ready_ldes)
        }
    );
