...
>(
    ...
    quotient_values
        ...
        .for_each_init(
            ...
            |scratch, (batch_i, (quotient_values_batch, xs_batch))| {
                ...
                // Leave the CPU constraint sums as quotient numerators. The
                // final point-major -> challenge-column scatter combines them
                // with any Metal numerators and applies the shared Z_H inverse
                // once to the sum, deleting this extra full-domain scale pass.
            },
        );
    ...
    // Keep the active Metal sources in fixed-size arrays selected once per
    // proof. Ranked circuits use different gate subsets, so building a
    // dynamic `Vec<&[F]>` here left a variable-length loop in the innermost
    // point/challenge scatter. These eight combinations let LLVM see the
    // exact source count while retaining CPU -> Poseidon -> range ->
    // permutation numerator accumulation order. Since every source shares the
    // same Z_H denominator, the scatter applies that inverse once after the
    // sum rather than once per CPU/GPU source.
    macro_rules! scatter_with_gpu_sources {
        ($sources:expr) => {{
            let gpu_quotient_sources = $sources;
            if num_challenges == 2 {
                // Ranked circuits have two challenges. Handle the pair as one
                // point so source iteration, index arithmetic, column lookup,
                // and loop control are paid once rather than once per lane.
                // The paired independent field operations also expose the
                // exact shape to LLVM's straight-line optimizer.
                quotient_values
                    ...
                    .for_each(|(chunk_i, chunk)| {
                        ...
                        for (k, point_values) in chunk.chunks_exact(2).enumerate() {
                            let point_i = base + k;
                            let value_i = point_i * 2;
                            let mut value_0 = point_values[0];
                            let mut value_1 = point_values[1];
                            for gpu_values in &gpu_quotient_sources {
                                value_0 += gpu_values[value_i];
                                value_1 += gpu_values[value_i + 1];
                            }
                            let denominator_inv = z_h_on_coset.eval_inverse(point_i);
                            value_0 *= denominator_inv;
                            value_1 *= denominator_inv;
                            // SAFETY: each chunk owns a disjoint point range,
                            // and this branch is guarded by exactly two columns.
                            unsafe {
                                *column_ptrs[0].0.add(point_i) = value_0;
                                *column_ptrs[1].0.add(point_i) = value_1;
                            }
                        }
                    });
            } else {
    ...
    match (
        ...
        (None, None, None) => scatter_with_gpu_sources!([] as [&[F]; 0]),
        (Some(poseidon), None, None) => scatter_with_gpu_sources!([poseidon]),
        (None, Some(range), None) => scatter_with_gpu_sources!([range]),
        (None, None, Some(permutation)) => scatter_with_gpu_sources!([permutation]),
        (Some(poseidon), Some(range), None) => {
            scatter_with_gpu_sources!([poseidon, range])
        }
        (Some(poseidon), None, Some(permutation)) => {
            scatter_with_gpu_sources!([poseidon, permutation])
        }
        (None, Some(range), Some(permutation)) => {
            scatter_with_gpu_sources!([range, permutation])
        }
        (Some(poseidon), Some(range), Some(permutation)) => {
            scatter_with_gpu_sources!([poseidon, range, permutation])
        }
    }

    #[cfg(not(all(feature = "std", target_arch = "aarch64", target_os = "macos")))]
    scatter_with_gpu_sources!([] as [&[F]; 0]);
