...
use crate::util::partial_products::{
    partial_products_and_zs_from_chunk_rows,
    quotient_chunk_products_80_by_8_from_numerators_and_inverses,
    quotient_chunk_products_from_numerators_and_inverses, use_fixed_permutation_chunk_products,
};
...
struct PermutationRow<'a, F: Field> {
    witness: &'a MatrixWitness<F>,
    row: usize,
    x: F,
    beta: F,
    gamma: F,
...
impl<F: Field> PermutationRow<'_, F> {
    fn numerators(&self) -> impl ExactSizeIterator<Item = F> + '_ {
        (0..self.k_is.len()).map(|j| {
            let wire_value = self.witness.get_wire(self.row, j);
            let s_id = self.k_is[j] * self.x;
            wire_value + self.beta * s_id + self.gamma
        })
    }

    fn fill_denominator_inverses(
        &self,
        denominators: &mut Vec<F>,
        denominator_inverses: &mut Vec<F>,
    ) {
        debug_assert_eq!(self.s_sigmas.len(), self.k_is.len());
        denominators.clear();
        denominators.extend((0..self.k_is.len()).map(|j| {
            let wire_value = self.witness.get_wire(self.row, j);
            wire_value + self.beta * self.s_sigmas[j] + self.gamma
        }));
        F::batch_multiplicative_inverse_into(denominators, denominator_inverses);
        debug_assert_eq!(denominator_inverses.len(), self.k_is.len());
    }
}
...
>(
    ...
    if use_fixed_permutation_chunk_products(num_routed_wires, degree, num_prods + 1) {
            ...
            .map_init(
                ...
                |(denominators, denominator_inverses), (i, &x)| {
                    let row = PermutationRow {
                        ...
                        gamma,
                        k_is,
                        s_sigmas: &prover_data.sigmas[i],
                    };
                    row.fill_denominator_inverses(denominators, denominator_inverses);
                    quotient_chunk_products_80_by_8_from_numerators_and_inverses(
                        row.numerators(),
        ...
        return partial_products_and_zs_from_chunk_rows(&all_quotient_chunk_products, num_prods)
            .into_iter()
