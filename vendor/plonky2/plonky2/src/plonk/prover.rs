...
>(
    ...
    // is written exactly once. Avoids both the single-threaded walk and the
    // alternative of `num_challenges` full stride-reads of the point-major buffer.
        ...
        .collect();
    {
        ...
        // SAFETY: each parallel iteration writes a distinct point index `i` in
        // every column; no two iterations share a destination cell.
        ...
        (0..n_points).into_par_iter().for_each(|i| {
            let base = i * w;
            for c in 0..w {
                // SAFETY: `i < n_points`, `c < w`, columns are length `n_points`,
                // and each (c, i) is written by exactly one iteration.
                unsafe {
