...
>(
    ...
    let num_routed_wires = common_data.config.num_routed_wires;
    // Chunk products per point: one per `degree`-sized chunk of the routed wires, the last of
    // which is associated with Z(gx).
    ...
    // One flat chunk-products buffer for the whole subgroup, with point `i`'s products at
    // `[i * num_chunks..(i + 1) * num_chunks]`. The batches write disjoint aligned slices of
    // it, deleting the per-point `quotient_chunk_products` Vec and the Vec<Vec<F>> that
    // collected them; values and their order are unchanged.
    let mut all_quotient_chunk_products = vec![F::ZERO; subgroup.len() * num_chunks];
    subgroup
        .par_chunks(INV_BATCH)
        .zip(all_quotient_chunk_products.par_chunks_mut(INV_BATCH * num_chunks))
        .enumerate()
        .for_each_init(
            // Denominator and numerator scratch buffers per worker thread. The numerators are
            // gathered in the same pass as the denominators, so each wire is read once per
            // point instead of once per loop; the numerator expression and the order of the
            // in-place multiplies below are exactly those of the two-pass version.
            || {
                (
            ...
            |(denominators, numerators), (chunk_idx, (xs, out_chunk))| {
                let base = chunk_idx * INV_BATCH;
                let denoms = &mut denominators[..xs.len() * num_routed_wires];
                let numers = &mut numerators[..xs.len() * num_routed_wires];
                for (t, &x) in xs.iter().enumerate() {
                    ...
                    for j in 0..num_routed_wires {
                        ...
                        numers[t * num_routed_wires + j] = wire_value + beta_k_is[j] * x + gamma;
                    }
                }
                ...
                for t in 0..xs.len() {
                    ...
                    for (quotient_value, &numerator) in point_quotients
                        .iter_mut()
                        .zip(&numers[t * num_routed_wires..(t + 1) * num_routed_wires])
                    {
