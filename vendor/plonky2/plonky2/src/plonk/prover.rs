...
>(
    ...
    macro_rules! merge_gpu_values {
        ($gpu_values:ident) => {{
            ...
            if num_challenges == 2 {
                // Keep the point arithmetic and the three producer completion
                // boundaries unchanged, but amortize Rayon splitting over a
                // contiguous tile. These merges stream tens of MiB and do only
                // two multiply-adds per point, so per-point parallel iterator
                // plumbing is otherwise a material part of the consumer cost.
                const QUOTIENT_MERGE_POINTS: usize = 1 << 11;
                quotient_values
                    .par_chunks_mut(QUOTIENT_MERGE_POINTS * 2)
                    .zip($gpu_values.par_chunks(QUOTIENT_MERGE_POINTS * 2))
                    .enumerate()
                    .for_each(|(chunk_i, (cpu_chunk, gpu_chunk))| {
                        let base = chunk_i * QUOTIENT_MERGE_POINTS;
                        for (k, (cpu_values, gpu_values)) in cpu_chunk
                            .chunks_exact_mut(2)
                            .zip(gpu_chunk.chunks_exact(2))
                            .enumerate()
                        {
