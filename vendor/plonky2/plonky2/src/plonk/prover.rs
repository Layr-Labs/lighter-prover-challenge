...
>(
    ...
    if lookup {
        // Marker: lookup-chal-par-1786711000. On the perm-inv-merge 30.560
        // keeper. The two challenge walks are independent (separate
        // `deltas` slices, separate output vecs, read-only witness).
        // Production `num_challenges == 2`; indexed `into_par_iter`
        // preserves challenge order. The serial tail's rayon pool is
        // otherwise idle here (`ParallelWitnessGuard` already claimed
        // it). Wrong order or a torn write is a wrong lookup Z —
        // fail-closed.
