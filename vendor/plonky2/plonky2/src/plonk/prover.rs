...
const DEFAULT_QUOTIENT_BATCH_SIZE: usize = 64;

/// The quotient evaluator processes independent points in column-major batches.
/// Larger batches amortize Rayon scheduling and the fixed per-column gathers.
/// Keep a same-binary control for end-to-end validation; unsupported values
/// deliberately select the production default.
#[inline]
fn quotient_batch_size() -> usize {
    #[cfg(feature = "std")]
    if let Ok(value) = std::env::var("PLONKY2_QUOTIENT_BATCH_SIZE") {
        if let Ok(value) = value.parse::<usize>() {
            if matches!(value, 32 | 64 | 128) {
                return value;
            }
        }
    }
    DEFAULT_QUOTIENT_BATCH_SIZE
}
...
>(
    ...
    let batch_size = quotient_batch_size();
