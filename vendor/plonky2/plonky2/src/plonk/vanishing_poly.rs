...
pub(crate) struct VanishingScratch<F> {
    ...
    pub gate_constraint_values: Vec<F>,
...
pub(crate) fn eval_vanishing_poly_base_batch<F: RichField + Extendable<D>, const D: usize>(
    ...
    evaluate_gate_constraints_base_batch_into_with_scratch::<F, D>(
        ...
        &mut scratch.gate_constraint_values,
    );
...
/// Hot-path form retaining result, selector-filter, and typed gate-constraint allocations per
/// Rayon worker across every quotient batch assigned to it.
