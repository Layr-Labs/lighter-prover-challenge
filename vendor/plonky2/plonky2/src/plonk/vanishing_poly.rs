...
pub(crate) fn eval_vanishing_poly_base_batch<F: RichField + Extendable<D>, const D: usize>(
    ...
    } = perm
    {
        ...
        if num_challenges == 2 {
            ...
            // once. Each challenge's product retains its exact j-ascending
            // factor order.
            //
            // Each factor is `wire + beta * point + gamma`. Fold `wire` into
            // the product's reduction with `multiply_accumulate`; Goldilocks'
            // AArch64 implementation does this in one eleven-instruction
            // reduction instead of materializing the product and then running
            // a separate field addition. The trait default is the original
            // expression, so other fields keep their established arithmetic.
            let beta_0 = betas[0];
            let beta_1 = betas[1];
            ...
            for c in 0..num_chunks {
                ...
                den_prod_second.clear();
                {
                    ...
                    for k in 0..n {
                        ...
                        let x = xs_batch[k];
                        num_prod.push(wire.multiply_accumulate(beta_k_0, x) + gamma_0);
                        den_prod.push(wire.multiply_accumulate(beta_0, sigma) + gamma_0);
                        num_prod_second.push(wire.multiply_accumulate(beta_k_1, x) + gamma_1);
                        den_prod_second.push(wire.multiply_accumulate(beta_1, sigma) + gamma_1);
                    }
                }
                for j in j_start + 1..j_end {
                    ...
                    for k in 0..n {
                        ...
                        let sigma = sigma_col[k];
                        let x = xs_batch[k];
                        num_prod[k] *= wire.multiply_accumulate(beta_k_0, x) + gamma_0;
                        den_prod[k] *= wire.multiply_accumulate(beta_0, sigma) + gamma_0;
                        num_prod_second[k] *= wire.multiply_accumulate(beta_k_1, x) + gamma_1;
                        den_prod_second[k] *= wire.multiply_accumulate(beta_1, sigma) + gamma_1;
                    }
                }
            ...
            for i in 0..num_challenges {
                for c in 0..num_chunks {
                    ...
                    den_prod.clear();
                    {
                        ...
                        for k in 0..n {
                            num_prod.push(
                                wire_col[k].multiply_accumulate(beta_k_i, xs_batch[k]) + gamma,
                            );
                            den_prod
                                .push(wire_col[k].multiply_accumulate(beta, sigma_col[k]) + gamma);
                        }
                    }
                    for j in j_start + 1..j_end {
                        let wire_col = &wires[j * n..][..n];
                        ...
                        for k in 0..n {
                            num_prod[k] *=
                                wire_col[k].multiply_accumulate(beta_k_i, xs_batch[k]) + gamma;
                            den_prod[k] *=
                                wire_col[k].multiply_accumulate(beta, sigma_col[k]) + gamma;
                        }
                    }
...
mod tests {
    ...
    /// The column-major permutation path folds the wire addend into each
    /// numerator/denominator multiplication. Pin field equality across both
    /// adversarial noncanonical limbs and pseudorandom inputs. Raw limbs need
    /// not agree (Goldilocks permits two representations for small residues),
    /// but every later prover operation and verifier comparison is over field
    /// values.
    #[test]
    fn permutation_factor_fma_matches_expression() {
        type F = GoldilocksField;

        let check = |wire: F, beta: F, point: F| {
            let expected = wire + beta * point;
            let actual = wire.multiply_accumulate(beta, point);
            assert_eq!(actual, expected);
        };

        let order = 0xffff_ffff_0000_0001u64;
        let edge = [0, 1, 2, order - 1, order, order + 1, u64::MAX];
        for &wire in &edge {
            for &beta in &edge {
                for &point in &edge {
                    check(
                        GoldilocksField(wire),
                        GoldilocksField(beta),
                        GoldilocksField(point),
                    );
                }
            }
        }
        ...
        for _ in 0..100_000 {
            state ^= state << 13;
            let wire = GoldilocksField(state);
            state ^= state >> 7;
            let beta = GoldilocksField(state);
            state ^= state << 17;
            let point = GoldilocksField(state);
            check(wire, beta, point);
        }
    }
