...
pub(crate) struct VanishingScratch<F> {
    ...
    /// Field lanes for the fused interleave/uninterleave evaluator. Every lane
    /// is overwritten before it is read, so retaining the allocation between
    /// quotient batches avoids both heap churn and redundant zero fills.
    pub interleave_scratch: Vec<F>,
    /// Wide delayed-reduction lanes paired with `interleave_scratch`.
    pub interleave_wide_scratch: Vec<u128>,
}
...
fn eval_interleave_pair_dense_fused<F: PrimeField64>(
    ...
    summed_filter: &[F],
    combined: &mut [F],
    field_scratch: &mut Vec<F>,
    wide_scratch: &mut Vec<u128>,
) {
    ...
    let required = FIELD_COLS * batch_size;
    if field_scratch.len() != required {
        field_scratch.resize(required, F::ZERO);
    }
    let (x_accumulators, rest) =
        field_scratch.split_at_mut(INTERLEAVE_OPS * batch_size);
    ...
    if wide_scratch.len() != wide_required {
        wide_scratch.resize(wide_required, 0);
    }
    let (wide_x, rest) = wide_scratch.split_at_mut(batch_size);
...
pub(crate) fn eval_vanishing_poly_base_batch<F: RichField + Extendable<D>, const D: usize>(
    ...
    evaluate_gate_constraints_base_batch_into_cpu_gates::<F, D>(
        ...
        interleave_pair,
        &mut scratch.interleave_scratch,
        &mut scratch.interleave_wide_scratch,
    );
...
>(
    ...
    let mut interleave_scratch = Vec::new();
    let mut interleave_wide_scratch = Vec::new();
    evaluate_gate_constraints_base_batch_into_cpu_gates(
        ...
        num_constraint_rows,
        None,
        &mut interleave_scratch,
        &mut interleave_wide_scratch,
    );
}
...
>(
    ...
    num_constraint_rows: usize,
    interleave_pair: Option<&InterleavePairPlan>,
    interleave_scratch: &mut Vec<F>,
    interleave_wide_scratch: &mut Vec<u128>,
) {
    debug_assert!(num_constraint_rows <= common_data.num_gate_constraints);
...
mod tests {
    ...
    fn delayed_interleave_pair_accumulators_match_termwise_reduction_in_raw_limbs() {
        ...
        // 64 is the production quotient batch width; smaller and larger sizes
        // exercise tails and prove one reused allocation handles every shape.
        for batch_size in [128usize, 64, 32, 1, 7, 33] {
            let mut field_scratch = Vec::new();
            let mut wide_scratch = Vec::new();
    ...
    fn packed_dense_interleave_pair_matches_independent_alpha_reduction_canonically() {
        ...
        for batch_size in [1usize, 7, 32, 64, 128] {
            let mut field_scratch = Vec::new();
            let mut wide_scratch = Vec::new();
