...
pub(crate) fn eval_vanishing_poly_base_batch<F: RichField + Extendable<D>, const D: usize>(
    ...
    } = perm
    {
        ...
        // Production always uses two challenges: fixed dual-acc path matches
        // reduce_gate_constraints_base_batch (accumulate into gate-seeded res).
        if alphas.len() == 2 {
            let alpha_0 = alphas[0];
            let alpha_1 = alphas[1];
            for t in (0..num_rows).rev() {
                ...
                for k in 0..n {
                    let res = &mut res_out[k * 2..k * 2 + 2];
                    res[0] = row[k].multiply_accumulate(res[0], alpha_0);
                    res[1] = row[k].multiply_accumulate(res[1], alpha_1);
                }
            }
        } else {
            for t in (0..num_rows).rev() {
