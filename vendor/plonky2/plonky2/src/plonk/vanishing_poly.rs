...
///
/// `cpu_gate_indices` is the ascending list of gate indices that survive on
/// the CPU for this proof (computed once after the GPU offload decision), and
/// `filters` is a caller-owned per-point filter buffer reused across batches.
