...
fn fill_gate_filters<F: Field>(
    ...
    for (selector_index, group) in groups.iter().enumerate() {
        ...
        for row in group.clone().rev() {
            if row + 1 < group.end {
                let filter = &mut filters[row * batch_size..(row + 1) * batch_size];
                if !multiple_selectors && row == group.start {
                    filter.copy_from_slice(suffix);
                } else {
                    batch_multiply_inplace(filter, suffix);
                }
            }
            if row > group.start {
...
/// Evaluates all gate constraints in the base field into reusable caller-owned buffers.
