...
impl<F: RichField + Extendable<D>, const D: usize> OpeningSet<F, D> {
    pub fn new<C: GenericConfig<D, F = F>>(
        ...
        let eval_polynomials = |pows: &[F::Extension], polynomials: &[PolynomialCoeffs<F>]| {
            // Width-2 peel only: pair adjacent polynomials that share the
            // same power table so Goldilocks can load each `z^i` once.
            // Output order is the original polynomial order. The odd tail
            // (if any) uses the single-dot hook. No wave rewrite, no
            // range recording, no g-zeta change.
