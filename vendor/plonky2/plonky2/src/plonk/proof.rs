...
impl<'a, F: RichField + Extendable<D>, const D: usize> OpeningWorkPlan<'a, F, D> {
    ...
    fn evaluate(mut self) -> Vec<F::Extension> {
        ...
        // `par_iter` is an indexed global plan. Each map result is a fixed
        // stack array plus a valid-lane count; `flat_map_iter` exposes those
        // lanes directly to the sole collection, with no per-task Vec/heap.
        let output: Vec<_> = self
            .tasks
            .par_iter()
            .map(OpeningTask::evaluate)
            .flat_map_iter(OpeningTaskResult::into_valid_values)
            .collect();
