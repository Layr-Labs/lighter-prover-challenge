...
/// One polynomial evaluation in the global opening work plan.
...
impl<'a, F: RichField + Extendable<D>, const D: usize> OpeningTask<'a, F, D> {
    #[inline]
    fn evaluate(&self) -> F::Extension {
        F::extension_base_dot_product(self.powers, &self.polynomial.coeffs)
    }
}

/// Builds one indexed Rayon wave while retaining exact ranges in the flattened
/// element stream. Keeping one task per polynomial gives Rayon enough units to
/// balance uneven worker progress without relaunching a wave for every logical
/// group.
...
impl<'a, F: RichField + Extendable<D>, const D: usize> OpeningWorkPlan<'a, F, D> {
    ...
    /// Add one ordered output group and return its exact element range.
    fn add_polynomials(
        &mut self,
        powers: &'a [F::Extension],
        ...
        self.tasks.extend(polynomials.iter().map(|polynomial| OpeningTask {
            powers,
            polynomial,
        }));
    ...
    /// Retained at the call site as an explicit point boundary; single tasks
    /// never combine work across points.
    #[inline]
    fn finish_point(&mut self) {}

    fn evaluate(self) -> Vec<F::Extension> {
        let output_len = self.output_len;
        // `par_iter` is an indexed global plan with one final collection.
...
impl<F: RichField + Extendable<D>, const D: usize> OpeningSet<F, D> {
    pub fn new<C: GenericConfig<D, F = F>>(
        ...
        let task_capacity = zeta_output_count + shifted_output_count;
...
mod tests {
    ...
    /// Exercise range accounting, group order, and the hard boundary between
    /// zeta and g*zeta directly.
    #[test]
    fn opening_work_plan_preserves_output_order_and_point_boundaries() {
        ...
        for task in &plan.tasks[..5] {
            assert!(core::ptr::eq(task.powers, zeta_pows.as_slice()));
        }
        for task in &plan.tasks[5..] {
            assert!(core::ptr::eq(task.powers, g_zeta_pows.as_slice()));
        }
        assert!(core::ptr::eq(plan.tasks[2].polynomial, &zeta_first[2]));
        assert!(core::ptr::eq(plan.tasks[3].polynomial, &zeta_second[0]));
        assert!(core::ptr::eq(plan.tasks[5].polynomial, &shifted_z[0]));
        assert!(core::ptr::eq(plan.tasks[6].polynomial, &shifted_lookup[0]));
    ...
    #[test]
    #[ignore]
    fn opening_single_wave_production_shape_microbenchmark() {
        ...
        for sample in 0..SAMPLES {
            if sample % 2 == 0 {
                ...
                combined_samples.push(combined_work_plan());
            } else {
                combined_samples.push(combined_work_plan());
                singles_samples.push(workplan_singles());
                original_samples.push(original_waves_singles());
        ...
        println!(
            "opening work-plan microbench: degree={DEGREE} outputs={OUTPUTS} samples={SAMPLES} original_waves_ms={:.3} direct_single_wave_ms={:.3} production_work_plan_ms={:.3} speedup_vs_original={:.3}x speedup_vs_direct={:.3}x",
