...
//! serial tail, each precomputed column is reused only if its IFFT
//! coefficients match the authoritative finished-witness IFFT bit-for-bit.
//! A later write (light-chain feed, lookup padding) changes those coefficients
//! and forces a recompute. Wrong reuse is therefore either a recompute (slow)
//! or a Merkle-cap / FRI mismatch (closed verify fail) — never a silent wrong
//! proof.
...
pub struct EarlyWireLde<F: Field> {
    /// IFFT of complete snapshot values. The snapshot values themselves are
    /// dropped after this transform; v2's authoritative gate compares these
    /// coefficients, so retaining every preimage through the light spine was
    /// dead memory.
...
mod tests {
    ...
    fn lde_one_column_matches_values_ifft_then_lde_values_body() {
        ...
        let from_production =
            PolynomialBatch::<F, PoseidonGoldilocksConfig, 2>::lde_values(
                &[coeffs],
                rate_bits,
                false,
                None,
            );
        assert_eq!(from_production.len(), 1);
        assert_eq!(from_one, from_production[0]);
    }
}
