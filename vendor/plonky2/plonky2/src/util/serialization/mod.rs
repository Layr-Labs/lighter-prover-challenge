...
pub trait Write {
    ...
    >(
        ...
        let ProverOnlyCircuitData {
            generators,
            // Runtime-only: reconstructed from `generators` on read, so it contributes no bytes
            // and the serialized format is unchanged.
...
mod tests {
    ...
    fn prover_only_data_round_trip_preserves_rep_map_and_watch_counts() -> Result<()> {
        ...
        assert_eq!(
            decoded.generator_readiness_is_authoritative,
            circuit.prover_only.generator_readiness_is_authoritative,
            "reconstructed readiness contracts differ from the builder-derived ones"
        );
        assert_eq!(
