...
pub trait Read {
    ...
    >(
        ...
        let permutation_factor_flags = forest.wire_partition().permutation_factor_flags(
            subgroup.len(),
            common_data.config.num_routed_wires,
...
mod tests {
    ...
    fn prover_only_data_round_trip_preserves_rep_map_and_watch_counts() -> Result<()> {
        ...
        assert_eq!(
            decoded.permutation_factor_flags,
            circuit.prover_only.permutation_factor_flags,
            "runtime cancellation flags were not reconstructed from the representative map"
        );
        assert_eq!(
