...
use crate::field::batch_util::batch_extension_scalar_multiply_add_inplace;
        ...
        F: FieldExtension<D, BaseField = BF>,
    {
        ...
        let accumulate_chunk = |ps: &[_], powers: &[F]| -> Vec<F> {
            ...
            for (base_power, poly) in ps_iter {
                let coeffs: &PolynomialCoeffs<BF> = Borrow::borrow(poly);
                batch_extension_scalar_multiply_add_inplace::<BF, F, D>(
                    &mut acc[..coeffs.coeffs.len()],
                    *base_power,
                    &coeffs.coeffs,
                );
            }
            ...
            .for_each(|(block, out)| {
                ...
                for (base_power, poly) in base_powers.iter().zip(&polys) {
                    ...
                    batch_extension_scalar_multiply_add_inplace::<BF, F, D>(
                        &mut out[..live],
                        *base_power,
                        &coeffs.coeffs[start..start + live],
                    );
                }
            });
