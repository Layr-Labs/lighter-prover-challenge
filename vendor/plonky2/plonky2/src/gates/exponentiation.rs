...
impl<F: RichField + Extendable<D>, const D: usize> Gate<F, D> for ExponentiationGate<F, D> {
    ...
    fn eval_unfiltered_base_batch_accumulate(
        ...
        for i in 0..self.num_power_bits {
            ...
            let intermediate = col(self.wire_intermediate_value(i));
            if i == 0 {
                for p in 0..n {
                    scratch[p] = (cur_bit[p] * base[p] + (F::ONE - cur_bit[p])) - intermediate[p];
                }
            } else {
                let prev = col(self.wire_intermediate_value(i - 1));
                for p in 0..n {
                    scratch[p] =
                        (prev[p].square() * (cur_bit[p] * base[p] + (F::ONE - cur_bit[p])))
                            - intermediate[p];
                }
            }
            let combined =
...
mod tests {
    ...
    fn direct_filtered_accumulation_matches_materialized_batch() {
        ...
        let filters = (0..N)
            ...
            .collect::<Vec<_>>();
        let materialized =
            <ExponentiationGate<F, D> as Gate<F, D>>::eval_unfiltered_base_batch(&gate, vars);
