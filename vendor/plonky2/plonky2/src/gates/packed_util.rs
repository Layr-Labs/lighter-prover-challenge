...
pub trait PackedEvaluableBase<F: RichField + Extendable<D>, const D: usize>: Gate<F, D> {
    ...
    /// lane group at a time and directly multiply-accumulates each emitted constraint into the
    /// shared quotient buffer, instead of materializing an intermediate constraint matrix. Per
