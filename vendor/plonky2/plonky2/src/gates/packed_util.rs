...
pub trait PackedEvaluableBase<F: RichField + Extendable<D>, const D: usize>: Gate<F, D> {
    ...
    fn eval_unfiltered_base_batch_accumulate_packed(
        ...
        for (i, vars_packed) in vars_packed_iter.enumerate() {
            ...
            // Same lanes, same order, same `multiply_accumulate` — values identical.
            // Alignment-safe by construction: the packed type is
            // `#[repr(transparent)]` over `[GoldilocksField; WIDTH]` and its
            // documentation states the representation "deliberately retains the
            // scalar field's eight-byte alignment so that a scalar slice can be
            // reinterpreted as packed values"; `from_slice` additionally asserts
            // the length equals `WIDTH`, which every slice below satisfies.
