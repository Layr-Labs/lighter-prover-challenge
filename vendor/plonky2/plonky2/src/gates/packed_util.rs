...
pub trait PackedEvaluableBase<F: RichField + Extendable<D>, const D: usize>: Gate<F, D> {
    ...
    /// Filter-fused variant of [`Self::eval_unfiltered_base_batch_packed`].
    /// Each packed gate emission is multiply-accumulated directly into the
    /// shared quotient row, avoiding an otherwise write-only scratch matrix.
    /// Per point `p` and constraint `j` this performs the same
    /// `combined[j * n + p] += constraint * filters[p]` product-accumulate the
    /// materialize-then-add default performs, with the same packed evaluation
    /// producing each constraint value.
    fn eval_unfiltered_base_batch_accumulate_packed(
        ...
        for (i, vars_packed) in vars_packed_iter.enumerate() {
            ...
            self.eval_unfiltered_base_packed(
                vars_packed,
                StridedConstraintConsumer::accumulating(
