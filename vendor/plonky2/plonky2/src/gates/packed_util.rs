...
pub trait PackedEvaluableBase<F: RichField + Extendable<D>, const D: usize>: Gate<F, D> {
    ...
    fn eval_unfiltered_base_batch_accumulate_packed(
        ...
        for (i, vars_packed) in vars_packed_iter.enumerate() {
            ...
            // Load packed lanes directly. The previous form zero-initialized
            // each packed value and then immediately overwrote every lane, so
            // the `ZEROS` store was dead. Alignment-safe by construction: the
            // packed type is `#[repr(transparent)]` over `[GoldilocksField; WIDTH]`
            // and `from_slice` asserts len == WIDTH.
