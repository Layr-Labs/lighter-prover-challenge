...
pub trait PackedEvaluableBase<F: RichField + Extendable<D>, const D: usize>: Gate<F, D> {
    ...
    /// lane group at a time and accumulates the filtered constraint rows straight into the shared
    /// quotient buffer, instead of materializing a transient `num_constraints x P::WIDTH` scratch
    /// block and re-walking it in a second pass.
    ///
    /// Per point `p` and constraint `j` this performs the same
    /// `combined[j * n + p] += constraint * filters[p]` product-accumulate that the
    /// materialize-then-add default performs, with the same packed evaluation producing each
    /// constraint value, so the result is value-identical; only the scratch store and reload are
    /// removed.
