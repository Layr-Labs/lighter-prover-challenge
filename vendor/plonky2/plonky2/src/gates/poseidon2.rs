    ...
    for Poseidon2Generator<F, D>
{
    ...
    fn run_once(
        ...
        if swap_value == F::ZERO {
            for i in 0..4 {
                out_buffer.set_wire(
                    local_wire(Poseidon2Gate::<F, D>::wire_delta(i)),
                    F::ZERO,
                )?;
            }
            ...
            // Preserve the original generator behavior for an invalid, non-boolean witness. The
            // valid 0/1 paths above avoid four general field multiplications per permutation.
            for i in 0..4 {
...
mod tests {
    ...
    fn generated_output() {
        ...
        for swap_value in [F::ZERO, F::ONE] {
            ...
            let mut expected_state: [F; WIDTH] =
                core::array::from_fn(|i| permutation_inputs[i]);
            if swap_value == F::ONE {
                for i in 0..4 {
                    expected_state.swap(i, i + 4);
                }
            }
            let expected_outputs = F::poseidon2(expected_state);
            for i in 0..WIDTH {
            ...
            for i in 0..4 {
                let delta = witness.get_wire(Wire {
                    row,
