...
impl<F: RichField + Extendable<D> + Poseidon2, const D: usize> Gate<F, D> for Poseidon2Gate<F, D> {
    ...
    fn eval_unfiltered_base_batch_accumulate(
        ...
        // Packed lane-group evaluation fused with the filter accumulate: the same
        // 4-lane NEON permutation arithmetic as `eval_unfiltered_base_batch`, with each
        // constraint row folded straight into the shared quotient buffer through an
        // L1-resident per-group scratch block instead of a materialized matrix or a
        // scalar per-point state walk.
