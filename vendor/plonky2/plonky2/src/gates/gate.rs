...
/// Static wire-layout metadata for the downstream RandomAccessGate
/// specialization advertised to optional quotient backends. The gate packs
/// `num_copies` independent list selections, each over a `2^bits`-element
/// list, with the leftover wires reused as `num_extra_constants` constant
/// equality checks.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
pub trait Gate<F: RichField + Extendable<D>, const D: usize>: 'static + Send + Sync {
    ...
    /// Advertises the exact RandomAccessGate layout to optional quotient
