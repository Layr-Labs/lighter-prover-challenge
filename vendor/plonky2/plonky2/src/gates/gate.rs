...
/// Static wire-layout metadata for `EqualityGate`: `num_ops` operations of
/// three routed wires (x, y, output) then three unrouted temporaries
/// (diff, invdiff, prod), plus one local constant.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct EqualityQuotientGate {
    pub num_ops: usize,
}
...
pub trait Gate<F: RichField + Extendable<D>, const D: usize>: 'static + Send + Sync {
    ...
    /// Advertises the `EqualityGate` layout to optional quotient backends.
    fn equality_quotient_gate(&self) -> Option<EqualityQuotientGate> {
        None
    }
