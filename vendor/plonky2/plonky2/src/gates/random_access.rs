...
impl<F: RichField + Extendable<D>, const D: usize> Gate<F, D> for RandomAccessGate<F, D> {
    ...
    fn eval_unfiltered_base_batch_accumulate(
        ...
        // Mirrors `eval_unfiltered_base_batch` constraint-for-constraint,
        // folding each row straight into the shared accumulator.
        macro_rules! emit {
            () => {{
...
mod tests {
    ...
    fn direct_filtered_accumulation_matches_materialized_batch() {
        ...
        let gate = RandomAccessGate::<F, D>::new(4, 4, 1);
