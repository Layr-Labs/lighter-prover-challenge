...
pub(crate) fn selector_polynomials<F: RichField + Extendable<D>, const D: usize>(
    ...
    // Each instance generates its own ID exactly once; rows resolve in parallel.
    ...
    // Every cell is a pure function of (group, instance gate index), so each selector
    // polynomial fills row-parallel with the identical values the serial loop wrote.
