...
impl<const D: usize> ReducingGate<D> {
    ...
    /// `coeff` is a single base limb, so its imaginary part is zero.
    fn eval_accumulate_packed<F: RichField + Extendable<D> + Packable>(
        ...
        let zero = <F as Packable>::Packing::ZEROS;
        ...
        for i in 0..self.num_coeffs {
            let coeff_start = Self::START_COEFFS + i;
            let acc_start = self.wires_accs(i).start;
            for g in 0..num_groups {
                let off = g * width;
...
mod tests {
    ...
    fn packed_accumulate_matches_scalar_across_batch_sizes() {
        ...
        let gate = ReducingGate::<D>::new(22);
        fn value(i: usize) -> F {
        ...
        let mut batch_sizes = vec![1,3,5,7,11,31,32,33, packing_width, packing_width+1, packing_width+2, 2*packing_width-1, 2*packing_width, 2*packing_width+1];
