...
fn accumulate_quadratic_scratch_free<F: RichField + Extendable<D>, const D: usize>(
    ...
    for i in 0..num_ops {
        ...
        for p in (0..packed_len).step_by(width) {
            let mut term_0 = MaybeUninit::<Packing<F>>::uninit();
            let mut term_1 = MaybeUninit::<Packing<F>>::uninit();
            // SAFETY: `PackedField` requires `Packing<F>` to have exactly the
            // layout of `[F; WIDTH]`. The loop below assigns all `WIDTH` lanes
            // of both values before either packed value is assumed initialized
            // or read. If computing a lane unwinds, `MaybeUninit` drops no
            // partially initialized packed value.
            let (term_0_lanes, term_1_lanes) = unsafe {
                (
                    core::slice::from_raw_parts_mut(term_0.as_mut_ptr().cast::<F>(), width),
                    core::slice::from_raw_parts_mut(term_1.as_mut_ptr().cast::<F>(), width),
                )
            };
            for lane in 0..width {
                let point = p + lane;
                ...
                term_0_lanes[lane] = arr[0];
                term_1_lanes[lane] = arr[1];
            }
            // SAFETY: the loop above wrote every scalar lane of both packed
            // values, and `PackedField` guarantees there is no extra padding.
            let (term_0, term_1) = unsafe { (term_0.assume_init(), term_1.assume_init()) };
