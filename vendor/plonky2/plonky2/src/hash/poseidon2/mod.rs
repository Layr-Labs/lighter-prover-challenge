...
#[cfg(all(feature = "std", target_arch = "aarch64", target_os = "macos"))]
pub use metal::{
    install_k6_dynamic_library, is_exclusive_gpu_phase, prewarm as prewarm_gpu,
    set_exclusive_gpu_phase, write_k6_dynamic_library,
};

/// Cross-build fallback: the helper executable runs on its Linux host and
/// emits the empty payload that retains the runtime-source K6 path.
#[cfg(all(
    feature = "std",
    not(all(target_arch = "aarch64", target_os = "macos"))
))]
pub fn write_k6_dynamic_library(path: &std::path::Path) -> Result<u64, String> {
    std::fs::write(path, []).map_err(|error| error.to_string())?;
    Ok(0)
}

/// No-op install on platforms without the Metal backend.
#[cfg(all(
    feature = "std",
    not(all(target_arch = "aarch64", target_os = "macos"))
))]
pub fn install_k6_dynamic_library(_bytes: &'static [u8]) {}
...
mod k6_contract_tests {
    ...
    const K6_WRAPPER: &str = include_str!("k6_residual_wrapper.metal");
    ...
    fn runtime_source_and_metadata_ownership_are_disjoint() {
        ...
        for binding in [
            ...
            assert!(K6_WRAPPER.contains(binding), "missing K6 ABI binding {binding}");
        }
        assert!(K6_BODY.contains("metadata + k6_index * 10u"));
        assert!(K6_BODY.contains("gl_add(output[(ulong)gid * 2u], total[0])"));
        assert!(K6_BODY.contains("__attribute__((visibility(\"default\")))"));
        assert!(K6_WRAPPER.len() <= 4 * 1024);
