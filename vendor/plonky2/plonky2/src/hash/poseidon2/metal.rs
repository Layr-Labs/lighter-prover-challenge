...
/// whose 60 ms CPU LDE fill was previously exposed before an ~83 ms GPU
/// hash. Narrow 2^19 trees (Zs/partial-products and quotient, ~20 columns)
/// stay on the classic path: their short fill would not amortize another
/// streamed hold.
///
...
pub(crate) fn build_merkle_tree_shared_streamed<F: RichField>(
    ...
    let exclusive = EXCLUSIVE_GPU_PHASE.load(core::sync::atomic::Ordering::Relaxed);
    // The streamed shapes are the wide wires trees that own the serialized GPU
    // stream: the exclusive phases'' 2^20+ leaf trees (pre-execution, final
    // block), the pipelined tx-proof wires tree (2^19 x 136, whose ~60 ms CPU
    // fill is comparable to the ~83 ms absorb), and the chain-step wires tree
    // (2^17 x 135) on the 49-fold serial chain spine, whose ~15 ms fill would
    // otherwise be exposed before a ~20 ms absorb on every fold. Narrower trees
    // (Zs/partial-products, quotient, ~20 columns) have short fills that would
    // not amortize holding the streamed buffers, so they stay classic.
    let large_wide = leaf_count >= 1 << 17 && leaf_width >= 128;
    if F::ORDER != 0xffff_ffff_0000_0001
        || size_of::<F>() != size_of::<u64>()
        || leaf_width < 16
        || leaf_count < 1 << 17
