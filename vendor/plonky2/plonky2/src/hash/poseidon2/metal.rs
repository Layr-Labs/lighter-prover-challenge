...
/// Two sets let the next caller populate its shared input buffer while the
/// current tree is on the GPU. [`MetalShared::execution`] still admits exactly
/// one command buffer at a time, preserving the promoted single-stream policy;
/// the extra set overlaps only CPU staging and completed-buffer readback.
...
struct MetalShared {
    ...
    available: Condvar,
    /// Serializes command submission and completion without serializing the
    /// CPU copies into and out of distinct pooled buffer sets.
    execution: Mutex<()>,
    ...
    > {
        ...
        let result = (|| -> Result<(Vec<HashOut<F>>, Vec<HashOut<F>>), String> {
            ...
            let gpu_guard = self
                .execution
                .lock()
                .map_err(|_| "GPU execution lock poisoned")?;
