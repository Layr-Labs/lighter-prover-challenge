...
/// the current shader. They are loaded as secondary libraries only: NTT,
/// Range, and permutation-quotient pipelines always come from
/// [`SHADER_METALLIB`].
///
...
fn spawn_optional_pipelines(
    ...
    for (name, slot, prefer_squaring) in [
        (
            "poseidon2_gate_quotient",
        ...
        ("poseidon2_absorb_pass", &ABSORB_PASS_PIPELINE, true),
    ] {
        let device = device.clone();
        let library = library.clone();
        let preferred = prefer_squaring
            .then(|| squaring_library.cloned())
            .flatten();
    ...
    fn new_with_historical_artifacts(use_historical_artifacts: bool) -> Result<Self, String> {
        autoreleasepool(|| {
            ...
            let squaring_library = use_historical_artifacts
                ...
                .flatten();
            let a5_library = use_historical_artifacts
            ...
            spawn_optional_pipelines(&device, &library, squaring_library.as_ref());
...
mod tests {
    ...
    fn metallib_loads_and_exposes_every_kernel() {
        ...
        // runtime. Lower the five selected historical kernels as well, so a
