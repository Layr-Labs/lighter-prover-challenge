...
pub(crate) struct RangeCheckGateQuotientJob<F> {
    ...
    output: Buffer,
    len: usize,
    accumulates_poseidon: bool,
    _job: GpuJobGuard,
    _phantom: PhantomData<F>,
}
impl<F: RichField> PoseidonGateQuotientJob<F> {
    pub(crate) fn wait_until_completed(&self) -> Result<(), String> {
...
impl<F: RichField> RangeCheckGateQuotientJob<F> {
    pub(crate) fn accumulates_poseidon(&self) -> bool {
        self.accumulates_poseidon
    }
...
pub(crate) fn start_range_check_gate_quotient<F: RichField>(
    ...
    quotient_rows: usize,
    step: usize,
    poseidon_job: Option<&PoseidonGateQuotientJob<F>>,
    ...
    let output_len = quotient_rows.checked_mul(2)?;
    ...
    if F::ORDER != 0xffff_ffff_0000_0001
        ...
        || step == 0
        || quotient_rows.checked_mul(step) != Some(wires.rows)
        || poseidon_job.map_or(false, |job| job.len != output_len)
        || wires.rows > u32::MAX as usize
        || quotient_rows > u32::MAX as usize
        || step > u32::MAX as usize
    {
    ...
    match context.start_range_check_gate_quotient(
        ...
        quotient_rows,
        step,
        poseidon_job.map(|job| &job.output),
        &metadata,
        specs.len(),
        promoted_specs.len(),
        &alpha_powers,
...
impl MetalShared {
    ...
    fn start_range_check_gate_quotient<F: RichField>(
        ...
        // Both kernels are committed to this context's serial command queue.
        // Reusing the earlier buffer therefore makes this kernel run after the
        // Poseidon write without a CPU-side synchronization or copy.
        let accumulates_poseidon = existing_output.is_some();
        let output = existing_output.map_or_else(
            || {
                autoreleasepool(|| {
                    self.device
                        .new_buffer(bytes as u64, MTLResourceOptions::StorageModeShared)
        ...
        let command_buffer = autoreleasepool(|| -> CommandBuffer {
            ...
            set_u32(encoder, 11, accumulates_poseidon as u32);
            dispatch(encoder, pipeline, quotient_rows);
            encoder.end_encoding();
            command_buffer.commit();
            command_buffer.to_owned()
