...
///
/// Every occupancy-conditional narrow 2^17 heavy tree (width in (4, 64]) changes route, only while both
...
fn heavy_narrow_tree_yields_to_light(
    lane: ChainGpuLane,
    light_ready: bool,
    exclusive: bool,
    leaf_width: usize,
    ...
    // Rung-2: yield every occupancy-conditional 2^17 narrow tree (width in
    // (4, 64]), not only the width-16 quotient tree. Heavy has 3 folds vs
    // Light's 49 — Light owns the deadline; Heavy has join slack.
    !exclusive
...
fn gpu_worthwhile(leaf_width: usize, leaf_count: usize, cap_height: usize) -> bool {
    ...
    if heavy_narrow_tree_yields_to_light(lane, light_ready, exclusive, leaf_width, leaf_count) {
        return false;
    }
...
mod tests {
    ...
    #[test]
    fn heavy_narrow_admission_yields_only_to_ready_light_work() {
        let narrow_width = 16;
        let chain_leaves = 1 << 17;

        assert!(heavy_narrow_tree_yields_to_light(
        ...
        for (lane, light_ready, exclusive, width, leaves) in [
            ...
            (ChainGpuLane::HeavyWithPeer, true, false, 4, chain_leaves),
            (
                ChainGpuLane::HeavyWithPeer,
                true,
                false,
