...
pub fn prewarm_streamed_buffers(leaf_count: usize) {
    ...
    // Seed the streamed pair pool with this page-walked pair. The pool holds
    // idle pairs rather than the single `Option` slot this prewarm was written
    // against: replace the smallest idle pair when it is too small for the
    // block (or add one when the pool has never allocated), which reproduces
    // the single-slot behaviour at the default capacity of 1 and simply warms
    // one more pair at higher capacities. Leased pairs are left alone -- a
    // build in flight owns its pair.
    if let Ok(mut pool) = STREAMED_BUFFERS.lock() {
        let too_small = |pair: &(Buffer, Buffer)| {
            pair.0.length() < state_bytes || pair.1.length() < output_bytes
        };
        match pool
            .idle
            .iter()
            .enumerate()
            .min_by_key(|(_, pair)| pair.0.length())
        {
            Some((index, pair)) if too_small(pair) => pool.idle[index] = (state, output),
            None => pool.idle.push((state, output)),
            Some(_) => {}
        }
    }
