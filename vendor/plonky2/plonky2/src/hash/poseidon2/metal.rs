...
/// Degree-2^14 serial circuits (pre-exec, chain) commit at 2^17 leaves.
/// Same 136 wires, 8-byte limbs: 136 MiB. Distinct from the chunk 544.
/// Best-fit would otherwise hand a warm 544 to this shape and force the
/// next chunk fill to zero-fault a fresh 544.
const SERIAL_TX_WIRES_STORE_BYTES: u64 = 136 * (1 << 17) * 8;
        ...
        .spawn(|| {
            ...
            if let Some(context) = shared_context() {
                ...
                for _ in 0..TX_WIRES_PREWARM_COPIES {
                    walk_into_pool(TX_WIRES_STORE_BYTES);
                }
                // After the chunk 544s: one serial-shaped 136 MiB so
                // take_best_fit(2^17 wires) does not steal a 544.
                walk_into_pool(SERIAL_TX_WIRES_STORE_BYTES);
            }
        })
        .ok();
}
...
fn gpu_worthwhile(leaf_width: usize, leaf_count: usize, cap_height: usize) -> bool {
    ...
    if fri_fold_shape && (exclusive || idle) {
        ...
        // width 17..=64 in 2^14..=2^17 without a 131k floor; width 8
        // at 2^15 (~65k, GPU/CPU 1.37) stays CPU.
        //
        // 2^13 width-32 is the serial circuit's first FRI fold
        // (degree 2^14, LDE 2^17, arity 16 → 2^13 leaves). ~41k perms,
        // still occupancy-gated. 2^13 width-8 stays CPU.
...
/// Whether a shared-column Merkle build should take the streamed fill/absorb
/// path. Scheduling-only: the two paths are bit-identical.
///
/// Exclusive drain previously streamed only the 2^20+ final-block tree.
/// Serial circuits commit at 2^17 × 136 during that phase and used the
/// pooled set (classic fill-then-hash). Admitting 2^17 here overlaps fill
/// with absorb out of `STREAMED_BUFFERS` and leaves the single pooled set
/// for occupancy-gated FRI folds. Width &lt; 16 still stays classic.
fn streamed_merkle_admitted(leaf_width: usize, leaf_count: usize) -> bool {
    if leaf_width < 16 || !leaf_count.is_power_of_two() {
        return false;
    }
    if EXCLUSIVE_GPU_PHASE.load(core::sync::atomic::Ordering::Relaxed) {
        return leaf_count >= 1 << 17;
    }
    leaf_count >= 1 << 19
        && (leaf_width > 64
...
/// the GPU absorbs each group while the CPU fills the next. Exclusive drain
/// streams 2^17+ (serial commitments plus the final block). The pipeline
/// streams the widest 2^19+ trees so they stay off the single buffer set.
///
...
pub(crate) fn build_merkle_tree_shared_streamed<F: RichField>(
    ...
    if F::ORDER != 0xffff_ffff_0000_0001
        || size_of::<F>() != size_of::<u64>()
        || !streamed_merkle_admitted(leaf_width, leaf_count)
        || leaf_count > u32::MAX as usize
        || leaf_width > u32::MAX as usize
    ...
    // One queue, commit order is FIFO: waiting on the last absorb
    // completes every earlier pass. Poll status afterwards so a
    // mid-stream error still fails the build. The old loop called
    // `wait_until_completed` on every group (17 times for serial
    // 136-wide 2^17); each wait is a userspace sync even when the
...
impl MetalShared {
    ...
    > {
        ...
        drop(job);
        let readback = result?;
        // Both buffers are GPU-complete after the wait above. Tree
        // readback (output) and the oracle coeff copy (coeffs) touch
        // disjoint shared memory; join them instead of serializing
        // two bulk copies on the exclusive-phase critical path.
        let coeff_source = unsafe {
            slice::from_raw_parts(coeffs_buffer.contents().cast::<F>(), value_len)
        };
        let ((digests, cap), coeff_columns) = join(
            || readback.finish(),
            || {
                coeff_source
                    .par_chunks(degree)
...
mod tests {
    ...
    fn gpu_worthwhile_admits_idle_fri_folds_above_break_even() {
        ...
        assert!(
            gpu_worthwhile(32, 1 << 13, 4),
            "2^13 width-32 idle is the serial first arity-16 fold"
        );
        assert!(
            !gpu_worthwhile(8, 1 << 13, 4),
            "2^13 width-8 idle stays CPU"
        );
        assert!(
        ...
        assert!(
            !gpu_worthwhile(32, 1 << 13, 4),
            "2^13 width-32 behind a chunk tree stays CPU"
        );
        assert!(
            gpu_worthwhile(8, 1 << 17, 4),
    ...
    #[test]
    fn streamed_merkle_admits_exclusive_serial_2_17() {
        let was_exclusive = EXCLUSIVE_GPU_PHASE.swap(true, core::sync::atomic::Ordering::Relaxed);
        let jobs = GPU_JOBS_IN_FLIGHT.swap(0, core::sync::atomic::Ordering::Relaxed);
        assert!(
            streamed_merkle_admitted(136, 1 << 17),
            "exclusive serial 2^17 x 136 streams"
        );
        assert!(
            streamed_merkle_admitted(136, 1 << 21),
            "exclusive final-block 2^21 still streams"
        );
        assert!(
            !streamed_merkle_admitted(8, 1 << 17),
            "width < 16 stays classic"
        );
        assert!(
            !streamed_merkle_admitted(136, 1 << 16),
            "exclusive 2^16 stays classic"
        );
        EXCLUSIVE_GPU_PHASE.store(false, core::sync::atomic::Ordering::Relaxed);
        assert!(
            !streamed_merkle_admitted(136, 1 << 17),
            "pipeline 2^17 does not stream"
        );
        assert!(
            streamed_merkle_admitted(136, 1 << 19),
            "pipeline idle wide 2^19 streams"
        );
        GPU_JOBS_IN_FLIGHT.store(1, core::sync::atomic::Ordering::Relaxed);
        assert!(
            streamed_merkle_admitted(136, 1 << 19),
            "pipeline busy width>64 2^19 still streams"
        );
        assert!(
            !streamed_merkle_admitted(32, 1 << 19),
            "pipeline busy narrow 2^19 stays classic"
        );
        GPU_JOBS_IN_FLIGHT.store(jobs, core::sync::atomic::Ordering::Relaxed);
        EXCLUSIVE_GPU_PHASE.store(was_exclusive, core::sync::atomic::Ordering::Relaxed);
    }
    ...
    fn tx_wires_prewarm_copies_fit_column_store_pool() {
        ...
        assert!(SERIAL_TX_WIRES_STORE_BYTES <= MAX_CACHED_COLUMN_STORE_BYTES);
        assert!(SERIAL_TX_WIRES_STORE_BYTES < TX_WIRES_STORE_BYTES);
        assert!(
            TX_WIRES_STORE_BYTES * TX_WIRES_PREWARM_COPIES as u64
                + SERIAL_TX_WIRES_STORE_BYTES
