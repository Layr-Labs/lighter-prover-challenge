...
/// AIR-specialized copy of the ranked light-transaction Range/U32 quotient
/// kernel. It retains the ordinary kernel ABI, but pins the exact metadata and
/// loop counts so the AIR lowerer can remove metadata branches while keeping a
/// single grid dispatch.
const LIGHT_TX_RANGE_METALLIB: &[u8] = include_bytes!("poseidon2_range_mono.metallib");
#[cfg(test)]
const LIGHT_TX_RANGE_METALLIB_SHA256: &str =
    "951b764b3984171c3061b62f4b6da3af7f18b910044a247e11ed8fdb85fc696a";
...
static LIGHT_TX_RANGE_PIPELINE: std::sync::OnceLock<Option<ComputePipelineState>> =
...
fn light_tx_range_pipeline() -> Option<&'static ComputePipelineState> {
    LIGHT_TX_RANGE_PIPELINE.get()?.as_ref()
}
...
/// Lowers the exact-light-tx Range pipeline after the ordinary optional
/// pipelines have settled. This keeps its compiler-service work off the
/// context's critical path; an unavailable specialization always falls back
/// to the original generic kernel.
fn spawn_light_tx_range_pipeline(device: &Device) {
    let device = device.clone();
    let spawned = std::thread::Builder::new()
        .name("poseidon2-metal-range-light-specialized".to_string())
        .spawn(move || {
            // Avoid adding AIR->ISA contention to the four existing optional
            // pipelines. Their work begins first and is needed by every
            // circuit shape; this pipeline is useful only for the 49 light tx
            ...
            let pipeline = autoreleasepool(|| {
                let library = device
                    .new_library_with_data(LIGHT_TX_RANGE_METALLIB)
                    .ok()?;
            ...
            if pipeline.is_some() {
                log::info!("light-tx specialized Range pipeline ready");
            } else {
                log::debug!(
                    "light-tx Range specialization unavailable; using the generic pipeline"
                );
            }
            let _ = LIGHT_TX_RANGE_PIPELINE.set(pipeline);
        });
    if spawned.is_err() {
        let _ = LIGHT_TX_RANGE_PIPELINE.set(None);
    }
}
...
impl MetalShared {
    fn new() -> Result<Self, String> {
        autoreleasepool(|| {
            ...
            spawn_light_tx_range_pipeline(&device);
    ...
    fn start_range_check_gate_quotient<F: RichField>(
        ...
        let specialized = is_light_tx_range_request(
            wires.rows,
            wires.cols,
            constants.rows,
            constants.cols,
        ...
        .then(light_tx_range_pipeline)
        .flatten();
        let pipeline = match specialized {
...
mod tests {
    ...
    #[test]
    fn light_tx_range_metallib_is_pinned_and_lowerable() {
        let dir = concat!(env!("CARGO_MANIFEST_DIR"), "/src/hash/poseidon2");
        let file = "poseidon2_range_mono.metallib";
        let output = std::process::Command::new("/usr/bin/shasum")
        ...
        assert_eq!(
            ...
            Some(LIGHT_TX_RANGE_METALLIB_SHA256)
        );
