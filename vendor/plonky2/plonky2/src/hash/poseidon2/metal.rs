...
pub fn prewarm_large_column_store(bytes: u64) {
    ...
    // Stash BEFORE walking: if the final block arrives mid-walk it takes the
    // buffer immediately and still benefits from every page touched so far
    // (a partially warmed buffer strictly dominates a cold one); the walk
    // continues touching pages the fill will write anyway — value-exact.
...
pub(crate) fn build_merkle_tree_shared_streamed<F: RichField>(
    ...
    // Exclusive-phase 2^20+ trees only: a red-team pass flagged the
    // occupancy-conditional 2^19 extension as state-conditional (its trigger
    // rate is a machine-state distribution that differs between hosts — the
    // same portability class that regressed ranked scoring before) for ~0
    // measured local gain, so it was removed.
    if F::ORDER != 0xffff_ffff_0000_0001
        || size_of::<F>() != size_of::<u64>()
        || leaf_width < 16
        || leaf_count < 1 << 20
