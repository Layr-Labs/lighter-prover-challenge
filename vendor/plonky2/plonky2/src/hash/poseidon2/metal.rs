...
pub mod startup_probe {
    ...
    /// Record the first event named `name` (later same-name calls are ignored).
    pub fn mark(name: &'static str) {
        let t0 = *START.get_or_init(Instant::now);
        let us = t0.elapsed().as_micros() as u64;
        let mut ev = match EVENTS.lock() {
            Ok(g) => g,
            Err(p) => p.into_inner(),
        };
    ...
    pub fn report() {
        ...
        match (get("ctx_ready"), get("first_commit")) {
            ...
            _ => {
                eprintln!("  SIGNED measure unavailable (missing ctx_ready and/or first_commit)");
            }
        }
    }
}
