...
use objc::Message;
use metal::{
    Buffer, CommandBuffer, CommandQueue, CompileOptions, ComputePipelineDescriptor,
    ComputePipelineState, Device, DynamicLibrary, MTLLibraryType, MTLCommandBufferStatus,
...
const K6_RESIDUAL_WRAPPER_SOURCE: &str = include_str!("k6_residual_wrapper.metal");

const K6_DYNAMIC_LIBRARY_INSTALL_NAME: &str = "@loader_path/liblighter_k6.metallib";
static K6_DYNAMIC_LIBRARY_BYTES: OnceLock<&'static [u8]> = OnceLock::new();

/// Installs a build-host-generated K6 dynamic library before Metal prewarming.
/// Empty bytes deliberately mean "use the checked-in runtime-source path";
/// incompatible nonempty bytes fail the K6 pipeline closed for this process.
pub fn install_k6_dynamic_library(bytes: &'static [u8]) {
    let _ = K6_DYNAMIC_LIBRARY_BYTES.set(bytes);
}
...
fn k6_residual_shader_source_for(dynamic_library: bool) -> Result<String, String> {
    const DYNAMIC_LIBRARY_HEADER: &str = "#include <metal_stdlib>\n\
using namespace metal;\n\
#define GOLDILOCKS_PRIME 0xffffffff00000001UL\n\
#define GOLDILOCKS_EPSILON 0xffffffffUL\n";
    ...
    let header = if dynamic_library {
        // Program-scope bindings in dynamic libraries require newer MSL
        // language modes. The implementation only needs these two scalars,
        // so macros keep the serialized dylib compatible with Metal 3.0/3.1.
        DYNAMIC_LIBRARY_HEADER
    } else {
        before("// Compile-time Poseidon2 round constants")?
    };
    let add_sub = between(
    ...
    let capacity = header
        ...
        .and_then(|len| {
            len.checked_add(if dynamic_library {
                0
            } else {
                K6_RESIDUAL_WRAPPER_SOURCE.len()
            })
        })
        .ok_or_else(|| "K6 shader source size overflow".to_string())?;
    let mut source = String::with_capacity(capacity + 64);
    if dynamic_library {
        source.push_str("#define LIGHTER_K6_DYNAMIC_LIBRARY 1\n");
    }
    ...
    if !dynamic_library {
        source.push('\n');
        source.push_str(K6_RESIDUAL_WRAPPER_SOURCE);
    }
    Ok(source)
}

fn k6_residual_shader_source() -> Result<String, String> {
    k6_residual_shader_source_for(false)
}

fn k6_dynamic_library_shader_source() -> Result<String, String> {
    k6_residual_shader_source_for(true)
}

fn metal_file_url(path: &Path) -> Result<URL, String> {
    let absolute = if path.is_absolute() {
        path.to_path_buf()
    } else {
        std::env::current_dir()
            .map_err(|error| format!("cannot resolve Metal library path: {error}"))?
            .join(path)
    };
    let path = absolute
        .to_str()
        .ok_or_else(|| "Metal library path is not valid UTF-8".to_string())?;
    // metal 0.33 wraps `+[NSURL URLWithString:]` (an autoreleased +0 object)
    // as an owned `URL`. Retain it through `Clone`, then forget only the
    // invalid +0 owner; the autorelease pool balances that original object
    // and the returned +1 wrapper can release safely on Drop.
    let autoreleased = URL::new_with_string(&format!("file://{path}"));
    let retained = autoreleased.clone();
    std::mem::forget(autoreleased);
    Ok(retained)
}

/// Compiles the K6 implementation as a GPU-specific dynamic library and
/// serializes it for embedding by the benchmark build script. The caller runs
/// this inside a child process: some restricted build bridges crash rather
/// than returning an NSError when they deny Metal compiler operations.
pub fn write_k6_dynamic_library(path: &Path) -> Result<u64, String> {
    autoreleasepool(|| {
        let device = Device::system_default().ok_or("no default Metal device on build host")?;
        if !device.supports_dynamic_libraries() {
            return Err("build-host Metal device does not support dynamic libraries".to_string());
        }

        let source = k6_dynamic_library_shader_source()?;
        let options = CompileOptions::new();
        options.set_library_type(MTLLibraryType::Dynamic);
        options.set_install_name(K6_DYNAMIC_LIBRARY_INSTALL_NAME);
        let library = device
            .new_library_with_source(&source, &options)
            .map_err(|error| format!("K6 dynamic-library compilation failed: {error}"))?;
        let dynamic_library = device
            .new_dynamic_library(&library)
            .map_err(|error| format!("K6 dynamic-library creation failed: {error}"))?;

        let _ = std::fs::remove_file(path);
        let url = metal_file_url(path)?;
        if !dynamic_library
            .serialize_to_url(&url)
            .map_err(|error| format!("K6 dynamic-library serialization failed: {error}"))?
        {
            return Err("K6 dynamic-library serializer returned false".to_string());
        }
        let bytes = std::fs::metadata(path)
            .map_err(|error| format!("cannot stat serialized K6 dynamic library: {error}"))?
            .len();
        if bytes == 0 {
            return Err("serialized K6 dynamic library is empty".to_string());
        }
        let reopened = device
            .new_dynamic_library_with_url(&url)
            .map_err(|error| format!("serialized K6 dynamic library cannot be reopened: {error}"))?;

        // Metal performs the final cross-library link while lowering the
        // pipeline, not while compiling the wrapper source. Validate the
        // serialized/reopened artifact with the exact runtime descriptor so
        // an ABI or symbol mismatch can never be published as nonempty bytes.
        // This can crash on a restricted build bridge, which is why build.rs
        // runs this entire function in a disposable child process.
        let wrapper_options = CompileOptions::new();
        wrapper_options.set_libraries(&[reopened.as_ref()]);
        let wrapper_library = device
            .new_library_with_source(K6_RESIDUAL_WRAPPER_SOURCE, &wrapper_options)
            .map_err(|error| format!("reopened K6 wrapper compilation failed: {error}"))?;
        let function = wrapper_library
            .get_function("k6_residual_quotient", None)
            .map_err(|error| format!("reopened K6 wrapper kernel unavailable: {error}"))?;
        let descriptor = ComputePipelineDescriptor::new();
        descriptor.set_compute_function(Some(&function));
        descriptor.set_insert_libraries(&[reopened.as_ref()]);
        device
            .new_compute_pipeline_state(&descriptor)
            .map_err(|error| format!("reopened K6 pipeline link failed: {error}"))?;
        Ok(bytes)
    })
}

fn load_embedded_k6_dynamic_library(device: &Device) -> Result<DynamicLibrary, String> {
    let bytes = K6_DYNAMIC_LIBRARY_BYTES
        .get()
        .copied()
        .unwrap_or_default();
    if bytes.is_empty() {
        return Err("no build-host K6 dynamic library was embedded".to_string());
    }
    if !device.supports_dynamic_libraries() {
        return Err("runtime Metal device does not support dynamic libraries".to_string());
    }

    // MTLDynamicLibrary only accepts a file URL. Keep the process-unique file
    // alive because the driver may defer mapping until pipeline publication.
    let directory: PathBuf = std::env::temp_dir().join(format!(
        "lighter-k6-dylib-{}",
        std::process::id()
    ));
    std::fs::create_dir_all(&directory)
        .map_err(|error| format!("cannot create K6 dynamic-library directory: {error}"))?;
    let path = directory.join("liblighter_k6.metallib");
    std::fs::write(&path, bytes)
        .map_err(|error| format!("cannot materialize K6 dynamic library: {error}"))?;
    let url = metal_file_url(&path)?;
    device
        .new_dynamic_library_with_url(&url)
        .map_err(|error| format!("embedded K6 dynamic library cannot be loaded: {error}"))
}

fn k6_dynamic_library_pipeline(device: &Device) -> Result<ComputePipelineState, String> {
    let dynamic_library = load_embedded_k6_dynamic_library(device)?;
    let options = CompileOptions::new();
    options.set_libraries(&[dynamic_library.as_ref()]);
    let library = device
        .new_library_with_source(K6_RESIDUAL_WRAPPER_SOURCE, &options)
        .map_err(|error| format!("K6 wrapper compilation failed: {error}"))?;
    let function = library
        .get_function("k6_residual_quotient", None)
        .map_err(|error| format!("K6 wrapper kernel unavailable: {error}"))?;
    let descriptor = ComputePipelineDescriptor::new();
    descriptor.set_compute_function(Some(&function));
    // metal 0.33 exposes the legacy spelling of `preloadedLibraries` as
    // `insertLibraries`; Apple documents the properties as equivalent.
    descriptor.set_insert_libraries(&[dynamic_library.as_ref()]);
    device
        .new_compute_pipeline_state(&descriptor)
        .map_err(|error| format!("K6 dynamic pipeline creation failed: {error}"))
}
    ...
    let spawned = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let result = autoreleasepool(|| -> Result<ComputePipelineState, String> {
                if K6_DYNAMIC_LIBRARY_BYTES
                    .get()
                    .is_some_and(|bytes| !bytes.is_empty())
                {
                    // A nonempty build artifact is authoritative. If it is
                    // incompatible with this worker, keep K6 on the CPU rather
                    // than paying the full source front end as a hidden retry.
                    return k6_dynamic_library_pipeline(&device);
                }

                // Probe builds whose Metal helper was unavailable retain the
                // exact 00d987dc behaviour. A later commit can replace the
                // generated payload with extracted static bytes and remove
                // this full-source fallback independently.
