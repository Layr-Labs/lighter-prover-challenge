...
mod tests {
    ...
    fn metal_gate_union_quotient_matches_cpu() {
        ...
        let mut shapes = vec![
            ...
            (
                63,
                UnionShape::U32(U32QuotientKind::BaseSum { base: 2 }),
            ),
            (
                31,
                UnionShape::U32(U32QuotientKind::BaseSum { base: 4 }),
            ),
        ];
                ...
                for spec in &u32_specs {
                    ...
                    match spec.kind {
                        ...
                        U32QuotientKind::Selection => {
                            for op in 0..spec.num_ops {
                                let routed = 4 * op;
                                let temporary = 4 * spec.num_ops + op;
                                let selector = wire(routed);
                                let x = wire(routed + 1);
                                let y = wire(routed + 2);
                                let result = wire(routed + 3);
                                let temp = wire(temporary);
                                constraints.push((selector * y - y) - temp);
                                constraints.push((selector * x - temp) - result);
                            }
                            assert_eq!(constraints.len(), spec.num_ops * 2);
                        }
                        U32QuotientKind::BaseSum { base } => {
                            let base_field = F::from_canonical_usize(base);
                            let mut computed = F::ZERO;
                            let mut power = F::ONE;
                            for limb_index in 0..spec.num_ops {
                                let limb = wire(1 + limb_index);
                                computed += limb * power;
                                power *= base_field;
                            }
                            ...
                            for limb_index in 0..spec.num_ops {
                                ...
                                constraints.push(
                                    (0..base)
                                        .map(|digit| limb - F::from_canonical_usize(digit))
                                        .product(),
                                );
                            }
