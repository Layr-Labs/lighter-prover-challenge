...
const SHADER_SOURCE_SHA256: &str =
    "0b9bc3f1c198d38306478480b71d5c25d4318f9f0b7079bb655b2cc5ab4c1a5c";
...
impl MetalShared {
    ...
    fn start_poseidon2_gate_quotient<F: RichField>(
        ...
        let command_buffer = autoreleasepool(|| -> CommandBuffer {
            ...
            dispatch_quotient(encoder, pipeline, quotient_rows);
            encoder.end_encoding();
            #[cfg(feature = "diagnostic_profile")]
    ...
    fn start_permutation_quotient<F: RichField>(
        ...
        let command_buffer = autoreleasepool(|| -> CommandBuffer {
            ...
            dispatch_permutation(encoder, pipeline, quotient_rows);
            encoder.end_encoding();
            #[cfg(feature = "diagnostic_profile")]
            profile_command_buffer(
                command_buffer,
...
///
/// Leaf and parent hashing stay at 128 after this account's `458be2b`
/// (absorb+leaf+parent at 256) rejected at 32.082. Default 256 is the
/// absorb geometry that printed official 32.973 (`75e59a0`). 512 (`8ac9b2f`)
/// printed 32.193 and is not retried as the default. `LIGHTER_HASH_TG=128`
/// restores the tip; `LIGHTER_HASH_TG=512` / `1024` remain available for an
/// isolated retry. Range/U32/Poseidon2-gate quotients use
/// [`quotient_threadgroup_cap`], not this absorb cap.
...
/// Threadgroup cap for `permutation_quotient` only.
///
/// Occupancy probe on this device: every kernel including
/// `permutation_quotient` reports `max_total_threads_per_threadgroup = 1024`,
/// so the previous 128 clamp was software, not a PSO ceiling. i34-9 named this
/// the hottest of the three quotient kernels (`num_routed_wires ×
/// quotient_rows`). Absorb-only 256 printed 32.973; permutation occupancy 256
/// is kept after `d494bcf` (31.542, slow-pool). Range/U32/gate use
/// [`quotient_threadgroup_cap`]. `LIGHTER_PERM_TG=128` restores the prior clamp.
fn permutation_threadgroup_cap() -> u64 {
    static CAP: OnceLock<u64> = OnceLock::new();
    *CAP.get_or_init(|| {
        std::env::var("LIGHTER_PERM_TG")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .filter(|&n| (32..=1024).contains(&n) && n.is_power_of_two())
            .unwrap_or(256)
...
fn dispatch_permutation(
    ...
    dispatch_with_group_cap(
        encoder,
        pipeline,
        thread_count,
        permutation_threadgroup_cap(),
    );
}

/// Threadgroup cap for `poseidon2_gate_quotient` and `range_check_gate_quotient`
/// (the latter also covers U32). Separate from [`permutation_threadgroup_cap`]
/// so a bad draw can restore 128 here without touching permutation or absorb.
/// Occupancy probe: both kernels report `max_threads=1024`. Leaf/parent stay
/// on [`dispatch`]'s 128. `LIGHTER_QUOT_TG=128` restores the prior clamp.
fn quotient_threadgroup_cap() -> u64 {
    static CAP: OnceLock<u64> = OnceLock::new();
    *CAP.get_or_init(|| {
        std::env::var("LIGHTER_QUOT_TG")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .filter(|&n| (32..=1024).contains(&n) && n.is_power_of_two())
            .unwrap_or(256)
