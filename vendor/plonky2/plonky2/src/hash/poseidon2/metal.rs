...
impl EnqueuedSpineCommand {
    fn new(queue: &CommandQueue) -> Self {
        ...
        #[cfg(feature = "diagnostic_profile")]
        let profile_sequence =
            PROFILE_SPINE_RESERVATION_SEQUENCE.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        #[cfg(feature = "diagnostic_profile")]
        crate::util::profile::counter("metal_enqueue", "reservation_sequence", profile_sequence);
        Self {
        ...
        std::thread::scope(|scope| {
            ...
            loop {
                ...
                assert!(
                    ...
                    "light spine did not reserve queue"
                );
                std::thread::yield_now();
            }
    ...
    fn preparing_station_owner_never_gets_an_early_spine_reservation() {
        ...
        std::thread::scope(|scope| {
            ...
            loop {
                ...
                assert!(
                    Instant::now() < deadline,
                    "spine did not wait for preparing owner"
                );
                std::thread::yield_now();
            }
            context.release_set(held);
