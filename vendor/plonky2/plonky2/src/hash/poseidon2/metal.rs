...
impl<F: RichField> MetalColumns<F> {
    ...
    /// Returns adjacent physical columns as one read-only strided view without
    /// copying them into a tightly packed scratch buffer.
    pub(crate) fn strided_columns(
        &self,
        columns: core::ops::Range<usize>,
    ) -> Option<&[F]> {
        if columns.start > columns.end || columns.end > self.cols {
            return None;
        }
        if columns.is_empty() {
            return Some(&[]);
        }
        let start = columns.start.checked_mul(self.rows)?;
        let end = columns.end.checked_mul(self.rows)?;
        // SAFETY: `start..end` is bounded by the validated column range, and
        // the shared allocation contains `rows * cols` initialized words.
        Some(unsafe { slice::from_raw_parts((self.base as *const F).add(start), end - start) })
    }
