...
/// Ask the scheduler to keep Metal pipeline-lowering threads on
/// performance cores (QOS_CLASS_USER_INTERACTIVE). Scheduling only:
/// kernels and proof values are unchanged. Best-effort on failure.
#[cfg(target_os = "macos")]
fn mark_metal_compile_latency_critical() {
...
fn spawn_optional_pipelines(device: &Device, library: &metal::Library) {
    for (name, slot) in [
        ...
        let spawned = std::thread::Builder::new()
            ...
            .spawn(move || {
                mark_metal_compile_latency_critical();
