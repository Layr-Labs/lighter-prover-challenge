...
fn gpu_worthwhile(leaf_width: usize, leaf_count: usize, cap_height: usize) -> bool {
    ...
    if serial_critical_shape {
        // With the chain measurably lagging (spine_urgent), spine builds hold
        // buffer-set priority, so the queue wait this occupancy check used to
        // dodge is bounded by a single in-flight build — the ~7.8 ms GPU
        // build beats the ~15 ms CPU fallback again even with the stream
        // momentarily busy.
        return exclusive
            || leaf_width > 64
            || spine_urgent()
    ...
    // Deep-queue demotion for the pipelined narrow trees (Z/partial-product
    // and quotient commits, width <= 64): with a deep proof window the GPU
    // stream backs up (~150-400 ms of queue ahead) while CPU cores idle; the
    // ~tens-of-ms parallel CPU build wins whenever several jobs are already
    // queued. Wide wires trees stay on the GPU unconditionally — their CPU
    // build is expensive enough to starve the pipeline's other CPU work.
    if !exclusive
        && leaf_width <= 64
        && GPU_JOBS_IN_FLIGHT.load(core::sync::atomic::Ordering::Relaxed) >= 3
    {
        return false;
    }
