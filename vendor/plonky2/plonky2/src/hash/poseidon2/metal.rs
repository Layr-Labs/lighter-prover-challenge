...
"#;
    ...
    fn shared_column_hash_matches_staged_full_tree_and_paths() {
        ...
        for (rows, cap_height) in [(32usize, 0usize), (256, 3), (1024, 10)] {
            for cols in [1usize, 4, 5, 8, 9, 16, 17, 31] {
                ...
                assert_tree_raw_eq(&direct, &staged, rows, cols, cap_height);
    ...
    fn assert_tree_raw_eq(
        actual: &MerkleBuild<GoldilocksField>,
        expected: &MerkleBuild<GoldilocksField>,
        rows: usize,
        width: usize,
        cap_height: usize,
    ) {
        let actual_digests = actual.0.recursive_digests(rows, cap_height);
        let expected_digests = expected.0.recursive_digests(rows, cap_height);
        assert_eq!(actual_digests.len(), expected_digests.len());
        assert_eq!(actual.1.len(), expected.1.len());
        for (index, (actual, expected)) in actual_digests
            .iter()
            .chain(&actual.1)
            .zip(expected_digests.iter().chain(&expected.1))
            .enumerate()
        {
