...
/// 2^17 spine trees take priority for the single GPU buffer set. Every
/// `spine_backlog_add(-1)` is paired with an earlier `spine_backlog_add(1)`
/// on the spawning thread, so the counter never goes negative and a
/// threshold of 0 leaves the lane open throughout. The counter and the
/// threshold are retained as the tuning point rather than deleted.
static SPINE_BACKLOG: core::sync::atomic::AtomicIsize = core::sync::atomic::AtomicIsize::new(0);
const SPINE_URGENT_BACKLOG: isize = 0;
