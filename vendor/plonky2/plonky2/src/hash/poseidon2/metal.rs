...
pub(crate) fn build_merkle_tree_shared_streamed<F: RichField>(
    ...
    // Group-wise fill + absorb. The final 2^20+ trees prepare two eight-column
    // sponge groups together: sixteen independent FFT columns expose enough
    // Rayon work to occupy all twelve P-cores, and both absorb dispatches share
    // one command buffer. Smaller pipelined 2^19 trees keep their promoted
    // eight-column scheduling exactly. Commands on one queue remain ordered,
    // and every prepared span is committed before the next fill starts.
    let groups = leaf_width.div_ceil(8);
    let groups_per_fill = if leaf_count >= 1 << 20 { 2 } else { 1 };
