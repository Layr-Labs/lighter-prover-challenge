...
fn gpu_worthwhile(leaf_width: usize, leaf_count: usize, cap_height: usize) -> bool {
    ...
    if serial_critical_shape {
        ...
        // queues behind another when every set is busy; one in flight still
        // leaves a free slot, so the head-of-line wait that motivated the CPU
        // fallback does not apply yet.
        return exclusive
            || leaf_width > 64
