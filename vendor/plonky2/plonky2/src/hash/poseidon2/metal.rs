...
/// kernel time. Trees below this bound go through the CPU path instead. The threshold value keeps
/// the upstream 2^19, which was validated on the ranked host (independently measured at 2^18 on an
/// M4 base; both route the pathological small trees to the CPU).
