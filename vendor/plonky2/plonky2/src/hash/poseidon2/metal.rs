...
pub fn prewarm() {
    std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            // hits COLUMN_STORE_POOL — not PREWARMED_LARGE_STORE. Walk it
            // here after the context is up, at UTILITY, so the first LDE
            // fill does not zero-fault 544 MiB on the critical path.
            // Compile QoS stays 0x19 above; only the walk drops.
            if let Some(context) = shared_context() {
                ...
                let buffer = autoreleasepool(|| {
                    context
                        .device
                        .new_buffer(TX_WIRES_STORE_BYTES, MTLResourceOptions::StorageModeShared)
                });
                let base = buffer.contents().cast::<u8>();
                if !base.is_null() {
                    const PAGE: isize = 16 * 1024;
