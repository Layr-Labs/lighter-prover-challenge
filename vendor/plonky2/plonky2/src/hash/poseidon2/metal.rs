...
fn build_archived_pipelines(
    ...
    // Construct all eleven states before publishing any of them. A single
    // miss discards the whole set and selects the exact generic 59c fallback.
    Ok(ArchivedPipelines {
        ...
        leaf_colmajor_hot: pipeline_from_archive(
            device,
            library,
            archive,
            "poseidon2_hash_leaves_colmajor_hot",
...
fn try_archived_pipelines(device: &Device) -> Result<Option<ArchivedPipelines>, String> {
    ...
    build_archived_pipelines(device, &library, &archive).map(Some)
}
...
///
/// The hot-width leaf pipeline is deliberately absent here. It is enabled only
/// when the archive supplies all eleven pipelines; an archive miss must retain
/// the exact ten-pipeline 59c cold-start cost.
///
...
fn spawn_optional_pipelines(device: &Device, library: &metal::Library) {
    ...
    // Production never lowers this pipeline after an archive miss. Tests still
    // build it so the existing width-20/136 differential cases exercise the
    // specialization even when no generated archive was embedded in the test.
    #[cfg(test)]
    let optional = [
        ("poseidon2_gate_quotient", &POSEIDON_GATE_QUOTIENT_PIPELINE),
        (
...
fn activate_archived_pipelines(pipelines: ArchivedPipelines) -> RequiredPipelines {
    ...
    // eleven archive probes have already succeeded before the first publish.
...
fn build_generic_pipelines(device: &Device) -> Result<RequiredPipelines, String> {
    ...
    // differential tests still need to execute it without depending on a
    // build.rs archive, so only their optional builders use the hot library.
    #[cfg(test)]
    {
        let hot_library = device
            .new_library_with_data(SHADER_METALLIB)
            .map_err(|error| format!("loading hot test metallib failed: {error}"))?;
        spawn_optional_pipelines(device, &hot_library);
    }
    Ok(pipelines)
}
...
/// archive APIs. The context performs the eleven strict probes synchronously
/// and publishes the hot optional pipeline before it becomes ready.
...
impl MetalShared {
    fn new() -> Result<Self, String> {
        autoreleasepool(|| {
            ...
            // Archive activation is all-or-nothing. Eleven strict hits use the
            // archive-matched hot library and synchronously publish every
            // optional state; any failure discards it and builds the exact 59c
            // generic library without ever requesting the hot pipeline.
            ...
            let (
                ...
                Some(pipelines) => activate_archived_pipelines(pipelines),
