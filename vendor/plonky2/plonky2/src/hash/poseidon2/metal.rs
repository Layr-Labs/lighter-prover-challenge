...
/// and promoted arithmetic gate, applies each gate's selector filter, and
...
mod tests {
    ...
    // All-kind differential for every promoted gate family in one union job.
    // Wire and raw local-constant columns mix random canonical values with the
    // packed-field suite's twelve raw boundary representatives, including
    // noncanonical encodings at and above the field order.
    #[test]
    fn metal_all_promoted_gate_quotients_match_cpu() {
        type F = GoldilocksField;
        const WIRE_COLUMNS: usize = 136;
        const QUOTIENT_ROWS: usize = 64;
        ...
        // Sixteen selector columns are followed by two raw local constants.
        // Equality reads raw constant zero; the RA4/RA6 layouts use both.
        const RAW_CONSTANT_BASE: usize = 16;
