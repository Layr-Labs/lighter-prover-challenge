...
///
/// The chain tail is a strictly serial drain of ~34 steps, each completing
/// three to four tree builds back-to-back (wires, Zs, quotient). With two
/// slots, the third completion in a row copies its digest inline while
/// holding the exclusive buffer set, delaying the next build's staging. The
/// tail is the single most serial window in the run; three slots cover the
/// longest run of consecutive completions without ever overlapping a GPU
/// build (the builds are serialized by the single set regardless).
...
pub(crate) fn build_merkle_tree_shared_streamed<F: RichField>(
    ...
    // Exclusive serial phases own an otherwise idle GPU stream (the pipelined
    // chunk trees have retired), so the 2^17-leaf chain-step Zs and quotient
    // trees can stream their CPU fill against GPU absorb instead of running
    // fill-then-hash serially on the spine. The old 2^20 gate matched no
    // ranked exclusive shape; the measured GPU/CPU break-even for the
    // mid-size shapes is far below 2^17.
