...
/// the worker actually ends up with. See [`PREFER_PREBUILT_METALLIB`].
///
...
/// Whether to accept [`SHADER_METALLIB`] when it loads.
///
/// FALSE, and this is load-bearing rather than a preference. A binary archive
/// is keyed on the *compiled function*, so it only hits for the library the
/// worker actually ends up with, and [`PIPELINE_ARCHIVE`] is built by compiling
/// `SHADER_SOURCE`. Accepting the prebuilt metallib on a host whose OS can load
/// it would swap in a different AIR, every archive lookup would miss, and --
/// because production must never set `FailOnBinaryArchiveMiss` -- the miss
/// would be SILENT: full cold lowering, a correct proof, a green verifier.
///
/// The committed metallib is in any case dead on macOS 15.x, where
/// `newLibraryWithData:` rejects it with "This library is using language
/// version 4.0 which is not supported on this OS" and the loader already falls
/// through to the source compile. Measured cost of that compile with the cache
/// denied: 185 ms, against the 1628 ms of lowering the archive removes.
///
/// Flip to `true` to restore the old preference; nothing else needs to change.
const PREFER_PREBUILT_METALLIB: bool = false;
...
impl MetalShared {
    fn new() -> Result<Self, String> {
        autoreleasepool(|| {
            ...
            // forget.
            let library = PREFER_PREBUILT_METALLIB
