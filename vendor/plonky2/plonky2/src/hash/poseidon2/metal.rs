...
/// Frontier #148's complete shader library, retained byte-for-byte so its
/// recorded M4 pipeline archive can continue to serve every unchanged kernel.
const SHADER_METALLIB: &[u8] = include_bytes!("poseidon2.metallib");

/// The changed Range/U32 kernel extracted from the candidate AIR. Keeping it in
/// a second library makes only this pipeline miss the frontier archive instead
/// of invalidating the other eleven archived pipelines with a new metallib.
const RANGE_SHADER_METALLIB: &[u8] = include_bytes!("poseidon2-range.metallib");

/// SHA-256 of the `poseidon2.metal` bytes [`RANGE_SHADER_METALLIB`] was built
/// from. The exact source pin is enforced by `metallib_matches_shader_source`.
const SHADER_SOURCE_SHA256: &str =
    "4f08685bab74b783e75c7e4348fbaad583529df640afc983bbf23367186078a0";
...
fn spawn_optional_pipelines(
    ...
    for (name, slot) in [
        ...
        let library = if name == "range_check_gate_quotient" {
            range_library.clone()
        } else {
            library.clone()
        };
...
impl MetalShared {
    fn new() -> Result<Self, String> {
        autoreleasepool(|| {
            ...
            //
            // The base metallib is deliberately the exact frontier artifact:
            // its M4 archive can still serve the eleven unchanged pipelines.
            // The modified Range/U32 kernel comes from a one-function delta
            // metallib and is the only intentional archive miss. If either AIR
            // artifact is rejected, compiling the complete candidate source is
            ...
            let (library, range_library) = if let Some(library) = prebuilt_library {
                let range_library = device
                    .new_library_with_data(RANGE_SHADER_METALLIB)
                    .ok()
                    .filter(|library| {
                        library
                    ...
                    .map_or_else(
                        || {
                            device
                                ...
                                .map_err(|error| {
                                    format!("range shader compilation failed: {error}")
                                })
                        },
                        Ok,
                    )?;
                (library, range_library)
            } else {
                let library = device
                    .new_library_with_source(SHADER_SOURCE, &options)
            ...
            spawn_optional_pipelines(&device, &library, &range_library, archive.as_ref());
...
mod tests {
    ...
    /// The Range/U32 delta library is only sound while it is the compiled form
    /// of the MSL we ship. Nothing in the type system ties the two together, so
    /// this pins the source bytes and fails loudly on a stale delta artifact.
    #[test]
    fn metallib_matches_shader_source() {
        ...
        assert_eq!(
            ...
            "poseidon2.metal changed but poseidon2-range.metallib was not regenerated. Run:\n  \
             xcrun -sdk macosx metal -c poseidon2.metal -o poseidon2.air\n  \
             xcrun -sdk macosx metallib poseidon2.air --function-name \
             range_check_gate_quotient -o poseidon2-range.metallib\n\
    ...
    fn metallib_loads_and_exposes_every_kernel() {
        ...
        let range_library = device
            .new_library_with_data(RANGE_SHADER_METALLIB)
            .expect("Range/U32 delta metallib must load");
        assert!(
            range_library
                .get_function("range_check_gate_quotient", None)
                .is_ok(),
            "Range/U32 delta metallib is missing its kernel"
        );
    }
