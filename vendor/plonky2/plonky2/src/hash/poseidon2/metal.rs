...
/// parallel, while the remaining five lower normally on detached builders.
#[cfg(test)]
...
/// states are published only after all six strict probes join, but retaining
/// these owners with [`MetalShared`] avoids any driver-side lifetime ambiguity
...
struct MetalShared {
    ...
    /// process lifetime. All six strict archive probes have joined before this
    /// object is published; later optional lowering never calls archive APIs.
...
fn activate_archived_pipelines(
    ...
    // six-way join above is the final archive API use in this process.
...
/// part of this requirement. The benchmark explicitly guarantees that its
/// exact proof-output path is absent and writable at worker launch.
pub fn prewarm_with_required_archive(bytes: &'static [u8], archive_output_path: &Path) {
    ...
    if !archive_output_path.is_absolute() || !parent_is_directory || !output_is_absent {
        abort_required_archive(
            "the exact proof-output path is not absent and absolute under an existing directory",
        );
    }
