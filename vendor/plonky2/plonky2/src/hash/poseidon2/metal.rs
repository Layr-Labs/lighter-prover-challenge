...
struct MetalShared {
    ...
    ntt_stage_radix4_pipeline: ComputePipelineState,
    quotient_accumulate_pipeline: ComputePipelineState,
...
/// Adds every completed specialized quotient job to the CPU survivor values
/// and transposes the two challenge lanes to final column-major order on the
/// GPU. This replaces three host read/add traversals plus the point-to-column
/// scatter with one streaming device pass.
pub(crate) fn accumulate_quotient_columns<F: RichField>(
    cpu_values: &[F],
    denominator_inverses: &[F],
    poseidon: Option<&PoseidonGateQuotientJob<F>>,
    range: Option<&RangeCheckGateQuotientJob<F>>,
    permutation: Option<&PermutationQuotientJob<F>>,
) -> Result<Vec<Vec<F>>, String> {
    let rows = cpu_values.len() / 2;
    if F::ORDER != 0xffff_ffff_0000_0001
        || size_of::<F>() != size_of::<u64>()
        || rows == 0
        || cpu_values.len() != rows * 2
        || rows > u32::MAX as usize
        || denominator_inverses.is_empty()
        || !denominator_inverses.len().is_power_of_two()
        || denominator_inverses.len() > rows
        || [
            poseidon.map(|job| job.len),
            range.map(|job| job.len),
            permutation.map(|job| job.len),
        ]
        .into_iter()
        .flatten()
        .any(|len| len != cpu_values.len())
    {
        return Err("invalid quotient accumulator inputs".to_string());
    }
    let context = shared_context().ok_or("Metal quotient accumulator unavailable")?;
    context.accumulate_quotient_columns(
        cpu_values,
...
impl MetalShared {
    fn new() -> Result<Self, String> {
        autoreleasepool(|| {
            ...
            let (
                ...
                ntt_stage_pipeline,
                ntt_stage_radix4_pipeline,
                quotient_accumulate_pipeline,
                ...
                let ntt_stage_radix4 =
                    scope.spawn(required("ntt_stage_radix4", "radix-4 ntt stage"));
                let quotient_accumulate = scope.spawn(required(
                    "quotient_accumulate_colmajor",
                ...
                (
                    ...
                    ntt_stage_radix4
                        .join()
                        .expect("radix-4 ntt stage pipeline thread panicked"),
                    quotient_accumulate
                        .join()
                        .expect("quotient accumulator pipeline thread panicked"),
                    ifft_finalize
                        .join()
            ...
            let ntt_stage_radix4_pipeline = ntt_stage_radix4_pipeline?;
            let quotient_accumulate_pipeline = quotient_accumulate_pipeline?;
    ...
    fn accumulate_quotient_columns<F: RichField>(
        &self,
        cpu_values: &[F],
        denominator_inverses: &[F],
        poseidon: Option<&PoseidonGateQuotientJob<F>>,
        ...
        let rows = cpu_values.len() / 2;
        let bytes = size_of_val(cpu_values) as u64;
        let cpu_buffer = autoreleasepool(|| {
            self.device.new_buffer_with_data(
                cpu_values.as_ptr().cast::<c_void>(),
                bytes,
                MTLResourceOptions::StorageModeShared,
            )
        });
        let denominator_buffer = autoreleasepool(|| {
            self.device.new_buffer_with_data(
                denominator_inverses.as_ptr().cast::<c_void>(),
                size_of_val(denominator_inverses) as u64,
                MTLResourceOptions::StorageModeShared,
            )
        });
        let output = self.acquire_quotient_output(bytes);
        let poseidon_buffer = poseidon
            .and_then(|job| job.output.as_ref())
            .unwrap_or(&cpu_buffer);
        let range_buffer = range
            .and_then(|job| job.output.as_ref())
            .unwrap_or(&cpu_buffer);
        let permutation_buffer = permutation
            .and_then(|job| job.output.as_ref())
            .unwrap_or(&cpu_buffer);
        let contribution_mask = u32::from(poseidon.is_some())
            | (u32::from(range.is_some()) << 1)
            | (u32::from(permutation.is_some()) << 2);

        let command_buffer = autoreleasepool(|| -> CommandBuffer {
            let command_buffer = self.queue.new_command_buffer();
            let encoder = command_buffer.new_compute_command_encoder();
            encoder.set_compute_pipeline_state(&self.quotient_accumulate_pipeline);
            encoder.set_buffer(0, Some(&cpu_buffer), 0);
            encoder.set_buffer(1, Some(poseidon_buffer), 0);
            encoder.set_buffer(2, Some(range_buffer), 0);
            encoder.set_buffer(3, Some(permutation_buffer), 0);
            encoder.set_buffer(4, Some(&denominator_buffer), 0);
            encoder.set_buffer(5, Some(&output), 0);
            set_u32(encoder, 6, rows as u32);
            set_u32(encoder, 7, denominator_inverses.len() as u32 - 1);
            set_u32(encoder, 8, contribution_mask);
            dispatch(encoder, &self.quotient_accumulate_pipeline, rows);
            encoder.end_encoding();
            #[cfg(feature = "diagnostic_profile")]
            profile_command_buffer(command_buffer, "quotient_accumulate", rows as u64);
            command_buffer.commit();
            command_buffer.to_owned()
        });
        command_buffer.wait_until_completed();
        if command_buffer.status() != MTLCommandBufferStatus::Completed {
            return Err(format!(
                "quotient accumulator command buffer ended with status {:?}",
                command_buffer.status()
            ));
        }

        // SAFETY: the completed kernel wrote two canonical field columns of
        // exactly `rows` elements each to the `2 * rows` output words.
        let source = unsafe { slice::from_raw_parts(output.contents().cast::<F>(), rows * 2) };
        let columns = source
            .par_chunks(rows)
...
/// Encodes `[start_stage, end_stage)` with radix-4 passes where possible and
/// one radix-2 tail when the stage count is odd. All passes share one compute
/// encoder; explicit barriers preserve the read-after-write dependency on the
/// in-place column buffer without paying an encoder boundary per FFT layer.
#[allow(clippy::too_many_arguments)]
fn encode_ntt_stages(
    ...
    debug_assert!(start_stage < end_stage);
    let values_resource: &metal::ResourceRef = values;
    let mut stage = start_stage;
    let mut dispatched = false;

    while stage + 1 < end_stage {
        if dispatched {
            encoder.memory_barrier_with_resources(&[values_resource]);
        }
        encoder.set_compute_pipeline_state(&context.ntt_stage_radix4_pipeline);
        ...
        set_u32(
            encoder,
            5,
            u32::from(canonicalize_final && stage + 2 == end_stage),
        );
        dispatch2d(encoder, &context.ntt_stage_radix4_pipeline, n / 4, cols);
        stage += 2;
        dispatched = true;
    }
    ...
    if stage < end_stage {
        ...
        encoder.set_compute_pipeline_state(&context.ntt_stage_pipeline);
        encoder.set_buffer(0, Some(values), 0);
        encoder.set_buffer(
            1,
            Some(roots),
        ...
        set_u32(encoder, 2, n as u32);
        set_u32(encoder, 3, stage);
        set_u32(
            encoder,
            4,
            u32::from(canonicalize_final && stage + 1 == end_stage),
        );
        dispatch2d(encoder, &context.ntt_stage_pipeline, n / 2, cols);
    }
}
...
mod tests {
    ...
    #[test]
    fn quotient_accumulator_matches_field_add_scale_and_transpose() {
        type F = GoldilocksField;
        let Some(device) = Device::system_default() else {
            return;
        };
        ...
        let rows = 8usize;
        let cpu = (0..rows * 2)
            .map(|i| F::from_canonical_usize(100 + i))
            .collect::<Vec<_>>();
        let poseidon_values = (0..rows)
            .flat_map(|i| {
                [
                    F::from_canonical_usize(i + 1),
                    F::from_canonical_usize(2 * i + 3),
                ]
            })
            .collect::<Vec<_>>();
        let range_values = (0..rows)
            .flat_map(|i| {
                [
                    F::from_canonical_usize(3 * i + 5),
                    F::from_canonical_usize(i + 7),
                ]
            })
            .collect::<Vec<_>>();
        let make_buffer = |values: &[F]| {
            autoreleasepool(|| {
        ...
        let pool = Arc::new(Mutex::new(QuotientOutputPool::default()));
        let poseidon = PoseidonGateQuotientJob {
            command_buffer: completed(),
            output: Some(make_buffer(&poseidon_values)),
            output_pool: Arc::clone(&pool),
            len: rows * 2,
            _job: GpuJobGuard::begin(),
            _phantom: PhantomData,
        };
        let range = RangeCheckGateQuotientJob {
            command_buffer: completed(),
            output: Some(make_buffer(&range_values)),
            output_pool: Arc::clone(&pool),
            len: rows * 2,
            failure_observer: None,
            _job: GpuJobGuard::begin(),
        ...
        let columns = accumulate_quotient_columns(
            &cpu,
            &denominator_inverses,
            Some(&poseidon),
            Some(&range),
        ...
        .expect("quotient accumulator must run");

        assert_eq!(columns.len(), 2);
        for i in 0..rows {
            let scale = denominator_inverses[i & (denominator_inverses.len() - 1)];
            assert_eq!(
                columns[0][i],
                cpu[2 * i] + (poseidon_values[2 * i] + range_values[2 * i]) * scale
            );
            assert_eq!(
                columns[1][i],
                cpu[2 * i + 1] + (poseidon_values[2 * i + 1] + range_values[2 * i + 1]) * scale
            );
        }
    }
