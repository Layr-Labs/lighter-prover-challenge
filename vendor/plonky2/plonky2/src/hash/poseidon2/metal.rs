...
// Poseidon, Range/U32, and permutation quotient jobs are simultaneously live
// for every recurring proof, so retain one output for each job.
...
mod tests {
    ...
    fn quotient_output_pool_is_bounded_and_best_fit() {
        ...
        let six = pool.take_best_fit(5 * mib).expect("6 MiB best fit");
