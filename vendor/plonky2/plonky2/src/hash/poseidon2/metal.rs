...
/// Build-host-compiled portable AIR containing a Range/U32 kernel whose full
/// 136-column production wire row is resident in a 16-row, 17 KiB threadgroup
/// tile. Empty on non-Mac builders or when the optional Metal toolchain is
/// unavailable; an empty artifact selects the asynchronous source fallback
/// below. Every established pipeline remains sourced from the checked-in
/// metallib above.
const SHADER_RANGE_TILE_METALLIB: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/poseidon2_range_tile.metallib"));

/// Deterministically generated source for the same tiled kernel. The dedicated
/// Range builder compiles this only when both native-archive and portable-AIR
/// construction were unavailable on the build host. That work stays on the
/// existing detached pipeline thread and overlaps worker startup.
const SHADER_RANGE_TILE_SOURCE: &str =
    include_str!(concat!(env!("OUT_DIR"), "/poseidon2_range_tile.metal"));
...
fn spawn_optional_pipelines(
    ...
    // Lower exactly one Range/U32 pipeline. Prefer the build-time native/AIR
    // tiled library. If that artifact is absent or incompatible, compile the
    // identical generated source on this already-detached builder thread, so
    // its front end overlaps startup rather than blocking MetalShared::new.
    // Only if both tiled paths fail do we restore the checked-in kernel.
    let device = device.clone();
    ...
    let spawned = std::thread::Builder::new()
        ...
        .spawn(move || {
            let (pipeline, tiled) = autoreleasepool(|| {
                let tiled_library = range_tiled_library.or_else(|| {
                    let options = CompileOptions::new();
                    device
                        .new_library_with_source(SHADER_RANGE_TILE_SOURCE, &options)
                        .ok()
                });
                let tiled = tiled_library.as_ref().and_then(|library| {
                    library
...
impl MetalShared {
    ...
    fn start_range_check_gate_quotient<F: RichField>(
        ...
        if tiled && quotient_rows % 16 != 0 {
            ...
            return Err("tiled Range/U32 quotient rows must be divisible by 16".to_owned());
        }
...
/// The tiled Range/U32 kernel's static 136-column allocation holds sixteen
/// rows. A full 32-lane SIMDgroup cooperatively fills it before the first
/// sixteen lanes evaluate one row each. Keep this dispatch shape identical to
/// the generated Metal source.
fn dispatch_range_tiled(encoder: &metal::ComputeCommandEncoderRef, thread_count: usize) {
    debug_assert_eq!(thread_count % 16, 0);
    encoder.dispatch_threads(
        MTLSize {
            width: (thread_count * 2) as NSUInteger,
            height: 1,
            depth: 1,
        },
        MTLSize {
