...
/// Streamed shared-column Merkle build: `fill_group(col_start, slices)` computes
/// the LDE columns `[col_start, col_start + slices.len())` directly in the
/// shared buffer. CPU fill width is 16 so Rayon can use all 14 M4 Pro cores
/// on the FFT/coset work; each 16-col fill still feeds two 8-col GPU absorb
/// passes (`poseidon2_absorb_pass` shader ABI unchanged: chunk <= 8). The GPU
/// absorbs each 8-col group while the CPU fills the next 16. Only used inside
...
pub(crate) fn build_merkle_tree_shared_streamed<F: RichField>(
    ...
    // Fill-width-16 CPU LDE + two 8-col GPU absorb passes. The CPU fill of
    // the next 16 columns overlaps the GPU's absorption of the previous
    // fill's two 8-col passes: commands on one queue execute in submission
    // order, and each pass is committed before the next fill starts.
    // GPU shader ABI is unchanged (chunk <= 8, col_start, is_first, is_last).
    const FILL_WIDTH: usize = 16;
    let mut fill_start = 0usize;
    let total_groups = leaf_width.div_ceil(8);
    let base = columns.buffer.contents().cast::<F>();
    let mut absorb_commands: Vec<CommandBuffer> = Vec::with_capacity(total_groups);
...
"#;
    ...
    fn streamed_merkle_keeps_digests_resident_and_matches_classic() {
        ...
        let rows = 1usize << 20;
        // 17 columns is three absorb groups (CPU fill 16 then 1) with a
        // *partial* final group, so the group that now carries the parent
        // ladder is the short one.
        let cols = 17;
        let cap_height = 4;
        let columns = context
