...
pub(crate) fn build_merkle_tree_shared_streamed<F: RichField>(
    ...
    for group in 0..groups {
        ...
        let command_buffer = autoreleasepool(|| -> CommandBuffer {
            ...
            // Absorb-only occupancy: 512 default; leaf/parent stay on `dispatch` (128).
...
/// this bisect unclamps only that kernel. Default 512 is the remaining
/// doubling between the 256 draw (`75e59a0`, official 32.973 vs 33.004) and
/// the 1024 ceiling that lost locally (48 s vs 43 s). `LIGHTER_HASH_TG=128`
/// restores the tip; `LIGHTER_HASH_TG=256` restores the previous ranked draw;
/// `LIGHTER_HASH_TG=1024` takes the PSO ceiling.
