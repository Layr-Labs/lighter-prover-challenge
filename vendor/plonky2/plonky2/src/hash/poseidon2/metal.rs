...
/// width-generic integer, byte, quintic, audited random-access, and native
/// arithmetic gate, applies each selector filter, and reduces the shared
/// constraint rows with the same two alpha challenges as the CPU quotient.
...
mod tests {
    ...
    fn metal_byte_and_quintic_gate_quotient_matches_cpu() {
        ...
        // one random-invalid job contains every promoted kind, including all
        // audited RandomAccess tuples and the native arithmetic families.
        const PROMOTED_SHAPE_COUNT: usize = 16;
        let raw_constant_base = PROMOTED_SHAPE_COUNT;
        let mut shapes = vec![
            (
                3,
                    ...
                    match spec.kind {
                        ...
                        U32QuotientKind::Reducing { extension_coeffs } => {
                            ...
                            for i in 0..spec.num_ops {
                                ...
                                constraints.push(
                                    acc_0 * alpha_1 + acc_1 * alpha_0 + coeff_1 - next_1,
                                );
                                acc_0 = next_0;
                                acc_1 = next_1;
                            }
