...
fn note_pipeline_settled() {
    ...
    // CANARY (one-shot diagnostic tree — never promote this guard): converts
    // the serve/miss bit above into the submission's visible status field,
    // because ranked stderr is not surfaced. Armed only when an archive
    // directory was registered — the prove binary always registers one, unit
    // tests never do. A worker whose archive does not fully serve aborts here
    // (after the report line goes out), so a `failed` status on this
    // submission means the archive did NOT serve on the ranked host, and any
    // score at all means it DID.
    if PIPELINE_ARCHIVE_DIR.get().is_some()
        && (ARCHIVE_STATE.load(Ordering::Acquire) != ARCHIVE_STATE_SERVES
            || ARCHIVE_MISSES.load(Ordering::Acquire) != 0)
    {
        std::process::abort();
    }
}
