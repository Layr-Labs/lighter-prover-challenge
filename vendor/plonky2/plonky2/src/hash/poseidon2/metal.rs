...
const SHADER_SOURCE_SHA256: &str =
    "0d39712d53c02c9b333cf5da5da14099cd260b73fe82c1b21691c7cdca3a893e";
...
const METALLIB_REQUIRED_KERNELS: [&str; 11] = [
    ...
    "poseidon2_hash_parents_simd2",
    "poseidon2_absorb_pass",
    "poseidon2_absorb_pass_simd2",
    "ntt_prepare",
    "ntt_stage",
    "ifft_finalize",
...
fn spawn_optional_pipelines(device: &Device, library: &metal::Library) {
    for (name, slot) in [
        ...
        ("poseidon2_absorb_pass_simd2", &ABSORB_PASS_PIPELINE),
    ] {
...
pub(crate) fn build_merkle_tree_shared_streamed<F: RichField>(
    ...
    for group in 0..groups {
        ...
        let command_buffer = autoreleasepool(|| -> CommandBuffer {
            ...
            dispatch_simd2(encoder, pipeline, leaf_count);
    ...
    let parents_command = autoreleasepool(|| -> CommandBuffer {
        ...
        while child_count > cap_count {
            ...
            dispatch_simd2(parent_encoder, &context.parent_pipeline, parent_count);
    ...
    fn new() -> Result<Self, String> {
        autoreleasepool(|| {
            ...
            let (
                ...
                let parent = scope.spawn(required("poseidon2_hash_parents_simd2", "parent"));
        ...
        let result = (|| -> Result<TreeReadback<'_, F>, String> {
            ...
            let command_buffer = autoreleasepool(|| -> CommandBuffer {
                ...
                while child_count > cap_count {
                    ...
                    set_u32(parent_encoder, 3, parent_count_u32);
                    dispatch_simd2(parent_encoder, &self.parent_pipeline, parent_count);
                    parent_encoder.end_encoding();
...
fn dispatch_simd2(
    ...
    dispatch(encoder, pipeline, item_count * 2);
}
...
"#;
            ...
            autoreleasepool(|| {
                ...
                let pipelines = |source: &str| {
                    ...
                    (
                        ...
                        pipeline("poseidon2_hash_parents_simd2"),
                    )
                };
                let native_source =
    ...
    #[test]
    fn metal_poseidon2_absorb_simd2_matches_scalar() {
        ...
        let cooperative = pipeline("poseidon2_absorb_pass_simd2");

        let mut rng = StdRng::seed_from_u64(0x5349_4d44_3241_4253);
        ...
        let run = |pipeline: &ComputePipelineState,
            ...
            for group in 0..groups {
                ...
                dispatch(
                    encoder,
                    pipeline,
                    LEAF_COUNT * if cooperative { 2 } else { 1 },
                );
                encoder.end_encoding();
                command.commit();
