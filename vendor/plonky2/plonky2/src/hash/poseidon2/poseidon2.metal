...
//   quintic-multiplication=4, quintic-squaring=5, base-sum=6), operation
//   count, addend count (byte-limb count for byte decomposition, limb count
//   for base-sum, zero except for add-many), base-4 result limbs (the
//   decomposition base, 2 or 4, for base-sum), and carry limbs (zero except
//   for add-many).
...
kernel void range_check_gate_quotient(
    ...
    for (uint u32_index = 0; u32_index < u32_count; ++u32_index) {
        ...
        if (kind == 0u) {
            ...
            uint num_limbs = num_addends;
            ulong word_base_bs = (ulong)result_limbs;
            for (uint op = 0; op < num_ops; ++op) {
                ...
                for (uint remaining = num_limbs - 1u; remaining > 0u; --remaining) {
                    uint j = remaining - 1u;
                    recomposed = gl_add(
                        gl_mul(recomposed, word_base_bs),
