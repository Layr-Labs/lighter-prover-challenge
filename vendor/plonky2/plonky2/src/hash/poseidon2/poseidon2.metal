...
//   equality=7, reducing=8), operation count, addend count (byte-limb count
...
kernel void range_check_gate_quotient(
    ...
    // BaseSumGate<B>: wire 0 is the sum, wires 1..1+num_limbs the base-B
    // limbs. The CPU emits `reduce_with_powers(limbs, B) - sum` — Horner from
    // the most significant limb downward — then one range product
    // `prod_{i < B} (limb - i)` per limb in ascending order.
