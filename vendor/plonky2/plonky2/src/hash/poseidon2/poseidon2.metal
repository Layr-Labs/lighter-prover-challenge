...
// Per-thread L1 of the 136-wire production row. Occupancy-safe: no
// threadgroup, same 128-wide dispatch as the fatlib kernel. The 16x136
// threadgroup tile (exp32) scored 19.29 TPS — 16-wide groups plus 17.4 KiB
// of TG memory collapsed occupancy. This cache is 1088 B/thread and keeps
// the original grid.
constant uint RANGE_ROW_CACHE = 136u;

inline ulong range_row_load(
    thread ulong* row,
    uint cached_cols,
    const device ulong* wires,
    ulong col,
    uint lde_rows,
    uint source_row) {
    if (col < (ulong)cached_cols) {
        return row[(uint)col];
    }
    return wires[col * lde_rows + source_row];
}
...
inline ulong random_access_select_8(
    ...
    for (uint i = 0; i < 8u; ++i) {
        ...
        items[i] = range_row_load(row, cached_cols, wires, column, lde_rows, source_row);
    }
    uint level_size = 8u;
    for (uint level = 0; level < 3u; ++level) {
        ulong b = range_row_load(row, cached_cols, wires, bit_base + level, lde_rows, source_row);
...
kernel void range_check_gate_quotient(
    ...
    constant uint& wire_cols [[buffer(11)]],
    uint gid [[thread_position_in_grid]]) {
    if (gid >= quotient_rows) {
        return;
    }
    ...
    uint source_row = gid * step;
    uint cached_cols = wire_cols < RANGE_ROW_CACHE ? wire_cols : RANGE_ROW_CACHE;
    thread ulong row[136];
    for (uint c = 0; c < cached_cols; ++c) {
        row[c] = wires[(ulong)c * lde_rows + source_row];
    }
    ulong total[2] = { 0, 0 };
    for (uint range_index = 0; range_index < range_count; ++range_index) {
        ...
        for (uint op = 0; op < num_ops; ++op) {
            ulong input = range_row_load(row, cached_cols, wires, (ulong)op, lde_rows, source_row);
            ulong aux_base = (ulong)num_ops + (ulong)num_aux * op;
            ulong computed = range_row_load(row, cached_cols, wires, (aux_base + num_aux - 1u), lde_rows, source_row);
            for (uint remaining = num_aux - 1u; remaining > 0u; --remaining) {
                uint j = remaining - 1u;
                ulong limb = range_row_load(row, cached_cols, wires, (aux_base + j), lde_rows, source_row);
            ...
            for (uint j = 0; j < num_aux; ++j) {
                ulong x = range_row_load(row, cached_cols, wires, (aux_base + j), lde_rows, source_row);
                ulong constraint;
    ...
    for (uint u32_index = 0; u32_index < u32_count; ++u32_index) {
        ...
        if (kind == 0u) {
            ...
            for (uint op = 0; op < num_ops; ++op) {
                ...
                ulong multiplicand_0 = range_row_load(row, cached_cols, wires, (routed_base + 0u), lde_rows, source_row);
                ulong multiplicand_1 = range_row_load(row, cached_cols, wires, (routed_base + 1u), lde_rows, source_row);
                ulong addend = range_row_load(row, cached_cols, wires, (routed_base + 2u), lde_rows, source_row);
                ulong output_low = range_row_load(row, cached_cols, wires, (routed_base + 3u), lde_rows, source_row);
                ulong output_high = range_row_load(row, cached_cols, wires, (routed_base + 4u), lde_rows, source_row);
                ulong inverse = range_row_load(row, cached_cols, wires, (routed_base + 5u), lde_rows, source_row);
                ...
                for (uint remaining = 32u; remaining > 0u; --remaining) {
                    uint j = remaining - 1u;
                    ulong x = range_row_load(row, cached_cols, wires, (limb_base + j), lde_rows, source_row);
                    ulong y = gl_mul(x, gl_sub(x, 3));
                    range_check_gate_emit(
                        gl_mul(y, gl_add(y, 2)),
                        alpha_powers,
            ...
            for (uint op = 0; op < num_ops; ++op) {
                ...
                ulong input_x = range_row_load(row, cached_cols, wires, (routed_base + 0u), lde_rows, source_row);
                ulong input_y = range_row_load(row, cached_cols, wires, (routed_base + 1u), lde_rows, source_row);
                ulong input_borrow = range_row_load(row, cached_cols, wires, (routed_base + 2u), lde_rows, source_row);
                ulong output_result = range_row_load(row, cached_cols, wires, (routed_base + 3u), lde_rows, source_row);
                ulong output_borrow = range_row_load(row, cached_cols, wires, (routed_base + 4u), lde_rows, source_row);
            ...
            for (uint op = 0; op < num_ops; ++op) {
                ...
                ulong computed = range_row_load(row, cached_cols, wires, (routed_base + num_addends), lde_rows, source_row);
                for (uint j = 0; j < num_addends; ++j) {
                    computed = gl_add(
                        computed,
                        range_row_load(row, cached_cols, wires, (routed_base + j), lde_rows, source_row));
                }
                ulong output_result =
                    range_row_load(row, cached_cols, wires, (routed_base + num_addends + 1u), lde_rows, source_row);
                ulong output_carry =
                    range_row_load(row, cached_cols, wires, (routed_base + num_addends + 2u), lde_rows, source_row);
            ...
            for (uint op = 0; op < num_ops; ++op) {
                ...
                for (uint byte_index = 0; byte_index < num_limbs; ++byte_index) {
                    ...
                    ulong recomposed = range_row_load(row, cached_cols, wires, (chunk + 3u), lde_rows, source_row);
                    for (uint remaining = 3u; remaining > 0u; --remaining) {
                        uint k = remaining - 1u;
                        recomposed = gl_add(
                            gl_quadruple(recomposed),
                            range_row_load(row, cached_cols, wires, (chunk + k), lde_rows, source_row));
                    }
                    ulong byte_value =
                        range_row_load(row, cached_cols, wires, (routed_base + 1u + byte_index), lde_rows, source_row);
                    range_check_gate_emit(
                ...
                ulong recomposed_sum =
                    range_row_load(row, cached_cols, wires, (routed_base + num_limbs), lde_rows, source_row);
                for (uint remaining = num_limbs - 1u; remaining > 0u; --remaining) {
                    uint k = remaining - 1u;
                    recomposed_sum = gl_add(
                        gl_mul(recomposed_sum, 256),
                        range_row_load(row, cached_cols, wires, (routed_base + 1u + k), lde_rows, source_row));
                }
                ulong expected_sum = range_row_load(row, cached_cols, wires, routed_base, lde_rows, source_row);
                range_check_gate_emit(
            ...
            for (uint op = 0; op < num_ops; ++op) {
                ...
                ulong b[5];
                for (uint j = 0; j < 5u; ++j) {
                    a[j] = range_row_load(row, cached_cols, wires, (routed_base + j), lde_rows, source_row);
                    b[j] = range_row_load(row, cached_cols, wires, (routed_base + 5u + j), lde_rows, source_row);
                }
                ...
                for (uint k = 0; k < 5u; ++k) {
                    ulong term = k < 4u
                        ...
                        : d[k];
                    ulong c = range_row_load(row, cached_cols, wires, (routed_base + 10u + k), lde_rows, source_row);
            ...
            for (uint op = 0; op < num_ops; ++op) {
                ...
                for (uint j = 0; j < 5u; ++j) {
                    a[j] = range_row_load(row, cached_cols, wires, (routed_base + j), lde_rows, source_row);
                    c[j] = range_row_load(row, cached_cols, wires, (routed_base + 5u + j), lde_rows, source_row);
                }
                for (uint j = 0; j < 10u; ++j) {
                    extra[j] = range_row_load(row, cached_cols, wires, (temp_base + j), lde_rows, source_row);
                }
            ...
            for (uint copy = 0; copy < num_copies; ++copy) {
                ...
                for (uint i = 0; i < bits; ++i) {
                    ulong b = range_row_load(row, cached_cols, wires, (bit_base + (ulong)copy * bits + i), lde_rows, source_row);
                    range_check_gate_emit(
                        gl_mul(b, gl_sub(b, 1)),
                        alpha_powers,
                        alpha_stride,
                ...
                ulong access_index = range_row_load(row, cached_cols, wires, copy_base, lde_rows, source_row);
                range_check_gate_emit(
                ...
                for (uint block = 0; block < block_count; ++block) {
                    block_results[block] = random_access_select_8(
                        row, cached_cols, wires, lde_rows, source_row, list_base, copy_bit_base, block);
                }
                uint level_size = block_count;
                for (uint i = 3u; i < bits; ++i) {
                    ulong b = range_row_load(row, cached_cols, wires, (copy_bit_base + i), lde_rows, source_row);
                    for (uint k = 0; k < level_size / 2u; ++k) {
                ...
                ulong claimed_element = range_row_load(row, cached_cols, wires, (copy_base + 1u), lde_rows, source_row);
                range_check_gate_emit(
            ...
            for (uint i = 0; i < num_extra_constants; ++i) {
                ...
                ulong extra_wire = range_row_load(row, cached_cols, wires, (extra_wire_base + i), lde_rows, source_row);
                range_check_gate_emit(
            ...
            ulong exponent_base = range_row_load(row, cached_cols, wires, (ulong)0, lde_rows, source_row);
            for (uint i = 0; i < num_power_bits; ++i) {
                ulong previous;
                if (i == 0u) {
                    previous = 1;
                } else {
                    ulong last = range_row_load(row, cached_cols, wires, ((ulong)2u + num_power_bits + i - 1u), lde_rows, source_row);
                    previous = gl_mul(last, last);
                }
                ulong current_bit =
                    range_row_load(row, cached_cols, wires, ((ulong)1u + (num_power_bits - i - 1u)), lde_rows, source_row);
                ulong multiplier =
                    gl_add(gl_mul(current_bit, exponent_base), gl_sub(1, current_bit));
                ulong intermediate =
                    range_row_load(row, cached_cols, wires, ((ulong)2u + num_power_bits + i), lde_rows, source_row);
                range_check_gate_emit(
                    gl_sub(gl_mul(previous, multiplier), intermediate),
                    alpha_powers, alpha_stride, gate_accumulators,
                    constraint_index++);
            }
            ulong output_value = range_row_load(row, cached_cols, wires, ((ulong)1u + num_power_bits), lde_rows, source_row);
            ulong final_intermediate =
                range_row_load(row, cached_cols, wires, ((ulong)1u + 2u * num_power_bits), lde_rows, source_row);
            range_check_gate_emit(
            ...
            for (uint op = 0; op < num_ops; ++op) {
                ...
                ulong x = range_row_load(row, cached_cols, wires, (routed_base + 0u), lde_rows, source_row);
                ulong y = range_row_load(row, cached_cols, wires, (routed_base + 1u), lde_rows, source_row);
                ulong equal = range_row_load(row, cached_cols, wires, (routed_base + 2u), lde_rows, source_row);
                ulong temporary_base = (ulong)num_ops * 3u + (ulong)op * 3u;
                ulong difference = range_row_load(row, cached_cols, wires, (temporary_base + 0u), lde_rows, source_row);
                ulong inverse = range_row_load(row, cached_cols, wires, (temporary_base + 1u), lde_rows, source_row);
                ulong product = range_row_load(row, cached_cols, wires, (temporary_base + 2u), lde_rows, source_row);
            ...
            ulong alpha_0 = range_row_load(row, cached_cols, wires, (ulong)2u, lde_rows, source_row);
            ulong alpha_1 = range_row_load(row, cached_cols, wires, (ulong)3u, lde_rows, source_row);
            ulong acc_0 = range_row_load(row, cached_cols, wires, (ulong)4u, lde_rows, source_row);
            ulong acc_1 = range_row_load(row, cached_cols, wires, (ulong)5u, lde_rows, source_row);
            for (uint i = 0; i < num_ops; ++i) {
                uint next_start = (i + 1u == num_ops) ? 0u : acc_start + 2u * i;
                ulong next_0 = range_row_load(row, cached_cols, wires, (ulong)next_start, lde_rows, source_row);
                ulong next_1 = range_row_load(row, cached_cols, wires, ((ulong)next_start + 1u), lde_rows, source_row);

                uint coeff_wire = coeff_start + i * coeff_wires;
                ulong coeff_0 = range_row_load(row, cached_cols, wires, (ulong)coeff_wire, lde_rows, source_row);
                ulong coeff_1 = extension_coeffs != 0u
                    ? range_row_load(row, cached_cols, wires, ((ulong)coeff_wire + 1u), lde_rows, source_row)
                    : 0;
            ...
            for (uint op = 0; op < num_ops; ++op) {
                ...
                ulong addend_0 = range_row_load(row, cached_cols, wires, (wire_base + 0u), lde_rows, source_row);
                ulong addend_1 = range_row_load(row, cached_cols, wires, (wire_base + 1u), lde_rows, source_row);
                ulong output_value = range_row_load(row, cached_cols, wires, (wire_base + 2u), lde_rows, source_row);
            ...
            for (uint remaining = num_ops; remaining > 0u; --remaining) {
                ...
                computed = gl_add(
                    ...
                    range_row_load(row, cached_cols, wires, ((ulong)1u + limb), lde_rows, source_row));
            }
            range_check_gate_emit(
                gl_sub(computed, range_row_load(row, cached_cols, wires, 0, lde_rows, source_row)),
                alpha_powers, alpha_stride, gate_accumulators,
                constraint_index++);
            for (uint limb = 0; limb < num_ops; ++limb) {
                ulong x = range_row_load(row, cached_cols, wires, ((ulong)1u + limb), lde_rows, source_row);
                ulong constraint;
                if (base == 2u) {
                    constraint = gl_mul(x, gl_sub(x, 1));
                } else {
            ...
            for (uint op = 0; op < num_ops; ++op) {
                ulong b = range_row_load(row, cached_cols, wires, ((ulong)(4u * op)), lde_rows, source_row);
                ulong x = range_row_load(row, cached_cols, wires, ((ulong)(4u * op + 1u)), lde_rows, source_row);
                ulong y = range_row_load(row, cached_cols, wires, ((ulong)(4u * op + 2u)), lde_rows, source_row);
                ulong result = range_row_load(row, cached_cols, wires, ((ulong)(4u * op + 3u)), lde_rows, source_row);
                ulong temp = range_row_load(row, cached_cols, wires, ((ulong)(4u * num_ops + op)), lde_rows, source_row);
                range_check_gate_emit(
