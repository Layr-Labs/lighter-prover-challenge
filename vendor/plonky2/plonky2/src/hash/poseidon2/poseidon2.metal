...
// Two consecutive sponge absorption passes with the intermediate 12-lane
// state kept thread-local. The column and digest layouts are identical to
// poseidon2_absorb_pass; only the state-buffer round trip between the two
// groups is removed.
kernel void poseidon2_absorb_pair_pass(
    const device ulong* leaves [[buffer(0)]],
    device ulong* state [[buffer(1)]],
    device ulong* hashes [[buffer(2)]],
    constant ulong* parameters [[buffer(3)]],
    ...
    constant uint& log_leaf_count [[buffer(5)]],
    constant uint& col_start [[buffer(6)]],
    constant uint& first_chunk_size [[buffer(7)]],
    constant uint& second_chunk_size [[buffer(8)]],
    constant uint& first_pass [[buffer(9)]],
    constant uint& final_pass [[buffer(10)]],
    uint gid [[thread_position_in_grid]]) {
    if (gid >= leaf_count) {
        return;
    }
    ...
    uint pass_count = second_chunk_size == 0u ? 1u : 2u;
    // Keep one inlined permutation body and loop over it. Two syntactic call
    // sites duplicate the large round body; an out-of-line call avoids that
    // duplication but costs two calls per leaf.
    #pragma clang loop unroll(disable)
    for (uint pass = 0; pass < pass_count; ++pass) {
        if (pass != 0u) {
            for (uint i = 0; i < second_chunk_size; ++i) {
