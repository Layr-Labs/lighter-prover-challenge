...
// quintic-multiplication=4, quintic-squaring=5, exponentiation=6, equality=7,
// reducing=8, random-access=9), operation or copy count, and three explicit
// kind words. Random access uses the final words for index bits, extra
// constants, and the raw constant-column base; other layouts retain their
// documented addend/result/carry meanings.
...
kernel void range_check_gate_quotient(
    ...
    for (uint u32_index = 0; u32_index < u32_count; ++u32_index) {
        ...
        if (kind == 0u) {
            ...
            // The Rust encoder emits only known discriminants. If a malformed
            // record reaches the shader, fail closed on every selected row.
            range_check_gate_emit(
