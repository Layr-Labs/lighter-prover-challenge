...
// exponentiation=7, equality=8, reducing=9), operation or copy count, and three
// explicit kind words. Random access uses the final words for index bits, extra
// constants, and the raw constant-column base. Exponentiation carries its
// power-bit count in the operation slot and leaves the three kind words zero.
// Equality carries its constant column, and reducing its extension-coefficient
// flag, in the addend slot; both leave the two width words zero.
