...
// Fast Goldilocks addition for a literal smaller than 2^32. If the u64 sum
// wraps, folding 2^64 = 2^32 - 1 (mod p) cannot overflow a second time.
...
// Fast Goldilocks subtraction for a literal smaller than 2^32. A borrow leaves
// r near 2^64, so subtracting epsilon cannot borrow again.
        ...
        for (uint op = 0; op < num_ops; ++op) {
            ...
            for (uint j = 0; j < num_aux; ++j) {
                ...
                if (j + 1u == num_aux && final_limb_range == 2u) {
                    ...
                    constraint = gl_mul(y, gl_add_small(y, 2));
                }
                range_check_gate_emit(
                    constraint,
                    alpha_powers,
    ...
    for (uint u32_index = 0; u32_index < u32_count; ++u32_index) {
        ...
        if (kind == 0u) {
            ...
            for (uint op = 0; op < num_ops; ++op) {
                ...
                ulong high_not_max = gl_sub_small(gl_mul(inverse, high_diff), 1);
                range_check_gate_emit(
