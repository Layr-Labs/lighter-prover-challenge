...
kernel void range_check_gate_quotient(
    ...
    // RandomAccessGate. Routed block per copy: access index, claimed element,
    // then `1 << bits` list items; index bits follow ALL routed wires at
    // `routed + copy * bits + i`. Per copy: `bits` booleanity checks, the
    // binary decomposition (Horner over bits, most significant first), then
    // the pairwise fold compared against the claimed element. Finally one
    ...
    // BaseSumGate<B>: sum on wire 0, base-B limbs on 1..1+num_limbs. Emits
