...
// Multiplying by four is two doublings. A doubling is one field add, so
// this replaces a full 128-bit widening multiply and its reduction with two
// adds whose carries the existing epsilon fold already handles. Same value,
// by the same homomorphism the adds already satisfy.
