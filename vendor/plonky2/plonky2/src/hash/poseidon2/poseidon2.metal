...
kernel void range_check_gate_quotient(
    ...
    for (uint u32_index = 0; u32_index < u32_count; ++u32_index) {
        ...
        if (kind == 0u) {
            ...
            ulong exponent_base_minus_one =
                gl_sub(wires[(ulong)0 * lde_rows + source_row], 1);
            // Each intermediate is the next iteration's recurrence input and
            // the final output witness. Keep it across iterations rather than
            // loading the same wire a second time. Constraints remain in their
            // original 0..n recurrence order, followed by output constraint n.
            ulong previous_intermediate = 1;
            for (uint i = 0; i < num_power_bits; ++i) {
                ulong previous = i == 0u
                    ? 1
                    : gl_mul(previous_intermediate, previous_intermediate);
                ulong current_bit =
                    wires[((ulong)1u + (num_power_bits - i - 1u)) * lde_rows + source_row];
                // b * base + (1 - b) = 1 + b * (base - 1).
                ulong multiplier =
                    gl_add(1, gl_mul(current_bit, exponent_base_minus_one));
                ulong intermediate =
            ...
            range_check_gate_emit(
                gl_sub(output_value, previous_intermediate),
                alpha_powers, alpha_stride, gate_accumulators,
                constraint_index++);
        } else if (kind == 8u) {
