...
//
// For an actual square the two cross-products are identical. Forming their
// doubled base-2^32 limbs explicitly uses three native 32x32 products instead
// of asking the generic four-product multiplier to rediscover that symmetry.
// The doubled cross-product is (cross_top, cross_high, cross_low); adding the
// high limb of p00 and low limbs of p11 below is ordinary multi-limb addition.
inline ulong gl_square(ulong a) {
    uint2 av = as_type<uint2>(a);
    uint2 p00 = as_type<uint2>((ulong)av.x * (ulong)av.x);
    uint2 p01 = as_type<uint2>((ulong)av.x * (ulong)av.y);
    uint2 p11 = as_type<uint2>((ulong)av.y * (ulong)av.y);

    uint cross_low = p01.x << 1;
    uint cross_high = (p01.y << 1) | (p01.x >> 31);
    uint cross_top = p01.y >> 31;

    uint l0 = p00.x;
    uint l1 = p00.y + cross_low;
    uint carry_to_high = (uint)(l1 < p00.y);
    uint h0 = p11.x + cross_high;
    uint carry_to_top = (uint)(h0 < p11.x);
    uint next = h0 + carry_to_high;
    carry_to_top += (uint)(next < h0);
    h0 = next;
    uint h1 = p11.y + cross_top + carry_to_top;
...
inline ulong pow7(ulong value) {
    ulong value2 = gl_square(value);
    ulong value3 = gl_mul(value, value2);
    ulong value6 = gl_square(value3);
