...
inline void external_linear_layer_capacity_lazy(
    ...
    // Let A/B/C be the three input blocks and M the 4x4 matrix. Capacity is
    // MC + (MA + MB + MC) = M(A + B + 2C). Form that input block first and
    // execute M once instead of executing three Mat4s and combining outputs.
    for (uint i = 0; i < 4; ++i) {
        lazy_t a = lazy_of(state[i]);
        lazy_t b = lazy_of(state[i + 4]);
        lazy_t c = lazy_of(state[i + 8]);
        capacity[i] = lazy_add(lazy_add(a, b), lazy_add(c, c));
    }
    mat4(capacity);
}
...
inline void external_linear_layer_digest(thread ulong state[12]) {
    // Digest is MA + (MA + MB + MC) = M(2A + B + C). Linearity lets the
    // unobserved blocks collapse before Mat4, deleting two complete Mat4s and
    // the cross-block output reduction while retaining the exact four lanes.
    lazy_t digest[4];
    for (uint i = 0; i < 4; ++i) {
        lazy_t a = lazy_of(state[i]);
        lazy_t b = lazy_of(state[i + 4]);
        lazy_t c = lazy_of(state[i + 8]);
        digest[i] = lazy_add(lazy_add(a, a), lazy_add(b, c));
    }
    mat4(digest);
    for (uint i = 0; i < 4; ++i) {
        state[i] = lazy_materialize(digest[i]);
    }
}
