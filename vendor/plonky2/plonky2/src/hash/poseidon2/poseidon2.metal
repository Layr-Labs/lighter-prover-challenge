...
kernel void poseidon2_gate_quotient(
    ...
    ulong filter = 1;
    // Wires and constants come from the retained `MetalColumns` store, never
    // an even-row companion. That store is canonical on the GPU build path
    // and is folded on the CPU Merkle decline path.
    ...
    for (uint i = 0; i < 4; ++i) {
        ...
        poseidon2_gate_emit(
            gl_sub_canonical_rhs(gl_mul(swap, gl_sub_canonical_rhs(rhs, lhs)), delta),
            alpha_powers,
            accumulators,
            constraint_index);
...
kernel void permutation_quotient(
    ...
    for (uint chunk = 0; chunk < num_chunks; ++chunk) {
        ...
        // `to_canonical_u64()` in `start_permutation_quotient`.
        // Numerator `beta_k*x + wire + gamma` and denominator
        // `beta*sigma + wire + gamma` share `wire + gamma`. One
        // canonical-rhs add per challenge, then two mul-adds.
        ulong wire_gamma0 = gl_add_canonical_rhs(wire, gamma0);
        ulong wire_gamma1 = gl_add_canonical_rhs(wire, gamma1);
        ulong numerator0 = gl_mul_add(beta_k0, x, wire_gamma0);
        ulong denominator0 = gl_mul_add(beta0, sigma, wire_gamma0);
        ulong numerator1 = gl_mul_add(beta_k1, x, wire_gamma1);
        ulong denominator1 = gl_mul_add(beta1, sigma, wire_gamma1);
        for (uint j = j_start + 1u; j < j_end; ++j) {
            ...
            wire_gamma0 = gl_add_canonical_rhs(wire, gamma0);
            wire_gamma1 = gl_add_canonical_rhs(wire, gamma1);
            numerator0 = gl_mul(numerator0, gl_mul_add(beta_k0, x, wire_gamma0));
            denominator0 = gl_mul(denominator0, gl_mul_add(beta0, sigma, wire_gamma0));
            numerator1 = gl_mul(numerator1, gl_mul_add(beta_k1, x, wire_gamma1));
            denominator1 = gl_mul(denominator1, gl_mul_add(beta1, sigma, wire_gamma1));
        }
...
inline ulong alpha_acc_materialize(alpha_acc_t acc) {
    ...
    // `H0 + H1` is ≤ 2 n (2^32 - 1). Host admission requires
    // `2 * alpha_stride * sizeof(u64) ≤ 4096`, so n ≤ 256 and the sum is
    // below 2^41 < p. One fold; bit-identical given that bound.
...
kernel void range_check_gate_quotient(
    ...
    for (uint range_index = 0; range_index < range_count; ++range_index) {
        ...
        ulong selector = constants[(ulong)selector_column * lde_rows + source_row];
        ulong filter = 1;
        // Selector comes from the constants store (or its even-row copy,
        // taken after that store is built). Those limbs are < p, so the
        // second EPSILON fold in `gl_sub` is unreachable.
        for (uint i = group_start; i < group_end; ++i) {
            if (i != gate_index) {
                filter = gl_mul(filter, gl_sub_canonical_rhs((ulong)i, selector));
            }
