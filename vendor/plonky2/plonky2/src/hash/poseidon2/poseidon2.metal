...
// Tile v2b: TG width 32 (4x tip's 128 group is still 4x better occupancy than
// the failed TG=16 experiment that scored 18.5). Tile 32 rows x 128 cols =
// 32 KiB exactly (Apple TG memory cap). Light alpha_stride=136: columns
// 0..127 from tile, 128..135 from device. step==1 only.
constant uint RANGE_TILE_ROWS = 32u;
constant uint RANGE_TILE_COLS = 128u;
...
inline ulong load_wire_tiled(
    ...
    uint lde_rows,
    uint source_row) {
    if (use_tile && col < n_cols && col < RANGE_TILE_COLS) {
...
kernel void range_check_gate_quotient(
    ...
    if (use_tile) {
        // Load min(alpha_stride, 128) columns for 32 consecutive gids.
        const uint n_cols = alpha_stride < RANGE_TILE_COLS ? alpha_stride : RANGE_TILE_COLS;
