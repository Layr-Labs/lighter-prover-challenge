...
kernel void range_check_gate_quotient(
    ...
    for (uint range_index = 0; range_index < range_count; ++range_index) {
        ...
        ulong selector = constants[(ulong)selector_column * lde_rows + source_row];
        // A row's selector value is its gate's index in the sorted gate list,
        // or the UNUSED sentinel for padding rows. The zero-product filter is
        // therefore nonzero exactly when the selector names this spec's gate;
        // every other row's constraint work would be multiplied by zero.
        // Skipping it makes the GPU cost proportional to the actual gate at
        // each row instead of the full spec roster.
        if (selector == (ulong)gate_index) {
            ulong filter = 1;
            for (uint i = group_start; i < group_end; ++i) {
                if (i != gate_index) {
                    filter = gl_mul(filter, gl_sub((ulong)i, selector));
    ...
    for (uint u32_index = 0; u32_index < u32_count; ++u32_index) {
        ...
        // or the UNUSED sentinel for padding rows. The zero-product filter is
        // therefore nonzero exactly when the selector names this spec's gate;
        // skipping the constraint work on every other row makes the GPU cost
        // proportional to the actual gate at each row instead of the full
        // spec roster (which is what made extra Metal families regress).
        if (selector == (ulong)gate_index) {
        ulong filter = 1;
        for (uint i = group_start; i < group_end; ++i) {
            if (i != gate_index) {
