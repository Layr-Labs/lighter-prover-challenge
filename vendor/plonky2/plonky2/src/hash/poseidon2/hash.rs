...
mod pair_hash_tests {
    ...
    #[test]
    fn permutation_x4_matches_scalar() {
        for _ in 0..64 {
            let mut scalar: [Poseidon2Permutation<F>; 4] =
                core::array::from_fn(|_| {
                    Poseidon2Permutation::new(core::iter::repeat_with(F::rand).take(WIDTH))
                });
            let mut batched = scalar;

            for state in &mut scalar {
                state.permute();
            }
            let [a, b, c, d] = &mut batched;
            <Poseidon2Permutation<F> as PlonkyPermutation<F>>::permute_x4(a, b, c, d);
    ...
    #[test]
    fn time_scalar_vs_x4_permutations() {
        let initial: [Poseidon2Permutation<F>; 4] = core::array::from_fn(|_| {
            Poseidon2Permutation::new(core::iter::repeat_with(F::rand).take(WIDTH))
        });
        ...
        assert_eq!(batched, scalar);
        println!(
            "scalar: {scalar_time:?} x4: {batched_time:?} speedup: {:.2}x",
