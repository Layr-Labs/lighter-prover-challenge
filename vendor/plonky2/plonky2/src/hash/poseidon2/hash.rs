...
impl Poseidon2 for F {
    #[inline]
    fn internal_linear_layer(state: &mut [Self; WIDTH]) {
        // Each lane fuses its diagonal product and the shared `sum` addend
        // into one `multiply_accumulate`, so a lane costs a single field
        // reduction instead of one for the product and another for the add.
