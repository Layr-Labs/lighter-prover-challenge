...
/// This is raw-u64 identical to two calls to `multiply`.
#[inline(always)]
...
// ========================================== FULL ROUNDS ==========================================
/// Reference full S-box using the scalar `multiply` body.
#[inline(always)]
#[unroll_for_loops]
...
/// Full S-box with adjacent independent products interleaved for ILP.
/// Pairing stays inside the x², (x³, x⁴), and x⁷ dependency waves.
#[inline(always)]
unsafe fn sbox_layer_full(state: [u64; WIDTH]) -> [u64; WIDTH] {
    ...
    while i < WIDTH {
        let (lo3, hi3) =
            mul_reduce_pair(state[i], state2[i], state[i + 1], state2[i + 1]);
        let (lo4, hi4) =
            mul_reduce_pair(state2[i], state2[i], state2[i + 1], state2[i + 1]);
        state3[i] = lo3;
        state3[i + 1] = hi3;
        state4[i] = lo4;
        state4[i + 1] = hi4;
    ...
    while i < WIDTH {
        let (lo, hi) =
            mul_reduce_pair(state3[i], state4[i], state3[i + 1], state4[i + 1]);
        state7[i] = lo;
        state7[i + 1] = hi;
        i += 2;
    }
