...
pub enum MerkleLeaves<F> {
    ...
    /// `columns[j][reverse_bits(i, log_rows)]` for each column `j`. This is the
    /// layout LDEs are produced in, so committing to them requires no transpose.
    Columns {
        columns: Vec<Vec<F>>,
        log_rows: usize,
    },
...
impl<F: RichField, H: Hasher<F>> MerkleTree<F, H> {
    ...
    pub fn leaf_width(&self) -> usize {
        match &self.leaves {
            ...
            MerkleLeaves::Columns { columns, .. } => columns.len(),
        }
    }
    ...
    pub fn leaf_vec(&self, i: usize) -> Vec<F> {
        match &self.leaves {
            ...
            MerkleLeaves::Columns { columns, log_rows } => {
                ...
                columns.iter().map(|column| column[natural]).collect()
            }
        }
    }
